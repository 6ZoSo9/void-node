...
The workflow constructs a fresh nonsparse 402,653,184-byte image, formats it ext4 with lazy inode/journal initialization disabled, mounts it only for the focused proof, creates one empty store directory, and passes that directory to the Node proof. The Node proof then:

1. opens the store root no-follow, verifies ext4 magic `0xef53`, and records filesystem-wide free-block state before the anonymous candidate exists;
2. opens one anonymous `O_TMPFILE` below the retained root, mode 0600, nlink 0, size 0, and records the next filesystem-wide snapshot;
3. launches exactly one bounded absolute `/usr/bin/fallocate --length 67108864 /proc/self/fd/3` with the anonymous payload fd inherited only as child fd 3;
4. requires the exact anonymous inode identity, owner UID, mode 0600, nlink 0, exact length, and at least 64 MiB of `st_blocks` allocation;
5. records the exact ext4-wide free-block snapshots and deltas without claiming they are byte-for-byte attributable to one inode; the inode-local `st_blocks * 512 >= 64 MiB` predicate is the reservation authority;
6. writes exactly 1,024 positioned 65,536-byte payload calls, fsyncs the anonymous inode, and requires the inode's allocation to remain unchanged from the post-`fallocate` reservation;
7. records the post-write filesystem-wide free-block state as observational evidence rather than a single-inode equality assertion;
...
10. launches exactly one bounded absolute, hash-pinned `/usr/bin/ln -L -T -- /proc/self/fd/3 /proc/self/fd/4/<S1>` with payload fd 3 and retained root fd 4;
11. verifies that S1 is the exact anonymous inode with the same allocated-block count, directory-fsyncs the root, and runs one test-only duplicate-link attempt proving the occupied destination is not replaceable;
12. closes the writable payload fd; and
13. freshly opens S1 read-only/no-follow, requires the same inode-local allocation, performs a separate 1,025-call full-H/EOF post-publication readback, and records the terminal ext4-wide free-block state.

Two earlier exact-head runs usefully falsified an invalid accounting assumption. The first showed that measuring only after `O_TMPFILE` creation omitted an observed 4,096-byte ext4-wide change. Moving the baseline earlier then showed that even the combined filesystem-wide `bfree` delta does not equal one inode's `st_blocks` byte-for-byte across the hosted ext4 proof. The corrected source does not add a tolerance and does not weaken the reservation requirement: it treats filesystem-wide free-block movement as exact observational evidence while requiring the candidate/published inode itself to remain fully reserved by `st_blocks` before and after write/publication/readback.
