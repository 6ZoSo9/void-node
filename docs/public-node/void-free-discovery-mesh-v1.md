...
- The IndexNow key and committed discovery config are opened with `O_NOFOLLOW`, bounded from descriptor metadata before body retention, read through the opened descriptor, checked for stable file metadata, and rechecked against the selected pathname before their bytes are accepted. The external key file ceiling is exactly 129 bytes (128 accepted key characters plus one optional newline); the committed config ceiling is 65,536 bytes.
