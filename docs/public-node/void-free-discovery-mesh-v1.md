...
- The IndexNow key and committed discovery config are opened with `O_NOFOLLOW`, bounded from descriptor metadata before body retention, and read through the opened descriptor with a second cap-plus-one boundary. If an admitted same inode grows during the read, the builder retains at most one detection byte beyond the applicable ceiling and HOLDs. Stable file metadata and the selected pathname generation are then rechecked before accepted bytes receive authority. The external key file ceiling is exactly 129 bytes (128 accepted key characters plus one optional newline); the committed config ceiling is 65,536 bytes.
- Config admission is closed and exact across the activation, provider, cost, authority, path, network, and source contract. Authority-bearing value drift and unknown fields HOLD before pack construction. Receipt claims are derived from the admitted activation/provider/cost/authority values rather than asserted independently of the config digest.
...
The proof exercises origin, date, key, path, same-host, output-location, inventory, cost, and authority boundaries. It admits the exact 128-character key plus newline, rejects a 130-byte key file and a sparse one-gigabyte key file from descriptor metadata before body retention, and applies a separate finite ceiling to the committed config. Deterministic after-open adversaries also grow the same admitted key and config inodes to exactly their respective ceiling plus one; each read must HOLD at that byte boundary without unbounded retention. A table-driven config proof flips every activation, provider, cost, and authority boolean class, changes a provider mode, and adds unknown authority fields; all variants must HOLD under the closed contract before publication. The proof additionally replaces opened key/config pathnames, injects output-parent symlink components, swaps an output ancestor, creates a foreign destination after private build bytes exist, mutates staged content and the staged receipt after receipt construction, and mutates a file after reserved publication but before the relevant verification pass. All such generation or content changes must HOLD; replacement namespaces receive zero writes, the exact foreign destination generation remains untouched, and no compromised pack detected by those passes survives as a successful result. It also proves the explicit machine-readable boundary that hostile concurrent same-UID mutation is not excluded and that consumers must reverify receipt hashes after handoff. CI is limited to public repositories so this lane cannot consume private-repository hosted-runner minutes.
