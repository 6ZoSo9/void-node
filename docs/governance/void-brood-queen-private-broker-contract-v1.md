...
### Exact broker nonce/session authority and committed recovery

Broker bootstrap authority states are exactly `FRESH` and `COMMITTED`.

The broker-specific outer policy layer does not introduce a second replay cache or a second session-creation commit. The exact parent-session transcript authority and exact outer broker-policy envelope are consumed by one versioned durable transition:

`FRESH(nonce, parent_session_transcript_sha256, outer_broker_envelope_sha256) -> COMMITTED(nonce, parent_session_transcript_sha256, outer_broker_envelope_sha256, session_id, session_generation="0", role_generation, role_record_sha256, parent_policy_sha256, broker_policy_sha256, bootstrap_receipt)`

There is no authoritative consumed-without-session state. If two contenders observed the same `FRESH` version, at most one may create fresh authority. A stale exact contender may only converge on the already committed generation-0 session and identical receipt.

Committed recovery is stricter than fresh admission. It must rederive the exact 13-field parent transcript, revalidate the exact parent server signature, Crown session approval and requester proof-of-possession, rederive the exact outer broker envelope, revalidate the exact broker signature and Crown outer broker-policy approval, and prove both derived digests and every stored authority identity are byte/identity-equivalent to the committed record.

Because committed recovery creates zero fresh authority, it may return the already committed session/receipt after the nonce is consumed, the original challenge expiry has passed, or canonical role authority later advances. Those later facts may invalidate ordinary task/rotation authority, but they do not rewrite which exact session/receipt was already committed. Any parent-transcript, requester-key, server identity, signature/PoP, outer-envelope, broker-generation key, capability, policy, role identity, session-id, or approval mismatch is terminal `BOOTSTRAP_CONFLICT` with zero state mutation.
