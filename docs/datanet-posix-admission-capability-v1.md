...
This standalone regression gate is valid only for the accepted proof source blob:

`c4d92dcaaed8879bcd98b96e199729098712d5b4`

The workflow must fail closed before execution if `git hash-object scripts/prove_datanet_posix_admission_capability_v1.py` differs from that blob. This restores the dedicated regression surface without modifying or silently substituting the accepted POSIX proof implementation.

Acceptance of this standalone gate requires the accepted source blob to remain preserved, this document and its workflow to be present, the blob guard to pass, and all nine bounded proof groups to pass at the exact refreshed head.

`full_v23_acceptance=false`
