# DataNet WC first work-pack private operator ledger-line example hold v1

Marker: `VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_LINE_EXAMPLE_HOLD_V1`

This brick provides a private/operator-only ledger-line example packet for a future DataNet first work-pack Work Credits award append.
...
This shows an example private operator ledger-line packet for a future award append review. It does not create an effective ledger line, append the ledger, issue WC, create a reward, or transfer VOID.

The example amount is 100 WC. That number is an example placeholder only; it is not an approval and not a supply limit.

## Boundary

- Private/operator-only ledger-line example
- No public route
