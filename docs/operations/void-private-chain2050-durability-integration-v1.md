...
1. the canonical prepared-transaction submission path can reach `eth_sendRawTransaction` without first arming durability debt;
2. a Buy VOID transaction can be confirmed and debt can later be satisfied against a reset or reorged Chain-2050 history at the same or greater height; and
3. a later Anvil start can load a valid but stale baseline instead of the highest durable state required by economic truth.
...
Only after reconciliation validates the transaction and reaches the configured confirmation floor does the execution-attempt journal contain a canonical confirmed record. New confirmed records bind the receipt block hash into their delivery fingerprint. The prepared-transaction reconciliation path also append-only persists the exact terminal receipt in the broadcast-evidence journal, providing an independent cross-check, including:

- delivery chain `2050`;
- exact delivery transaction hash;
- exact delivery block number; and
- exact delivery block hash.

The checkpoint minimum and delivery-history proof must therefore be bound to that confirmed receipt, not guessed immediately after submission. The recorded hash is revalidated during checkpoint capture and again against the post-capture receipt and canonical block.
...
Both canonical broadcast entry points wrap the Chain-2050 broadcaster:

- the native delivery dependency injector; and
- the prepared-transaction Chain-2050 transport used by the private broadcaster service.

The prepared transport applies the wrapper unconditionally before its only `eth_sendRawTransaction` call. There is no submission-enabled configuration that exposes the raw native broadcaster directly.
...
5. requires an exact confirmed receipt delivery block hash from the confirmed record or recoverable prepared-transaction terminal evidence;
6. when prepared terminal evidence is present, requires it to be `confirmed` and exactly match the attempt, transaction hash, delivery block number, delivery block hash, and Chain-2050 receipt;
7. invokes the merged #1177 checkpoint tool against the server-controlled loopback Chain-2050 RPC;
8. passes the confirmed delivery block as both `--minimum-block-number` and `--expected-delivery-block-number`, plus the receipt hash as `--expected-delivery-block-hash`;
9. requires the tool to read and match that exact delivery block hash before and after `anvil_dumpState`, while retaining the existing stable-head bracket;
10. requires a finalized checkpoint on chain `2050` at or above that block with the exact delivery binding;
11. re-reads the receipt and its canonical block from loopback RPC after capture;
12. requires the receipt transaction, success status, block number, and block hash to match the journal, and the canonical block hash to match the receipt; and
13. append-only records the delivery block number, delivery block hash, checkpoint identity, and successful continuity verification before the active claim can be reused.

If checkpoint capture, receipt continuity, or satisfaction fails, the active debt remains blocking. This includes a reset that removes the receipt and a reorg that replaces the delivery block at the same height. A later successful retry may reuse the content-addressed checkpoint generated by an earlier partial run.
...
| confirmed record and recoverable terminal evidence both lack an exact block hash | active debt; checkpoint capture is not attempted | no |
| receipt confirmed, before checkpoint | active durability debt bound to exact confirmed block number and hash | no |
| checkpoint sees a reset/reorg delivery-block hash | capture HOLD; active debt remains | no |
| checkpoint written, receipt missing after reset | post-capture continuity fails; active debt remains | no |
| checkpoint written, delivery block reorged | canonical block hash continuity fails; active debt remains | no |
| checkpoint written, satisfaction write fails | active debt remains; checkpoint may be reused | no |
| satisfaction durable | finalized checkpoint covers the still-canonical exact confirmed receipt history | yes |
...
Prepared-transaction transport proof:

```bash
npx tsx scripts/prove_buy_void_prepared_transaction_chain2050_transport_v1.ts
```
