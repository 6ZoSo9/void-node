...
- Required signed transaction sequence: blocks `37368`, `37369`, `37370`, and `37371`.
...
4. each expected transaction hash and recovered sender;
...
A block header may be `complete` only when its block hash, parent hash, timestamp, mix hash, and parent-header digest are supplied with `guessed_values=false`. Those normalized values are retained in the plan and therefore bound by its SHA-256 identifier. The current block-37369 evidence must name only its still-unknown `timestamp`; its mix hash is deterministically derived and its parent header is the preserved exact block-37368 header. Guessed values and stale claims that those known fields are missing are rejected.
