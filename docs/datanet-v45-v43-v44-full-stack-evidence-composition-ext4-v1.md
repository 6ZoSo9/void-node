...
The V45 acceptance object is the downstream terminal source-execution receipt together with the exact Node 22/24/26 aggregate bytes it binds from the same exact head and workflow run. The retained top artifact contains that pair plus the independently bound downstream selftest, external source A→B→A control, relabeled-selftest argv control, and preexisting-output control. Three separate green matrix jobs, a candidate receipt, a per-node aggregate, an unaccompanied top JSON object, or a static fixture declaration are insufficient. A later source change makes every earlier receipt stale.
