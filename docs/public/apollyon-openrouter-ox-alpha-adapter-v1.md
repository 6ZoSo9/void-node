...
Immediately before chat POST, provider execution is durably admitted and transitions to `UNCERTAIN`. Crash, timeout, malformed/oversize provider output, or another ambiguous outcome after that point never grants automatic resend authority. After a validated provider response, the broker writes one private mode-0600 staged capsule, file-fsyncs it, directory-fsyncs the staged generation, create-only hard-links that exact inode to the canonical final name, directory-fsyncs the final publication, removes the stage name, directory-fsyncs cleanup, and rereads the exact final generation **before** publishing `PROVIDER_RESULT`. If a complete broker-private staged/final capsule survives while the ledger is `UNCERTAIN`, an exact retry may finalize/verify that same generation and append `PROVIDER_RESULT` under the operation flock with **zero provider-send callback**; partial, noncanonical, mismatched, symlinked, or foreign generations HOLD and never restore send authority. Durable `ACCEPTED` disclosure then requires the read-only replay capability and performs no catalog/chat. Once durable `ACCEPTED` has been proven, later flock-release/cleanup failure is resource-terminal only and cannot replace or hide the committed operation result.
