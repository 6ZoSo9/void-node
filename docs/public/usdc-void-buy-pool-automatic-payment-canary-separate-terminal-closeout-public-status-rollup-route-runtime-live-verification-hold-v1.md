# USDC/VOID Buy Pool Automatic Payment Canary Separate Terminal Closeout Public Status Rollup Route Runtime Live Verification Hold v1
...
Define a read-only runtime live verification hold for the public status rollup route.

Route:

`/public-node/usdc-void-buy-pool/automatic-payment-canary-separate-terminal-closeout-public-status-rollup-v1.json`

This hold verifies whether the running node serves the public status route.
...
If the node is not running locally, the proof reports runtime unavailable without failing the sealed static route proof.
