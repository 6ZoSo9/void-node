...
Publication removes already-durable logical receipts, reuses the active bytes admitted from that same descriptor-bound scan, writes the complete next shard generation to an exclusive temporary file, fsyncs it, atomically renames it over the prior generation, and fsyncs the receipts directory before publishing the same truth in memory. A fault before publication leaves the prior shard intact; a fault after rename but before directory sync is classified by the next bounded scan, and an exact durable retry re-fsyncs the visible directory generation before a cold lookup or append can return success. The retry therefore cannot duplicate, conflict with, or prematurely bless the receipt.
