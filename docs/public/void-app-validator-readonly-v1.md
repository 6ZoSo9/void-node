...
The view does not create a second validator protocol. Its only source is the existing public-safe definition-only matrix:
...
The browser client performs one same-origin `GET` using `cache: no-store`, `credentials: omit`, redirect rejection, same-origin fetch mode, and no referrer. The response must be stream-readable and is capped at 128 KiB before JSON parsing.
...
This keeps the lane path-disjoint from the active DataNet adapter while avoiding a broad edit to `views.js`.
