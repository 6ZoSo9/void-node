...
- any administrator-set ETH/VOID opening price.
...
VOID does not make USD an input to VOID's canonical market policy.
...
- one market's quote reserves must not silently back another market;
...
is genuinely asset-agnostic, but prior USDC-specific assumptions are not carried
forward as ETH/VOID market authority. Native BTC still requires its own Bitcoin
...
This policy does not fund any 10,000,000-VOID market inventory and does not
activate any market. Vault identity, market code, opening price discovery,
