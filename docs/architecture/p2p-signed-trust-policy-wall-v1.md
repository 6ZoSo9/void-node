...
Conversely, an absent `current` pointer is accepted as fresh genesis only when `activation.ndjson` is absent and `generations/` is empty. If journal evidence, a completed generation, or any other retained generation entry survives while `current` is missing, activation fails closed before staging, generation creation, pointer publication, or journal mutation. The wall does not implicitly choose, reconstruct, or replace an authoritative current generation from orphaned durable state.
...
- Missing `current` plus any surviving journal or generation state is a fail-closed hold before activation-state mutation; only an otherwise empty state may enter genesis.
