# USDC/VOID automatic payment activation candidate preflight status rollup hold v1

Marker: `VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_CANDIDATE_PREFLIGHT_STATUS_ROLLUP_HOLD_V1`

This private/operator-only brick rolls up the activation candidate preflight chain status as held closed.

This is a preflight status rollup hold, not activation, not activation release, not execution, and not automatic fulfillment enablement.

Source fixture: `fixtures/private/usdc-void-buy-pool-automatic-payment-activation-candidate-preflight-complete-closeout-hold-v1.json`
...
- Status rollup generated: `true`
- Preflight chain status: `complete_as_hold_only`
- Preflight complete closeout held: `true`
...
- Preflight status rollup is held closed.
- The preflight chain status is complete as hold only.
- All preflight gates are held.
...
Next required gate: `activation_candidate_preflight_status_rollup_closeout_hold_v1`
