...
contract MockVoidTokenEpochSnapshot {
...
contract ValidatorEpochSnapshotTest is Test {
    MockVoidTokenEpochSnapshot internal token;
    ...
    function setUp() public {
        token = new MockVoidTokenEpochSnapshot();
    ...
    function test_captureEpoch1_freezesOrderedSet() public {
        vm.prank(admin);
        snapshot.captureEpoch(1);

        assertEq(snapshot.getEpochValidatorCount(1), 3);
        assertEq(snapshot.getEpochTotalPower(1), 4500 ether);

        IValidatorSelectionSource.SelectableValidator memory v0 = snapshot.getEpochValidatorAt(1, 0);
        IValidatorSelectionSource.SelectableValidator memory v1 = snapshot.getEpochValidatorAt(1, 1);
        IValidatorSelectionSource.SelectableValidator memory v2 = snapshot.getEpochValidatorAt(1, 2);
    ...
    function test_captureEpoch2_afterExit_keepsEpoch1Stable() public {
        vm.prank(admin);
        snapshot.captureEpoch(1);
        ...
        assertEq(snapshot.getEpochValidatorCount(2), 2);
        assertEq(snapshot.getEpochTotalPower(2), 2500 ether);
