...
contract ValidatorSelectionAdapterTest is Test {
    MockVoidTokenAdapter internal token;
    ...
    function setUp() public {
        token = new MockVoidTokenAdapter();
    ...
    function test_adapter_reportsSelectableValidator() public {
        vm.startPrank(controller);
        staking.registerAndStake(reward, consensusKey, MIN_STAKE);
        staking.activate();
        vm.stopPrank();

        assertEq(adapter.getSelectableValidatorCount(), 1);
        assertEq(adapter.totalSelectablePower(), MIN_STAKE);

        IValidatorSelectionSource.SelectableValidator memory v = adapter.getSelectableValidatorAt(0);
    ...
    function test_adapter_excludesValidatorAfterExitBegins() public {
        vm.startPrank(controller);
        staking.registerAndStake(reward, consensusKey, MIN_STAKE);
        staking.activate();
        staking.beginExit();
        ...
        assertEq(adapter.getSelectableValidatorCount(), 0);
        assertEq(adapter.totalSelectablePower(), 0);

        vm.expectRevert(ValidatorSelectionAdapter.IndexOutOfBounds.selector);
        adapter.getSelectableValidatorAt(0);
    }
}
