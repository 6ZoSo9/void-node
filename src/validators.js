...
// src/validators.ts
var LOOSE = process.env.VALIDATION_LOOSE === "1";
/** overall JSON size cap (bytes) to avoid pathological messages */
var MAX_JSON_BYTES = Number(process.env.MAX_JSON_BYTES || 1000000); // 1 MB default
var ok = function () { return ({ ok: true }); };
var bad = function (e) { return ({ ok: false, error: e }); };
/* ------------------------- hex helpers ------------------------- */
var RX_HEX_BARE = /^[0-9a-f]+$/i;
function isBareHex(s, lenChars) {
    if (typeof s !== "string")
        return false;
    if (!RX_HEX_BARE.test(s))
        return false;
    if (lenChars !== undefined && s.length !== lenChars)
        return false;
    return true;
}
function is0xHex(s, lenChars) {
    if (typeof s !== "string")
        return false;
    if (!s.startsWith("0x"))
        return false;
    var rest = s.slice(2);
    if (!RX_HEX_BARE.test(rest))
        return false;
    if (lenChars !== undefined && rest.length !== lenChars)
        return false;
    return true;
}
/** Accepts either bare or 0x-prefixed hex; optional byteLen constraint. */
function isHexEither(s, byteLen) {
    var charLen = byteLen !== undefined ? byteLen * 2 : undefined;
    return isBareHex(s, charLen) || is0xHex(s, charLen);
}
/* ------------------------- number helpers ------------------------- */
function isSafeInt(x, min) {
    if (min === void 0) { min = 0; }
...
function safeParseJSON(jsonStr) {
    if (typeof jsonStr !== "string")
        return { ok: false, error: "not string" };
    // quick byte-length guard
    var bytes = Buffer.byteLength(jsonStr, "utf8");
...
function validateTx(jsonStr) {
    ...
    // required
    if (!isHexEither(o.hash, 32))
        return bad("hash must be 32-byte hex (0x/bare)");
    if (typeof o.body !== "object" || o.body === null || Array.isArray(o.body)) {
        return bad("body must be an object");
    }
    // optional
    if (o.from !== undefined && !isHexEither(o.from, 20))
        return bad("from must be 20-byte hex");
    if (o.to !== undefined && !isHexEither(o.to, 20))
        return bad("to must be 20-byte hex");
    ...
    if (o.sig !== undefined) {
        if (LOOSE) {
            if (typeof o.sig !== "string" || o.sig.length === 0)
                return bad("sig must be non-empty string");
        }
        else {
            ...
            var okSig = isHexEither(o.sig, 65);
            if (!okSig)
                return bad("sig must be 65-byte hex (0x/bare)");
        }
    }
    return ok();
}
 ...
 */
function validateHttpAnnounce(jsonStr) {
    var parsed = safeParseJSON(jsonStr);
    if (!parsed.ok)
        return parsed;
    var o = parsed.value;
    ...
    var http = String(o.http || "").trim();
    if (!id)
        return bad("missing id");
    if (!/^https?:\/\/.+/i.test(http))
        return bad("bad http");
    if (http.length > 2048)
        return bad("http too long");
    return ok();
}
/**
 ...
 */
function validateBlockHeader(jsonStr) {
    var parsed = safeParseJSON(jsonStr);
    if (!parsed.ok)
        return parsed;
    var o = parsed.value;
    if (typeof o !== "object" || o === null)
        return bad("not object");
    var numOk = isSafeInt(o.number, 0);
    if (!numOk)
        return bad("number must be integer >= 0");
    if (!isHexEither(o.parentHash, 32))
        return bad("parentHash must be 32-byte hex");
    if (!isHexEither(o.txRoot, 32))
        return bad("txRoot must be 32-byte hex");
    if (!isHexEither(o.blobRoot, 32))
        return bad("blobRoot must be 32-byte hex");
    var tsOk = isSafeInt(o.timestamp, 1);
    if (!tsOk)
        return bad("timestamp must be a positive integer");
    if (o.proposer !== undefined) {
        var prop = String(o.proposer || "").trim();
        if (!prop)
            return bad("proposer, if present, must be non-empty");
    }
    if (o.sig !== undefined && !LOOSE) {
        // For our signed headers we use Ed25519 (64B). Accept 64B in strict mode.
        if (!isHexEither(o.sig, 64))
            return bad("sig must be 64-byte hex");
    }
    else if (o.sig !== undefined && LOOSE) {
        if (typeof o.sig !== "string" || o.sig.length === 0)
            return bad("sig must be string");
    }
    return ok();
}
/**
 ...
 */
function validateBlobAnnounce(jsonStr) {
    var parsed = safeParseJSON(jsonStr);
    if (!parsed.ok)
        return parsed;
    var o = parsed.value;
    if (typeof o !== "object" || o === null)
        return bad("not object");
    if (!isHexEither(o.cid, 32))
        return bad("cid must be 32-byte hex (sha256)");
...
function validateTopic(topic, msg) {
    var v = validators[topic];
    return v ? v(msg) : ok();
}
function hasValidator(topic) {
