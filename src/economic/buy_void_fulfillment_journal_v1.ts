...
export type BuyVoidFulfillmentJournalPreviewDecisionV1 =
  | {
      ok: true;
      status: "available" | "duplicate";
      new_claim: boolean;
      claim: BuyVoidFulfillmentClaimV1;
      intent: BuyVoidFulfillmentJournalIntentV1;
    }
...
function previewHeld(
  reason: string,
  detail?: Record<string, unknown>,
): BuyVoidFulfillmentJournalPreviewDecisionV1 {
  return {
    ok: false,
    status: "held",
    new_claim: false,
...
export function previewBuyVoidFulfillmentJournalClaimV1(
  input: ClaimBuyVoidFulfillmentJournalInputV1,
): BuyVoidFulfillmentJournalPreviewDecisionV1 {
  ...
  try {
    ...
    return previewHeld(String((error as Error)?.message || error));
  }

  const requestId = String(input?.request?.request_id || "").trim();
  if (!/^[A-Za-z0-9._:-]{3,160}$/.test(requestId)) {
    return previewHeld("invalid_request_id");
  }
  ...
  try {
    ...
    if ("reason" in decision) {
      return previewHeld(decision.reason, decision.detail);
    }
    return previewHeld("invalid_canonical_payment_identity");
  }
  ...
  try {
    ...
    if (existingPaymentRaw) {
      ...
      if ("reason" in compatibility) {
        return previewHeld(compatibility.reason, compatibility.detail);
      }
      return {
        ok: true,
        status: "duplicate",
    ...
    if (existingRequestRaw) {
      ...
      if (
        ...
        return previewHeld("journal_inconsistent_missing_payment_claim", {
          request_id: requestId,
          canonical_payment_identity: canonicalPaymentIdentity,
        });
      }
      return previewHeld("request_already_claimed", {
        request_id: requestId,
    ...
    if (!binding) return previewHeld("incomplete_verification_binding");
    ...
    return previewHeld("fulfillment_journal_preview_failed", {
