...
export const VOID_BUY_VOID_PREPARED_TRANSACTION_BROADCASTER_INSPECTION_ACTIVATION_V1 =
  "VOID_BUY_VOID_PREPARED_TRANSACTION_BROADCASTER_INSPECTION_ACTIVATION_V1";

export const VOID_BUY_VOID_PREPARED_TRANSACTION_BROADCASTER_INSPECTION_ACTIVATION_CONFIRMATION_V1 =
  "buyVoidStartPreparedTransactionBroadcasterInspectionOnlyV1";

export const VOID_BUY_VOID_PREPARED_TRANSACTION_BROADCASTER_INSPECTION_ACTIVATION_AUTHORITY_V1 = {
  ...
  service_submission_enabled: false,
  submit_once_rejected_before_custody_lookup: true,
  submit_once_rejected_before_durable_intent: true,
  inspection_submission_supported: true,
  read_only_rpc_probe_possible_when_invoked: true,
  ...
  transaction_broadcast_possible: false,
...
export type BuyVoidPreparedTransactionBroadcasterInspectionActivationPolicyV1 = {
  socket_path: string;
  custody_store_dir: string;
  state_dir: string;
  expected_signer_fingerprint_sha256: string;
...
export type RunBuyVoidPreparedTransactionBroadcasterInspectionActivationInputV1 = {
  policy: Readonly<BuyVoidPreparedTransactionBroadcasterInspectionActivationPolicyV1>;
  apply?: boolean;
  confirmation?: unknown;
};

export type BuyVoidPreparedTransactionBroadcasterInspectionActivationDependenciesV1 = {
...
export type BuyVoidPreparedTransactionBroadcasterInspectionActivationDecisionV1 =
  | {
      ok: true;
      status: "dry_run";
      applied: false;
      ...
      required_confirmation:
        typeof VOID_BUY_VOID_PREPARED_TRANSACTION_BROADCASTER_INSPECTION_ACTIVATION_CONFIRMATION_V1;
      submission_enabled: false;
      ...
      authority:
        typeof VOID_BUY_VOID_PREPARED_TRANSACTION_BROADCASTER_INSPECTION_ACTIVATION_AUTHORITY_V1;
    }
  | {
      ok: true;
      status: "started";
...
function held(
  ...
  BuyVoidPreparedTransactionBroadcasterInspectionActivationDecisionV1,
  { ok: false }
> {
  return {
    ok: false,
...
export async function runBuyVoidPreparedTransactionBroadcasterInspectionActivationV1(
  input: Readonly<RunBuyVoidPreparedTransactionBroadcasterInspectionActivationInputV1>,
  dependencies: BuyVoidPreparedTransactionBroadcasterInspectionActivationDependenciesV1 = {},
): Promise<BuyVoidPreparedTransactionBroadcasterInspectionActivationDecisionV1> {
  ...
  if (
    ...
    return held(true, "broadcaster_inspection_activation_confirmation_required");
  }
  ...
  if (!socketPath || !custodyStore || !stateDir) {
    return held(apply, "broadcaster_inspection_activation_paths_invalid");
  }
  if (
    ...
    return held(
      apply,
      "broadcaster_inspection_activation_paths_must_be_distinct",
    );
  }
  if (!SHA256.test(signerFingerprint)) {
    return held(
      apply,
      "broadcaster_inspection_activation_signer_fingerprint_invalid",
    );
  }
  ...
  if (!chainTransport.ok) {
    return held(
      apply,
      "broadcaster_inspection_activation_chain_transport_not_ready",
    );
  }
  ...
  try {
    ...
    return held(
      apply,
      "broadcaster_inspection_activation_service_module_load_failed",
    );
  }
  ...
  if (
    ...
    return held(
      apply,
      "broadcaster_inspection_activation_service_factory_missing",
    );
  }
  ...
  try {
    ...
    return held(
      apply,
      "broadcaster_inspection_activation_service_create_failed",
    );
  }
  ...
  if (
    ...
    return held(
      apply,
      "broadcaster_inspection_activation_service_invalid",
    );
  }
  ...
  try {
    ...
    return held(
      true,
      "broadcaster_inspection_activation_service_start_failed",
    );
  }
