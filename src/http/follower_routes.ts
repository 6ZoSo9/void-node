  ...
  for (const candidate of raw.split(",").map((value) => value.trim()).filter(Boolean)) {
    try {
      ...
      if (!["http:", "https:"].includes(parsed.protocol)) {
        throw new Error("peer protocol must be HTTP or HTTPS");
      }
      if (parsed.username || parsed.password || parsed.search || parsed.hash) {
        throw new Error("peer origin must not contain credentials, query, or fragment");
      }
...
export function registerFollowerRoutes(app: Express, node: any, metrics?: any) {
  ...
  app.get("/follower/peers", (_req, res) => {
    try {
      res.json({ ok: true, ...(node.peersSnapshot?.() ?? {}) });
    } catch (e: any) {
      res.status(500).json({ ok: false, error: String(e?.message || e) });
    }
  });
