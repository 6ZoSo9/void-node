...
export class Node {
  ...
  subscribe (topic: string) {
    ...
    // Mark as seen so we don't re-handle our own message
    if (this.pubsub.subscribers(topic).has(this.id)) {
      const key = `${topic}:${nonce}`
      this.seen.add(key)
      this.seenTimestamps.set(key, Date.now())

      // IMPORTANT: do NOT re-enqueue our own tx locally;
      // the /tx endpoint already pushed into mempool.
      if (topic !== 'void/tx') {
        console.log(`[pubsub] ${topic} <- ${this.id}: ${data}`)
      }
    }
  }
  ...
  startProposer(intervalMs = 5000) {
    ...
    /** Seal one block; pure engine (no metrics here). */
  
    /** Safely take up to `max` txs from the mempool, regardless of its API shape. */
  private takeTxBatch(max = 1000): any[] {
    try {
      // Preferred: drain(max) -> any[]
  ...
  /** Seal one block; pure engine (no metrics here). */
  async sealBlock(): Promise<{ ok:true, number:number, txs:number }> {
    const parent = this.store.loadHeadNumber();
    const number = parent + 1;

    // Take txs robustly and validate/normalize them
    const batch = this.takeTxBatch(1000);
    const txs = batch.filter((t) =>
      t && typeof t === 'object' &&
      typeof t.body === 'object' && t.body !== null &&
      typeof t.hash === 'string' && /^[0-9a-fA-F]{64}$/.test(t.hash)
    ...
    const parentHash =
      parent >= 0 && parentBlock ? blockHash(parentBlock) : ''.padStart(64, '0');
    ...
    // Index + receipts (only if we actually have txs)
    if (b.txs?.length) {
      try {
    ...
    // Publish compact header so followers learn the tip immediately
...
// follower: one-shot pull of any missing blocks, update index & receipts
...
async pullOnce(peerHttp: string, hooks?: { onImportBlock?: (b: any) => void }) {
  ...
  // if we somehow didn’t get the tip, retry once
  ...
  for (const b of arr) {
    ...
    if (!existing) {
      ...
      if (incomingHasTxs) {
        // index + receipts for brand new block
        try {
        ...
        try {
          const anyReceipts: any = this.receipts as any
          const recs = b.txs.map((tx: any, i: number) => ({ h: String(tx.hash).toLowerCase(), n, o: i, ts: b.timestamp ?? Date.now() }))
    ...
    // “fill” header-only block with full txs
    if (!existingHasTxs && incomingHasTxs) {
      // merge header we had with incoming full block (ensure txs from incoming)
    ...
    // we either already had full block, or incoming didn’t include txs
    alreadyHad++
  }
  ...
  /** follower (HTTP-based), calls pullOnce immediately then on an interval */
  startFollower(
    peerHttp = 'http://127.0.0.1:4101',
    intervalMs = 2000,
    opts?: { onImportBlock?: (b: Block) => void }
  ) {
    let running = false
    const tick = async () => {
      if (running) return
      running = true
      try { await this.pullOnce(peerHttp, opts) } catch {}
      running = false
    }
    // run immediately, then on a cadence
    void tick()
