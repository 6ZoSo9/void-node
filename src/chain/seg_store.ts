 ...
 * Layout:
 *   data/
 *     heads.json               { head: number, currentSegment: string }
 *     segments/
 *       00000000/
...
private lastSegmentName(): string | null {
  ...
  loadHeadNumber(): number {
    return Math.max(this.heads.head, -1)
  }
  ...
  *loadTail(count: number): Generator<Block> {
  ...
  saveBlock(b: Block): void {
    // roll to a current segment (create one if missing)
    ...
    try {
      ...
      if (b.number % this.opts.sparseEvery === 0) {
        ...
        writeU64BE(rec, 4, before) // offset of this record
      ...
      // segment roll if exceeded size
      if (after >= this.opts.segmentMaxBytes) {
        // next append will go to a new segment
  ...
  /**
   * Stream blocks [from..to] in order.
   * Uses existing random-access offsets (via loadBlock) to avoid
   * re-parsing raw file slices that can include partial JSON or zeros.
   */
 
 async *findRange (from: number, to: number): AsyncGenerator<any> {
    // Clamp and validate
    ...
    for (let n = from; n <= to; n++) {
      ...
      // tiny micro-yield to keep the event loop responsive during long ranges
  ...
  private saveHeads() {
    this.ensureDir(this.root)
    fs.writeFileSync(this.headsFile, JSON.stringify(this.heads, null, 2))
  }

  private newSegmentName(): string {
    // increment from last existing segment or start at 0
    const all = this.listSegmentsSorted()
  ...
  private loadOrInitMeta(seg: string, firstBlockNum: number): Meta {
    ...
    else if (fs.existsSync(p)) m = JSON.parse(fs.readFileSync(p, 'utf8'))
    else {
  ...
  private listSegmentsSorted(): string[] {
    ...
    return fs.readdirSync(this.segDir)
      .filter((d) => /^\d{8}$/.test(d))
      .sort((a, b) => Number(a) - Number(b))
  }

  private seekOffset(seg: string, n: number): number {
    // use sparse index to find largest entry <= n
  ...
  private getSparseBuffer(seg: string): Buffer {
    ...
    // only keep last segment cached for memory economy
    this.sparseCache.clear()
    this.sparseCache.set(seg, buf)
    return buf
  }

  // simple sync iterator (used by loadTail)
...
function writeU64BE(buf: Buffer, offset: number, value: number) {
  // value <= Number.MAX_SAFE_INTEGER assumed
