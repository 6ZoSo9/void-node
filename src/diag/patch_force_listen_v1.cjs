...
(function () {
  ...
  try { console.log(`${TAG} installed pid=${process.pid}`); } catch {}
  ...
  function tryListen(app) {
    ...
    try { console.log(`${TAG} attempting app.listen(${host}:${port})`); } catch {}
    try {
      const srv = app.listen(port, host, () => {
        try { console.log(`${TAG} LISTENING ${host}:${port}`); } catch {}
      });
      // keep a handle for later inspection
      try { globalThis.__void_force_listen_v1_server = srv; } catch {}
    } catch (e) {
      const msg = String(e && (e.code || e.message || e) || e);
      if (String(e && e.code || "").includes("EADDRINUSE") || msg.includes("EADDRINUSE")) {
        try { console.log(`${TAG} already in use (EADDRINUSE) -> ok`); } catch {}
        return;
      }
      try { console.log(`${TAG} listen threw: ${msg}`); } catch {}
    }
  }
  ...
  function loop() {
    ...
    if (up > maxMs) {
      armed = false;
      try { console.log(`${TAG} giving up after ${maxMs}ms (app not found or listen not needed)`); } catch {}
      return;
    }
    const app = getApp();
    if (app) {
