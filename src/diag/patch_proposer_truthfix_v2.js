...
function fetchText(url, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request({
      ...
      method: 'GET',
      headers: { 'accept': '*/*', 'user-agent': 'void-proposer-truthfix-v2' }
    }, (res) => {
      let data = '';
...
async function computeTruth() {
  ...
  try {
    ...
    if (r.status === 200) v3bProm = parseProm(r.body);
...
function mount(app) {
  ...
  if (globalThis.__void_proposer_truthfix_v2_installed) return true;
  globalThis.__void_proposer_truthfix_v2_installed = true;
  ...
  app.get('/metrics/void/proposer.v3c.prom', async (_req, res) => {
    try {
      ...
      body += '# HELP void_proposer_v3b_enabled_exported Exporter v3b enabled (what Prom scrapes today)\n';
      ...
      res.statusCode = 500;
      res.setHeader('content-type', 'text/plain; version=0.0.4');
      res.end(`# ERROR void-proposer-truthfix-v2 ${String(e && e.message ? e.message : e)}\n`);
    }
  });
...
function wrapExpressExport(exp) {
  if (typeof exp !== 'function') return exp;
  if (exp.__void_wrapped_truthfix_v2) return exp;
  ...
  wrappedExpress.__void_wrapped_truthfix_v2 = true;
  return wrappedExpress;
}

function interceptExpressLoad() {
  if (globalThis.__void_truthfix_v2_hooked) return;
  globalThis.__void_truthfix_v2_hooked = true;

  const origLoad = Module._load;
  Module._load = function(request, parent, isMain) {
    const out = origLoad.apply(this, arguments);
    if (request === 'express') {
      try { return wrapExpressExport(out); } catch { return out; }
    }
    return out;
  };
}

// boot: hook require('express') so we mount even if __void_http_app hook is missing
interceptExpressLoad();

// also: if some code sets globalThis.__void_http_app later, poll briefly
(function pollGlobalApp() {
