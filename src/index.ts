...
(function featureFlags(){
  ...
  const F = g.__void_flags;
  if (typeof F.lastmile_v4b_enabled === "undefined") F.lastmile_v4b_enabled = true;
  if (typeof F.lastmile_legacy_enabled === "undefined") F.lastmile_legacy_enabled = false;  // v2/v3/v3b
  if (typeof F.diag_routes_enabled === "undefined") F.diag_routes_enabled = false;          // /__void/diag/*
})();
...
(function featureFlagsAPI(){
  ...
  (function attach(){
    ...
    const a:any = app();
    if (!a || typeof a.get!=="function"){ if((tries+=RETRY)<=MAX) return setTimeout(attach,RETRY); return; }
    if (a.__voidFeatureFlagsAPI) return; attached=true;

    a.get("/__void/flags", (_req:any,res:any)=> res.json(F()));
    a.post("/__void/flags/set", (req:any,res:any)=>{
      try{
        const url = new URL(req?.url || "", "http://x/");
        const name = url.searchParams.get("name")||"";
        const value = url.searchParams.get("value");
        if (!name) return res.status(400).json({ok:false,err:"name required"});
        const v = (value==="true") ? true : (value==="false") ? false : value;
        F()[name] = v;
        res.json({ok:true, flags:F()});
      }catch(e:any){ res.status(500).json({ok:false,err:String(e)})}
    });
...
(function voidAboutAndBasics(){
  ...
  async function fetchJSON(url:string){
    try{
      const r = await (await import("node-fetch")).default(url);
      return await r.json();
  ...
  (function attach(){
    ...
    const a:any = app();
    if (!a || typeof a.get!=="function"){ if((tries+=RETRY)<=MAX) return setTimeout(attach,RETRY); return; }
    if (a.__voidAboutBasics) return; attached=true;

    a.get("/__void/about", async (_req:any,res:any)=>{
      res.json(await snapshot());
    });

    a.get("/__void/metrics/void.basics.prom", async (_req:any,res:any)=>{
      const s = await snapshot();
      const lines = [];
      lines.push("# HELP void_head_number Current chain head (node view)");
      lines.push("# TYPE void_head_number gauge");
      if (typeof s.head === "number") lines.push(`void_head_number ${s.head}`);

      lines.push("# HELP void_txroot_health_live TxRoot health (1 ok, 0 bad) live");
      lines.push("# TYPE void_txroot_health_live gauge");
      if (typeof s.txroot_health_live === "number") lines.push(`void_txroot_health_live ${s.txroot_health_live}`);

      lines.push("# HELP void_uptime_ms Process uptime (ms)");
      lines.push("# TYPE void_uptime_ms gauge");
      lines.push(`void_uptime_ms ${s.uptime_ms}`);
      ...
      if (typeof s.lastmile_v4b.total === "number") lines.push(`void_lastmile_block_txs_total ${s.lastmile_v4b.total}`);
      ...
      if (typeof s.lastmile_v4b.latest_txs === "number") lines.push(`void_lastmile_latest_txs ${s.lastmile_v4b.latest_txs}`);
      ...
      if (typeof s.lastmile_v4b.last_seen_block === "number") lines.push(`void_lastmile_last_seen_block ${s.lastmile_v4b.last_seen_block}`);
  ...
  (globalThis as any).fetch = async (url:string, opts:any={})=>{
    ...
    return await new Promise((resolve,reject)=>{
      const req = mod.request({
        ...
        const chunks:any[]=[]; res.on("data",(c:any)=>chunks.push(c));
        res.on("end",()=>{
          const buf = Buffer.concat(chunks); const text = buf.toString("utf8");
          resolve({
...
(function basicsExporterV2(){
  ...
  async function withTimeout(p:Promise<any>, ms:number){
  ...
  async function snapshot(){
    ...
    if (now() - g.__void_basics_cache.ts < 1000) return g.__void_basics_cache;
    const [head, health, lm] = await Promise.all([
      readHead(), readTxrootHealthLive(), readLastmileV4b()
    ]);
    g.__void_basics_cache = { head, txroot_health_live: health, lastmile: lm, ts: now() };
    return g.__void_basics_cache;
  }
  ...
  (function attach(){
    ...
    a.get("/__void/about", async (_req:any,res:any)=>{
      ...
      res.json({
        ...
        lastmile_v4b: {
          total: s.lastmile?.total||0,
          latest_txs: s.lastmile?.latest_txs||0,
          last_seen_block: s.lastmile?.last_seen_block||0,
          errors_total: s.lastmile?.errors_total||0,
        }
      });
    });
...
(function basicsExporterV2b(){
  ...
  async function fetchText(url:string, ms=300){
    const r = await withTimeout((globalThis as any).fetch(url), ms);
    if (!r || !r.ok) throw new Error("bad status"); return await r.text();
  }
  function num(s:string, d=0){ const n=Number(s); return Number.isFinite(n)?n:d; }
  function prom(text:string, name:string, d=0){
  ...
  async function snapshot(){
    if (now()-g.__void_basics_cache.ts<1000) return g.__void_basics_cache;
    const [head,health,lm]=await Promise.all([readHead(),readTxrootHealthLive(),readLastmile()]);
    g.__void_basics_cache={head,txroot_health_live:health,lastmile:lm,ts:now()}; return g.__void_basics_cache;
  }
  (function attach(){
    if (attached) return; const a:any = app();
    if (!a || typeof a.get!=="function"){ if((tries+=RETRY)<=MAX) return setTimeout(attach,RETRY); return; }
    if (a.__voidBasicsExporterV2b) return; attached=true;

    // NEW metrics path
    a.get("/__void/metrics/void.basics.v2.prom", async (_req:any,res:any)=>{
      try{
        const s=await snapshot(); const lm=s.lastmile||{total:0,latest_txs:0,last_seen_block:0,errors_total:0};
        res.type("text/plain").send([
          "# HELP void_head_number Current chain head (node view)",
          "# TYPE void_head_number gauge",
          `void_head_number ${s.head||0}`,
...
`);
    ...
    // NEW JSON path
    a.get("/__void/about2", async (_req:any,res:any)=>{
      const s=await snapshot();
      res.json({
        time_ms: Date.now(),
        uptime_ms: Math.max(0,(process.uptime?.()||0)*1000)|0,
        head: s.head||0,
        txroot_health_live: s.txroot_health_live||0,
        flags: (g.__void_flags||{}),
        lastmile_v4b: {
...
(function txrootForensicsStickyV3(){
  ...
  // shared state
  const forensic = g.__void_txroot_forensic_state_v3 || (g.__void_txroot_forensic_state_v3 = {
    calls_total: 0,
    last_kind: 'unknown',
    last_shape: 'n/a',
    last_number: -1,
    last_duration_ms: 0,
    observed_saves_total: 0,  // fallback based on head advance
    observed_last_head: -1,
  });
  const meta = g.__void_txroot_forensic_meta_v3 || (g.__void_txroot_forensic_meta_v3 = {
    wraps_total: 0,
    last_bind_ms: 0,
    route_mounted: false,
    last_probe_err: '',
  ...
  function isWrapped(fn:any){ return !!(fn && (fn as any)[SYM]); }
  ...
  function wrap(fn:any){
    ...
    const wrapped = function(this:any, ...args:any[]){
      ...
      if (Array.isArray(a0)) {
        forensic.last_kind = 'array';
        forensic.last_shape = `len=${a0.length}; keys=${Object.keys(a0[0]||{}).slice(0,8).join(',')}`;
        if (a0[0] && typeof a0[0].number === 'number') forensic.last_number = a0[0].number;
      } else if (a0 && typeof a0 === 'object') {
        forensic.last_kind = 'object';
        forensic.last_shape = `keys=${Object.keys(a0).slice(0,12).join(',')}`;
        if (typeof (a0 as any).number === 'number') forensic.last_number = (a0 as any).number;
      } else {
        forensic.last_kind = typeof a0;
  ...
  function mountRoute(){
    ...
    app.get('/__void/metrics/txroot4/forensics.prom', (_req:any, res:any)=>{
      res.type('text/plain; version=0.0.4; charset=utf-8').end([
        ...
        '# TYPE void_txroot_forensics_calls_total counter',
        `void_txroot_forensics_calls_total ${forensic.calls_total}`,
        '# HELP void_txroot_forensics_wraps_total times we (re)bound the wrapper',
        '# TYPE void_txroot_forensics_wraps_total counter',
        ...
        `void_txroot_forensics_observed_saves_total ${forensic.observed_saves_total}`,
    ...
    console.log('[txroot/forensics/sticky-v3] route mounted');
  }

  async function getSegProto(){
    try {
      const mod = g.SegStore ? {SegStore:g.SegStore} : await import('./chain/seg_store.js');
      if (mod && mod.SegStore && mod.SegStore.prototype) {
        g.SegStore = mod.SegStore; // cache
        return mod.SegStore.prototype;
      }
  ...
  function* findAllStoreMethods(){
    const seen = new Set<any>();
    const cand:any[] = [];
    // obvious globals
    for (const k of ['__void_node','__voidNode','node','VOID_NODE']) {
      const v = (g as any)[k];
      if (v && v.store) cand.push(v.store);
    }
    // scan globals lightly
    for (const k of Object.keys(g)) {
      const v = (g as any)[k];
      if (v && v.store && typeof v.store.saveBlock === 'function') cand.push(v.store);
    }
    for (const store of cand) {
      if (!store || seen.has(store)) continue; seen.add(store);
      const fns = ['saveBlock','persistBlock','append','_saveBlock','save']; // catch common variants
      for (const name of fns) {
        const fn = (store as any)[name];
        if (typeof fn === 'function') yield {store, name, fn};
      }
    }
  }
  ...
  async function observeHeadTick(){
    try {
      ...
      if (res && res.ok) {
        ...
        if (n >= 0 && n > forensic.observed_last_head) {
          if (forensic.observed_last_head >= 0) {
            forensic.observed_saves_total += (n - forensic.observed_last_head);
          }
          forensic.observed_last_head = n;
        }
        meta.last_probe_err = '';
      }
    } catch(e:any){ meta.last_probe_err = String(e?.message||e); }
  ...
  async function tick(){
    try {
      ...
      // 1) Prototype wrap
      const proto = await getSegProto();
      if (proto && typeof proto.saveBlock === 'function' && !isWrapped(proto.saveBlock)) {
        proto.saveBlock = wrap(proto.saveBlock);
      }

      // 2) Instance wrap (any method name we recognize)
      for (const {store, name, fn} of findAllStoreMethods()) {
        if (!isWrapped(fn)) {
          (store as any)[name] = wrap(fn);
        }
      }
    } catch(e:any){ meta.last_probe_err = String(e?.message||e); }
    setTimeout(tick, TICK);
