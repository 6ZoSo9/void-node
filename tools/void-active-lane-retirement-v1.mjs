...
export const RETIREMENT_MARKER = "VOID_ACTIVE_LANE_RETIREMENT_V1";
...
function captureCandidate({
  ...
  if (branchHead && liveMainObjectPresent) {
    ...
    if (!headAncestorOfLiveMain) {
      reasons.push("branch_head_not_ancestor_of_live_origin_main");
    }
  }

  const openPrs = openPrsForBranch(repoRoot, branch, requireGithub);
  if (openPrs.length > 0) {
...
function createBundle(repoRoot, branch, head, archiveDir) {
  ...
  const bundlePath = resolve(archiveDir, "active-lane-source-v1.bundle");
  git(repoRoot, [
    "bundle",
    "create",
    bundlePath,
...
export function applyRetirement({
  ...
  try {
    ...
    try {
      ...
      if (existsSync(validated.archive_dir)) {
        writeJsonAtomic(
          resolve(validated.archive_dir, "retirement-failure-receipt-v1.json"),
          failure,
        );
      }
    } catch {
...
function helpText() {
  return `VOID Active Lane Retirement V1
...
function main() {
  ...
  try {
    if (args.command === "plan") {
      ...
      console.log("VOID_ACTIVE_LANE_RETIREMENT_V1_COMPLETE=true");
