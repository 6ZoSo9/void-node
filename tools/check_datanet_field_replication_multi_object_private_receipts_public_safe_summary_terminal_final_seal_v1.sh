...
bash tools/check_datanet_field_replication_multi_object_private_receipts_public_safe_summary_closeout_runtime_v1.sh
...
base_main_commit = "3d9cbe65"
final_marker = "VOID_DATANET_FIELD_REPLICATION_MULTI_OBJECT_PRIVATE_RECEIPTS_PUBLIC_SAFE_SUMMARY_TERMINAL_FINAL_SEAL_V1_GREEN"
current_main_audit_marker = "VOID_DATANET_FIELD_REPLICATION_MULTI_OBJECT_PRIVATE_RECEIPTS_PUBLIC_SAFE_SUMMARY_CLOSEOUT_RUNTIME_CURRENT_MAIN_AUDIT_GREEN"
closeout_runtime_marker = "VOID_DATANET_FIELD_REPLICATION_MULTI_OBJECT_PRIVATE_RECEIPTS_PUBLIC_SAFE_SUMMARY_CLOSEOUT_RUNTIME_V1_GREEN"
...
json_path = Path("public/public-node/datanet/field-replication-multi-object-private-receipts-public-safe-summary-terminal-final-seal-v1.json")
doc_path = Path("docs/public/datanet-field-replication-multi-object-private-receipts-public-safe-summary-terminal-final-seal-v1.md")
html_path = Path("public/public-node/datanet/field-replication-multi-object-private-receipts-public-safe-summary-terminal-final-seal-v1.html")
...
assert seal["source_main_commit"] == base_main_commit
assert seal["sealed_scope"]["multi_object_manifest_runtime"] == manifest_runtime_marker
assert seal["sealed_scope"]["candidate_fixture_runtime"] == candidate_runtime_marker
assert seal["sealed_scope"]["private_receipts_public_safe_summary"] == summary_marker
assert seal["sealed_scope"]["private_receipts_public_safe_summary_runtime"] == summary_runtime_marker
assert seal["sealed_scope"]["private_receipts_public_safe_summary_closeout"] == closeout_marker
assert seal["sealed_scope"]["private_receipts_public_safe_summary_closeout_runtime"] == closeout_runtime_marker
assert seal["sealed_scope"]["current_main_audit"] == current_main_audit_marker
assert seal["public_commitments"]["source_public_safe_candidate_sha256"] == candidate_sha
assert seal["public_commitments"]["aggregate_receipts_sha256"] == aggregate_sha
assert seal["public_commitments"]["object_count"] == 2
assert seal["public_commitments"]["minimum_object_count_satisfied"] is True
assert seal["terminal_final_seal_marker"] == final_marker

disabled = seal["disabled_boundaries"]
...
publication_boundary = seal["publication_boundary"]
...
idx = index["field_replication_multi_object_private_receipts_public_safe_summary_terminal_final_seal_v1"]
assert idx["status"] == "green"
assert idx["marker"] == final_marker
assert idx["current_main_audit_marker"] == current_main_audit_marker
assert idx["closeout_runtime_marker"] == closeout_runtime_marker
assert idx["closeout_marker"] == closeout_marker
