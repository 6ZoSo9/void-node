...
marker="VOID_PUBLIC_NODE_OPERATOR_TRIAL_TESTER_LAUNCH_TERMINAL_FINAL_SEAL_V1"
green="VOID_PUBLIC_NODE_OPERATOR_TRIAL_TESTER_LAUNCH_TERMINAL_FINAL_SEAL_V1_GREEN"

json="public/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.json"
html="public/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.html"
doc="docs/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.md"
...
seal = json.loads(Path("public/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.json").read_text())
...
assert seal["status"] == "tester_launch_terminal_final_sealed"
assert seal["public_safe"] is True
assert seal["read_only"] is True
assert seal["boundary"]["terminal_final_seal_only"] is True
assert seal["boundary"]["operator_review_required_for_real_receipts"] is True
assert seal["boundary"]["no_private_keys_required"] is True
assert seal["boundary"]["no_secrets_required"] is True
assert seal["boundary"]["no_pii_required"] is True
...
assert [item["key"] for item in seal["sealed_components"]] == required
...
entry = index["public_node_operator_trial_tester_launch_terminal_final_seal"]
assert entry["marker"] == marker
assert entry["expected_green_marker"] == green
assert entry["status"] == "tester_launch_terminal_final_sealed"
assert entry["html"] == "/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.html"
assert entry["json"] == "/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.json"

assert index["links"]["public_node_operator_trial_tester_launch_terminal_final_seal"] == "/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.html"
assert index["links"]["public_node_operator_trial_tester_launch_terminal_final_seal_json"] == "/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.json"
assert index["route_markers"]["public_node_operator_trial_tester_launch_terminal_final_seal"] == marker
...
for key in [
    ...
    assert index[key]["tester_launch_terminal_final_sealed"] is True, key
...
assert has_route("/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.html")
assert has_route("/public-node/public-node-operator-trial-tester-launch-terminal-final-seal-v1.json")

print("trial tester launch terminal final seal/index assertions passed")
PY
