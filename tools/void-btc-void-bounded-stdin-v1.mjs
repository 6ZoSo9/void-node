  ...
  return new Promise((resolve, reject) => {
    ...
    const armIdleTimer = () => {
      ...
      idleTimer = setTimeout(
        () => finishError(
          new Error(`stdin idle deadline exceeded after ${idleTimeoutMs}ms`),
        ),
        idleTimeoutMs,
      );
    };
    function onData(chunk) {
      ...
      bytes += buffer.length;
      if (bytes > maxBytes) {
        finishError(new Error(`stdin exceeds ${maxBytes} bytes`));
        return;
      }
      chunks.push(buffer);
      armIdleTimer();
    ...
    totalTimer = setTimeout(
      () => finishError(
        new Error(`stdin total deadline exceeded after ${totalTimeoutMs}ms`),
      ),
      totalTimeoutMs,
    );
    stream.resume?.();
