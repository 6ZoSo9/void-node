...
const MARKER = "VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_HOLD_V1";
...
function validateCloseout(closeout) {
  if (!closeout || typeof closeout !== "object" || Array.isArray(closeout)) throw new Error("closeout_invalid");
  if (closeout.schema !== "void.datanet.wc.evidence_packet_ledger_append_scratch_apply_receipt_closeout.v1") throw new Error("closeout_schema_mismatch");
  if (closeout.marker !== CLOSEOUT_MARKER) throw new Error("closeout_marker_mismatch");
  if (closeout.status !== "scratch_receipt_chain_closed_for_operator_review") throw new Error("closeout_status_mismatch");
  ...
  assertNonEmpty(receipt.path, "receipt_path");
  ...
  assertBool(summary.scratch_receipt_chain_complete, true, "scratch_receipt_chain_complete");
  assertBool(summary.scratch_ledger_preview_bound, true, "scratch_ledger_preview_bound");
  ...
  assertBool(policy.scratch_receipt_closeout_only, true, "scratch_receipt_closeout_only");

  const boundary = closeout.boundary || {};
  assertBool(boundary.scratch_apply_receipt_closeout_only, true, "scratch_apply_receipt_closeout_only");
...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log("Usage: node tools/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index.mjs --closeout <closeout.json> --out <index.json> --indexer <indexer> --reason <reason>");
    return;
  }

  const closeoutPath = path.resolve(requireArg("closeout"));
  ...
  const reviewIndexId = stableHashObject({
    scratch_apply_receipt_closeout_id: closeout.scratch_apply_receipt_closeout_id,
  ...
  const index = {
    schema: "void.datanet.wc.evidence_packet_ledger_append_scratch_apply_receipt_closeout_review_index.v1",
    marker: MARKER,
    status: "scratch_receipt_closeout_indexed_for_operator_review",
    scratch_apply_receipt_closeout_review_index_id: reviewIndexId,
    created_at: process.env.VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CREATED_AT || new Date().toISOString(),
    indexer,
    reason,
    closeout: {
      path: closeoutPath,
    ...
    receipt: {
      marker: closeout.receipt.marker,
      scratch_apply_receipt_id: closeout.receipt.scratch_apply_receipt_id,
      reviewer: closeout.receipt.reviewer,
    },
    scratch_apply: {
      marker: closeout.scratch_apply.marker,
      scratch_apply_id: closeout.scratch_apply.scratch_apply_id,
      operator: closeout.scratch_apply.operator,
    },
    scratch_ledger: {
      path: closeout.scratch_ledger.path,
      line_count: closeout.scratch_ledger.line_count,
    ...
    review_index_summary: {
      scratch_receipt_closeout_bound: true,
      scratch_preview_chain_indexed: true,
      operator_review_index_only: true,
    ...
    work_credits_policy: {
      ...
      scratch_receipt_closeout_review_index_only: true,
    },
    boundary: {
      scratch_apply_receipt_closeout_review_index_only: true,
...
main().catch((err) => {
  console.error(`datanet_wc_evidence_packet_ledger_append_scratch_apply_receipt_closeout_review_index_error=${err.message}`);
  process.exit(1);
});
