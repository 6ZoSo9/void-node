...
const green = 'VOID_PUBLIC_NODE_OPERATOR_RECEIPT_REVIEW_DECISION_TEMPLATE_CLOSEOUT_V1_GREEN';

const markers = {
  decisionTemplate: 'VOID_PUBLIC_NODE_OPERATOR_RECEIPT_REVIEW_DECISION_TEMPLATE_V1',
  decisionTemplateGreen: 'VOID_PUBLIC_NODE_OPERATOR_RECEIPT_REVIEW_DECISION_TEMPLATE_V1_GREEN',
  decisionTemplateIndexRouteGreen: 'VOID_PUBLIC_NODE_OPERATOR_RECEIPT_REVIEW_DECISION_TEMPLATE_INDEX_ROUTE_V1_GREEN',
...
const routes = {
  ...
  decisionTemplateHtml: '/public-node/public-node-operator-receipt-review-decision-template-v1.html',
...
const files = {
  ...
  decisionDoc: 'docs/public/public-node-operator-receipt-review-decision-template-v1.md',
  decisionPage: 'public/public-node/operator-receipt-review-decision-template-v1.html',
  decisionJson: 'public/public-node/public-node-operator-receipt-review-decision-template-v1.json',
  decisionHtml: 'public/public-node/public-node-operator-receipt-review-decision-template-v1.html',
...
const decisionText = read(files.decisionJson);
const decision = JSON.parse(decisionText);
const decisionDoc = read(files.decisionDoc);
const decisionPage = read(files.decisionPage);
const decisionHtml = read(files.decisionHtml);
...
if (decision.schema !== 'void.public_node.operator_receipt_review_decision_template.v1') throw new Error('bad decision template schema');
if (decision.marker !== markers.decisionTemplate) throw new Error('bad decision template marker');
if (decision.status !== 'operator_receipt_review_decision_template_ready') throw new Error('bad decision template status');
if (decision.expected_green_marker !== markers.decisionTemplateGreen) throw new Error('bad decision template expected green marker');

if (!Array.isArray(decision.allowed_outcomes) || decision.allowed_outcomes.length < 5) throw new Error('allowed outcomes incomplete');
if (!Array.isArray(decision.decision_template_fields) || decision.decision_template_fields.length < 10) throw new Error('decision template fields incomplete');
if (!Array.isArray(decision.reject_if_present) || decision.reject_if_present.length < 10) throw new Error('reject list incomplete');

if (decision.example_decision_template?.template_only !== true) throw new Error('example decision is not template_only');
if (decision.example_decision_template?.private_material_included !== false) throw new Error('example includes private material');
if (decision.example_decision_template?.review_decision_created !== false) throw new Error('example creates review decision');
if (decision.example_decision_template?.receipt_created !== false) throw new Error('example creates receipt');
if (decision.example_decision_template?.work_credit_claim_created !== false) throw new Error('example creates Work Credit claim');
if (decision.example_decision_template?.money_transfer_claim !== false) throw new Error('example makes money transfer claim');
if (decision.example_decision_template?.validator_admission_claim !== false) throw new Error('example makes validator admission claim');
if (decision.example_decision_template?.public_internet_mesh_claim !== false) throw new Error('example makes public internet mesh claim');

if (root.links?.public_node_operator_receipt_review_decision_template !== routes.decisionTemplatePage) throw new Error('root missing decision template page link');
if (root.links?.public_node_operator_receipt_review_decision_template_json !== routes.decisionTemplateJson) throw new Error('root missing decision template json link');
if (root.links?.public_node_operator_receipt_review_decision_template_html !== routes.decisionTemplateHtml) throw new Error('root missing decision template html link');
if (root.route_markers?.public_node_operator_receipt_review_decision_template !== markers.decisionTemplate) throw new Error('root missing decision template route marker');
if (root.public_node_operator_receipt_review_decision_template?.marker !== markers.decisionTemplate) throw new Error('root decision template marker mismatch');
if (root.public_node_operator_receipt_review_decision_template?.status !== 'operator_receipt_review_decision_template_ready') throw new Error('root decision template status mismatch');
if (root.public_node_operator_receipt_review_decision_template?.expected_green_marker !== markers.decisionTemplateIndexRouteGreen) {
  throw new Error('root decision template expected index-route green mismatch');
}

for (const route of [routes.decisionTemplatePage, routes.decisionTemplateJson, routes.decisionTemplateHtml]) {
  if (!rootText.includes(route)) throw new Error(`root missing decision template route ${route}`);
  if (!routeSource.includes(route)) throw new Error(`route source missing decision template route ${route}`);
}
...
for (const needle of [
  ...
  'publicNodeOperatorReceiptReviewDecisionTemplatePagePath',
  'publicNodeOperatorReceiptReviewDecisionTemplateJsonPath',
  'publicNodeOperatorReceiptReviewDecisionTemplateHtmlPath',
...
for (const [key, expected] of Object.entries({
  ...
  if (decision.boundary?.[key] !== expected) throw new Error(`bad decision template boundary ${key}`);
  if (root.public_node_operator_receipt_review_decision_template?.boundary?.[key] !== expected) {
    throw new Error(`bad root decision template boundary ${key}`);
  }
}
...
for (const field of [
  ...
  if (!surfaceText.includes(field)) throw new Error(`missing decision field binding ${field}`);
}
...
for (const marker of [
  markers.decisionTemplateGreen,
  markers.decisionTemplateIndexRouteGreen,
  markers.checklistCloseoutGreen,
  markers.receiptExampleCloseoutGreen,
  markers.handoffPacketCloseoutGreen,
  markers.statusRollupCloseoutGreen,
