...
function forbiddenGitEnvironmentV1(){
 const explicit=FORBIDDEN_GIT_REPOSITORY_ENV_V1.filter(k=>process.env[k]!==undefined);
 const indexed=Object.keys(process.env).filter(k=>INDEXED_CONFIG_ENV_RE.test(k));
 return [...new Set([...explicit,...indexed])].sort();
}
function assertGitRepositoryEnvironmentV1(){const present=forbiddenGitEnvironmentV1();if(present.length)fail(`Git repository-selection environment is not allowed: ${present.join(',')}`);}
