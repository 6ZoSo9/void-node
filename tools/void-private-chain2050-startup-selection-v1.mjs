...
function exactBlockHash(value, reason) {
  ...
  if (!BLOCK_HASH_RE.test(normalized)) hold(reason);
  return normalized;
}
...
function requireCheckpointRoot(checkpointRoot) {
  ...
  if (typeof process.getuid === "function" && stat.uid !== process.getuid()) {
    hold("checkpoint_root_owner_invalid");
  }
...
function normalizeBaseline(baseline) {
  ...
  const observedSha256 = sha256Buffer(fs.readFileSync(baseline.state_file));
...
function validateCheckpointManifest(checkpointRoot, manifestPath) {
  ...
  if (typeof manifest.captured_at !== "string" || manifest.captured_at.length < 10) {
    hold("checkpoint_captured_at_invalid");
  }
  ...
  const expectedStem = `chain2050-block-${blockNumber}-${checkpointId}`;
  if (
    path.basename(manifestPath) !==
    `${expectedStem}${CHECKPOINT_MANIFEST_SUFFIX}`
  ) {
...
export function readVoidPrivateChain2050CheckpointCandidatesV1(checkpointRoot) {
  if (!requireCheckpointRoot(checkpointRoot)) return [];
  const entries = fs.readdirSync(checkpointRoot, { withFileTypes: true });
  const manifestPaths = [];
  const observedStateFiles = new Set();
  for (const entry of entries) {
    ...
    if (entry.name.endsWith(CHECKPOINT_MANIFEST_SUFFIX)) {
      manifestPaths.push(pathname);
    } else if (entry.name.endsWith(CHECKPOINT_STATE_SUFFIX)) {
      observedStateFiles.add(entry.name);
    } else {
      hold("checkpoint_root_unrecognized_entry");
    }
  }
  manifestPaths.sort();
  const candidates = manifestPaths.map((manifestPath) =>
    validateCheckpointManifest(checkpointRoot, manifestPath),
  );
  const referencedStateFiles = new Set(
    candidates.map((candidate) => path.basename(candidate.state_file)),
  );
  if (
    JSON.stringify([...observedStateFiles].sort()) !==
    JSON.stringify([...referencedStateFiles].sort())
  ) {
    hold("checkpoint_orphan_state_file");
  }
  return candidates;
}
...
export function selectVoidPrivateChain2050StartupStateV1({
  ...
  const checkpoints = readVoidPrivateChain2050CheckpointCandidatesV1(
    checkpointRoot,
  );
