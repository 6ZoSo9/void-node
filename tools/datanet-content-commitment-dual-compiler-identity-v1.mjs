...
export const VOID_DATANET_CONTENT_COMMITMENT_DUAL_COMPILER_IDENTITY_V1 =
  "VOID_DATANET_CONTENT_COMMITMENT_DUAL_COMPILER_IDENTITY_V1";
export const PROTOCOL =
  "void-datanet-content-commitment-dual-compiler-identity/1";
export const ENVIRONMENT_MARKER =
  "VOID_SOLC_COMPILER_ENVIRONMENT_V1";
export const DECISION =
  "HOLD_PENDING_COMPILED_IDENTITY_COMMIT_AND_EXACT_CHAIN2050_DEPLOYMENT_LINEAGE_ATTESTATION";
...
function normalizeHex(raw, code) {
  ...
  if (!value || !/^[0-9a-f]+$/.test(value) || value.length % 2 !== 0) fail(code);
  return value;
}
...
function metadataStripped(bytecodeHex, code) {
  ...
  if (metadataLength <= 0 || start <= 0 || start >= bytes.length - 2) fail(code);
  return bytes.subarray(0, start).toString("hex");
}

function assertNoPush0(bytecodeHex, label) {
  const executable = metadataStripped(bytecodeHex, label + "_metadata_invalid");
  const bytes = Buffer.from(executable, "hex");
  for (let i = 0; i < bytes.length; i += 1) {
    const opcode = bytes[i];
    if (opcode === 0x5f) {
      fail("push0_opcode_forbidden_for_paris_profile", { label, byte_offset: i });
    }
    if (opcode >= 0x60 && opcode <= 0x7f) {
      i += opcode - 0x5f;
      if (i >= bytes.length) fail(label + "_truncated_push_immediate");
    }
  }
}

function diagnostics(output, label) {
  const values = output.errors === undefined ? [] : output.errors;
  if (!Array.isArray(values)) fail(label + "_diagnostics_invalid");
  const errors = values.filter((entry) => entry?.severity === "error");
  if (errors.length) fail("compiler_reported_errors", { label, errors });
  return values.filter((entry) => entry?.severity === "warning").length;
}

function containsReferences(value) {
  if (Array.isArray(value)) return value.length > 0;
  if (plain(value)) return Object.values(value).some(containsReferences);
...
function immutableDeclarations(ast) {
  ...
  function walk(value) {
    ...
    if (
      ...
      ["publisher", "predecessor"].includes(value.name)
    ) {
  ...
  if (
    found.size !== 2 ||
    ![...found.values()].includes("publisher") ||
    ![...found.values()].includes("predecessor")
  ) {
    fail("immutable_declaration_map_invalid", { found: Object.fromEntries(found) });
  }
  return found;
}
...
function normalizeImmutableReferences(refs, ast, runtimeBytes) {
  ...
  for (const [astId, name] of declarations.entries()) {
    ...
    if (!Array.isArray(entries) || entries.length < 1) {
      fail("immutable_reference_missing", { ast_id: astId, name });
    }
    const normalized = entries.map((entry) => {
      if (
        ...
        fail("immutable_reference_invalid", { ast_id: astId, name, entry });
      }
      for (let offset = entry.start; offset < entry.start + entry.length; offset += 1) {
        if (occupied[offset]) fail("immutable_reference_overlap", { offset });
        occupied[offset] = true;
      }
      return { start: entry.start, length: entry.length };
    });
    layout[name] = { ast_id: Number(astId), references: normalized };
  }
  const referencedIds = Object.keys(refs).sort();
  const expectedIds = [...declarations.keys()].sort();
  if (JSON.stringify(referencedIds) !== JSON.stringify(expectedIds)) {
    fail("unexpected_immutable_reference_ids", { referencedIds, expectedIds });
  }
  return layout;
}
...
function parseMetadata(raw, label) {
  ...
  if (parsed?.settings?.viaIR !== undefined && parsed.settings.viaIR !== false) {
  ...
  ) fail("compiler_metadata_append_cbor_mismatch", { label });
  if (
    parsed?.settings?.metadata?.useLiteralContent !== undefined &&
    parsed.settings.metadata.useLiteralContent !== true
  ) fail("compiler_metadata_literal_content_mismatch", { label });
...
export function parseCompilerOutput(raw, label = "compiler") {
  ...
  if (!plain(source) || !plain(ast)) fail(label + "_source_ast_missing");
...
export function validateCompilerEnvironment(value, label) {
  ...
  if (
    ...
    typeof value.artifact_identity !== "string" || !value.artifact_identity
...
function exact(a, b, code) {
  if (a !== b) fail(code);
}
function canonicalEqual(a, b, code) {
  if (canonicalJson(a) !== canonicalJson(b)) fail(code);
}
function timestamp(raw) {
  const value = String(raw ?? "").trim();
  const parsed = Date.parse(value);
  if (!value || !Number.isFinite(parsed)) fail("reviewed_at_invalid");
  return new Date(parsed).toISOString();
}

export function reviewDatanetContentCommitmentDualCompilerV1({
  sourceBytes,
  inputBytes,
  outputABytes,
  outputBBytes,
  ...
  exact(a.creation.object, b.creation.object, "creation_bytecode_mismatch");
  exact(a.runtime_template.object, b.runtime_template.object, "runtime_template_mismatch");
  canonicalEqual(a.abi, b.abi, "abi_mismatch");
  exact(a.metadata, b.metadata, "metadata_mismatch");
  canonicalEqual(a.storage_layout, b.storage_layout, "storage_layout_mismatch");
  canonicalEqual(a.method_identifiers, b.method_identifiers, "method_identifiers_mismatch");
  canonicalEqual(a.immutable_layout, b.immutable_layout, "immutable_layout_mismatch");
  exact(a.creation.source_map, b.creation.source_map, "creation_source_map_mismatch");
  exact(a.runtime_template.source_map, b.runtime_template.source_map, "runtime_source_map_mismatch");
  ...
  const body = {
    ...
    source: {
      ...
      compiler_profile_marker: VOID_DATANET_CONTENT_COMMITMENT_COMPILER_PROFILE_V1,
      standard_json_input_canonical_sha256: sha256(canonicalJson(expectedInput)),
    },
    compiler_profile: {
      compiler: "solc",
    ...
    environments: {
      compiler_a: {
        ...
        output_canonical_sha256: a.canonical_output_sha256,
      },
      compiler_b: {
        ...envB.descriptor,
        fingerprint_sha256: envB.fingerprint_sha256,
        output_raw_sha256: b.raw_sha256,
        output_canonical_sha256: b.canonical_output_sha256,
      },
    },
    comparison: {
    ...
    artifacts: {
      ...
      creation_bytecode_keccak256: a.creation.keccak256,
      ...
      runtime_template_keccak256: a.runtime_template.keccak256,
    ...
    deployment_identity_requirements: {
      ...
      constructor_order: ["publisher", "predecessor"],
      deployed_runtime_must_patch_exact_immutable_layout: true,
      deployed_runtime_keccak256_must_match_reconstructed_runtime: true,
      live_registry_version_must_equal: "1",
      live_max_object_bytes_must_equal: "268435456",
      live_publisher_view_must_match_constructor: true,
      live_predecessor_view_must_match_constructor: true,
      predecessor_lineage_must_be_attested: true,
      fresh_is_committed_preflight_required: true,
    },
    unresolved: {
      compiled_identity_committed: false,
      registry_contract_address: null,
    ...
    decision: {
      ...
      chain2050_write_authorized: false,
      next_gate:
        "commit_exact_compiled_identity_then_verify_explicit_chain2050_registry_deployment_and_lineage",
    },
  };

  return {
    identity_id: "voiddccci1_" + sha256(canonicalJson(body)),
    ...body,
  };
}
