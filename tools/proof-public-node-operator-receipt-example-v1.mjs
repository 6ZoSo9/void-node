...
const green = 'VOID_PUBLIC_NODE_OPERATOR_RECEIPT_EXAMPLE_V1_GREEN';
...
const jsonText = read(files.receiptExampleJson);
const html = read(files.receiptExampleHtml);
const page = read(files.receiptExamplePage);
const data = JSON.parse(jsonText);

if (data.schema !== 'void.public_node.operator_receipt_example.v1') throw new Error('bad schema');
if (data.marker !== marker) throw new Error('bad marker');
if (data.status !== 'operator_receipt_example_ready') throw new Error('bad status');
if (data.expected_green_marker !== green) throw new Error('bad expected green marker');
if (data.example_receipt?.example_only !== true) throw new Error('example receipt is not marked example_only');
if (data.example_receipt?.private_material_included !== false) throw new Error('example includes private material');
if (data.example_receipt?.money_transfer_claim !== false) throw new Error('example makes money claim');
if (data.example_receipt?.validator_admission_claim !== false) throw new Error('example makes validator claim');
if (data.example_receipt?.work_credit_claim !== false) throw new Error('example makes Work Credit claim');
if (data.example_receipt?.public_internet_mesh_claim !== false) throw new Error('example makes public mesh claim');

for (const key of ['routes', 'components', 'example_receipt', 'forbidden_receipt_fields', 'boundary']) {
...
for (const field of [
  ...
  if (!combined.includes(field)) throw new Error(`missing forbidden field ${field}`);
}
