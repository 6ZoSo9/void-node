...
for (const p of [indexPath, statusPath, htmlPath]) {
...
if (!index.includes('VOID_LOCAL_MULTIBOX_RUNTIME_ROUTE_V1')) {
  throw new Error('route marker missing from src/index.ts');
}
...
for (const route of [
  ...
  if (!index.includes(route)) throw new Error(`missing route in src/index.ts: ${route}`);
}
