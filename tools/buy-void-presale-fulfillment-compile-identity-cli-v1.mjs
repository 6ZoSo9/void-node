...
function regularFile(file) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    fail("not_regular_file=" + resolved);
  }
  return resolved;
}
...
if (command === "review") {
  ...
  const input = regularFile(
    required(map, "input"),
  );
  const outputA = regularFile(
    required(map, "output-a"),
  );
  const outputB = regularFile(
    required(map, "output-b"),
  );
  const environmentA = regularFile(
    required(map, "environment-a"),
  );
  const environmentB = regularFile(
    required(map, "environment-b"),
  );
  const out = required(map, "out");
  ...
  try {
    envA = JSON.parse(
      fs.readFileSync(environmentA, "utf8"),
    );
    envB = JSON.parse(
      fs.readFileSync(environmentB, "utf8"),
    );
  } catch {
