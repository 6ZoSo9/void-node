...
export const VOID_DATANET_CONTENT_COMMITMENT_DEPLOYMENT_OBSERVER_AUTHORITY_V1 = {
...
function createHttpTransport(policy){
  ...
  return async(call)=>{
    ...
    if(Buffer.byteLength(body,"utf8")>MAX_REQUEST_BYTES){
      throw new Error("datanet_deployment_observer_request_too_large");
    }
    return await new Promise((resolve,reject)=>{
      ...
      const request=http.request({
        ...
        headers:{
          ...
          Connection:"close",
          "User-Agent":"void-datanet-content-commitment-deployment-observer-v1",
        },
      },(response)=>{
        const chunks=[];
        let total=0;
        response.on("data",(chunk)=>{
          ...
          if(total>policy.max_response_bytes){
            request.destroy(
              new Error("datanet_deployment_observer_response_too_large"),
            );
            return;
          }
          chunks.push(buffer);
        ...
        response.on("end",()=>{
          if(Number(response.statusCode||0)!==200){
            finish(new Error("datanet_deployment_observer_http_status_invalid"));
            return;
          }
          let payload;
          try{
            ...
            finish(new Error("datanet_deployment_observer_rpc_json_invalid"));
            return;
          }
          if(
            !payload||
            ...
            finish(new Error("datanet_deployment_observer_rpc_envelope_invalid"));
            return;
          }
      ...
      request.on("timeout",()=>{
        request.destroy(new Error("datanet_deployment_observer_timeout"));
      });
...
function decodeAddress(name,raw){
  ...
  if(!data) throw new Error("datanet_deployment_observer_view_result_invalid:"+name);
  const decoded=VIEWS.decodeFunctionResult(name,data);
  const value=address(decoded[0]);
  if(!value) throw new Error("datanet_deployment_observer_view_address_invalid:"+name);
  return value;
}
function decodeUint(name,raw){
  ...
  if(value<0n) throw new Error("datanet_deployment_observer_view_uint_invalid:"+name);
  return value.toString();
}

export async function observeDatanetContentCommitmentDeploymentV1(input){
  const rpcPolicy=normalizeRpcPolicy(input);
  const contract=address(input?.registry_contract_address);
  const publisher=address(input?.publisher_address);
  const predecessor=address(input?.predecessor_address);
  const txHash=hash(input?.deployment_transaction_hash);
  const minimum=(()=>{
    try{
      const value=BigInt(text(input?.min_confirmations));
      return value>0n&&value<=1_000_000n?value:null;
    }catch(error){
      return null;
    }
  })();
  ...
  if(
    !rpcPolicy||!contract||!publisher||!predecessor||!txHash||minimum===null
  ){
    return held("datanet_deployment_observer_input_invalid",{
  ...
  try{
    ...
    if(chainId!==2050n){
      return held("datanet_deployment_observer_chain_id_mismatch",{
    ...
    if(head===null||head<=0n){
      return held("datanet_deployment_observer_head_invalid",{
        rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
        rpc_methods_used:methods,
      });
    }
    ...
    if(!blockHashA||quantity(blockA?.number)!==head){
      return held("datanet_deployment_observer_observation_block_invalid",{
        rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
        rpc_methods_used:methods,
      });
    }
    ...
    const receiptA=await call("eth_getTransactionReceipt",[txHash]);
    if(!tx||!receiptA){
      return held("datanet_deployment_observer_transaction_not_visible",{
        rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
        rpc_methods_used:methods,
      });
    }

    const runtimeCode=bytes(await call("eth_getCode",[contract,headTag]));
    if(!runtimeCode||runtimeCode==="0x"){
      return held("datanet_deployment_observer_contract_code_absent",{
        rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
        rpc_methods_used:methods,
      });
    }
    ...
    const receiptB=await call("eth_getTransactionReceipt",[txHash]);

    if(
      hash(blockB?.hash)!==blockHashA||
      quantity(blockB?.number)!==head||
      hash(receiptA?.transactionHash)!==txHash||
      hash(receiptB?.transactionHash)!==txHash||
      hash(receiptA?.blockHash)!==hash(receiptB?.blockHash)||
      quantity(receiptA?.blockNumber)!==quantity(receiptB?.blockNumber)||
      address(receiptA?.contractAddress)!==address(receiptB?.contractAddress)
    ){
      return held("datanet_deployment_observer_revalidation_mismatch",{
        rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
        rpc_methods_used:methods,
      });
    }

    const txNonce=quantity(tx?.nonce);
    const txValue=quantity(tx?.value);
    const txChainId=quantity(tx?.chainId);
    const receiptBlock=quantity(receiptA?.blockNumber);
    if(
      txNonce===null||
      txValue===null||
      txChainId!==2050n||
      ...
      return held("datanet_deployment_observer_transaction_shape_invalid",{
        rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
        rpc_methods_used:methods,
      });
    }
    ...
    const observation={
      ...
      deployment_transaction:{
        ...
        to:tx?.to===null?null:address(tx?.to),
        nonce:txNonce.toString(),
        input:bytes(tx?.input??tx?.data),
        value_wei:txValue.toString(),
        chain_id:"2050",
      },
      deployment_receipt:{
        transaction_hash:txHash,
        status:quantity(receiptA?.status)?.toString()??"",
        block_number:receiptBlock.toString(),
        block_hash:hash(receiptA?.blockHash),
        contract_address:address(receiptA?.contractAddress),
      },
      runtime_code:runtimeCode,
      views:{
        registry_version:decodeUint("registryVersion",viewRaw.registryVersion),
        max_object_bytes:decodeUint("maxObjectBytes",viewRaw.maxObjectBytes),
        publisher_address:decodeAddress("publisher",viewRaw.publisher),
        predecessor_address:decodeAddress("predecessor",viewRaw.predecessor),
      },
    };

    const attestation=verifyDatanetContentCommitmentDeploymentObservationV1({
    ...
    if(attestation.ok===false){
      return held(
        "datanet_deployment_observer_attestation_held:"+attestation.reason,
        {
          rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
          rpc_methods_used:methods,
          detail:{attestation_reason:attestation.reason},
        },
      );
    }
    ...
    return held("datanet_deployment_observer_rpc_failed",{
      rpc_url_fingerprint_sha256:rpcPolicy.rpc_url_fingerprint_sha256,
      rpc_methods_used:methods,
      detail:{
