...
const green = 'VOID_PUBLIC_NODE_OPERATOR_REVIEW_LANE_FINAL_SEAL_V1_GREEN';

const markers = {
  finalSealGreen: green,
  ...
  handoffGreen: 'VOID_PUBLIC_NODE_OPERATOR_HANDOFF_PACKET_V1_GREEN',
  handoffIndexRouteGreen: 'VOID_PUBLIC_NODE_OPERATOR_HANDOFF_PACKET_INDEX_ROUTE_V1_GREEN',
  ...
  quickstartGreen: 'VOID_PUBLIC_NODE_OPERATOR_QUICKSTART_V1_GREEN',
  quickstartIndexRouteGreen: 'VOID_PUBLIC_NODE_OPERATOR_QUICKSTART_INDEX_ROUTE_V1_GREEN',
...
if (!rootRollup) throw new Error('root missing operator review lane rollup');
...
if (root.links?.public_node_operator_review_lane_rollup !== rollupRoutes.page) throw new Error('root page link mismatch');
if (root.links?.public_node_operator_review_lane_rollup_json !== rollupRoutes.json) throw new Error('root json link mismatch');
if (root.links?.public_node_operator_review_lane_rollup_html !== rollupRoutes.html) throw new Error('root html link mismatch');
if (root.route_markers?.public_node_operator_review_lane_rollup !== markers.rollup) throw new Error('root route marker mismatch');

if (!Array.isArray(rollup.review_sequence) || rollup.review_sequence.length !== 6) throw new Error('unexpected review sequence length');
...
const expectedComponentNames = new Set([
  'operator_status_rollup',
  'operator_quickstart',
  'operator_handoff_packet',
  'operator_receipt_example',
  'operator_receipt_review_checklist',
  'operator_receipt_review_decision_template',
  'operator_receipt_review_decision_example',
  'public_node_connect_lane',
  'connect_pack',
  'receipt_template',
  'bootstrap_gateway'
]);
...
for (const name of expectedComponentNames) {
  if (!rollup.review_lane_components.some((component) => component.name === name && component.status === 'green')) {
    throw new Error(`missing green component ${name}`);
  }
}

for (const route of Object.values(rollup.routes)) {
  const routeSurface = [
    rootText,
    rollupText,
  ...
  if (!routeSurface.includes(route)) throw new Error(`missing route binding ${route}`);
}
...
for (const marker of Object.values(markers)) {
  if (marker === markers.finalSealGreen) continue;
...
for (const proofFile of Object.values(files).filter((path) => path.startsWith('tools/proof-'))) {
  ...
  if (!proofFile.startsWith('tools/proof-')) throw new Error(`unexpected proof file path ${proofFile}`);
}
