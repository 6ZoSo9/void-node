...
try {
  ...
  const heldIssueLock = path.join(stateDir, ".issue.lock");
  fs.writeFileSync(
    heldIssueLock,
    JSON.stringify({
      marker: "VOID_PUBLIC_PARTICIPANT_LOGIN_PAIRING_ISSUE_LOCK_V1",
      pid: process.pid,
    }) + "\n",
    { mode: 0o600 },
  );
  fs.chmodSync(heldIssueLock, 0o600);
  assert.throws(
    () => issueVoidPublicParticipantLoginPairingV1({
      account,
      dataDir,
    ...
    /pairing_issue_busy/,
    "concurrent pairing issuer admitted",
  );
  fs.unlinkSync(heldIssueLock);
  ...
  assert.match(source, /pairing_issue_busy/);
  assert.match(source, /\.issue\.lock/);
  ...
  console.log("concurrent_issue_lock_rejected=true");
