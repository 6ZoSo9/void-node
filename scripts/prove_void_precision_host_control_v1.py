...
PROOF_WORKFLOW = ROOT / ".github/workflows/void-precision-host-control-proof-v1.yml"
...
def run_dynamic_proof(module) -> None:
    with tempfile.TemporaryDirectory(prefix="void-precision-host-control-v1-") as raw:
        ...
        try:
            ...
            require(inspect["service_restart"] is False, "inspect restart false")
            require(inspect["credential_read"] is False, "inspect credential false")
            require(inspect["money_movement"] is False, "inspect money false")
            ...
            require(reload_result["daemon_reload_performed"] is True, "reload marker")
            ...
            try:
                ...
                require(str(error) == "confirmation_mismatch", "wrong confirmation reason")
            require(backend.reload_calls == 0, "wrong confirmation before reload")
            ...
            gate.write_text(
                "Environment=VOID_BUY_VOID_SAGA_EXECUTE_PREPARED_TRANSACTION_RUNTIME_SUBMISSION_ENABLED=1\n",
                encoding="utf-8",
            )
            backend = FakeBackend(module, dropin, gate)
            try:
                ...
                raise AssertionError("enabled money gate accepted")
            except module.Hold as error:
                require(str(error).startswith("money_or_preparation_gate_enabled:"), "gate fail closed")
            require(backend.reload_calls == 0, "enabled gate blocks before reload")
        finally:
...
def run_static_proof() -> None:
    ...
    require(hashlib.sha256(helper_bytes).hexdigest() == EXPECTED_HELPER_SHA, "helper SHA pinned")
    tree = ast.parse(helper_bytes.decode("utf-8"))
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            require(node.func.attr not in {"system", "popen"}, "no shell execution primitives")
    ...
    require("pull_request:" not in host and "push:" not in host, "host workflow dispatch-only")
    require("permissions: {}" in host, "no token permissions")
    require("runs-on: [self-hosted, Linux, X64, void-precision-control-v1]" in host, "exact runner labels")
    require("actions/checkout" not in host, "no checkout on host runner")
    ...
    for forbidden in ("systemctl --user restart", "systemctl --user start", "eth_sendRawTransaction", "submit_once"):
        require(forbidden not in host, f"host workflow forbids {forbidden}")

    proof_workflow = PROOF_WORKFLOW.read_text(encoding="utf-8")
    require("runs-on: ubuntu-24.04" in proof_workflow, "proof stays GitHub-hosted")
    require("actions/checkout@v6" in proof_workflow, "proof checkout only")

    sudoers = SUDOERS.read_text(encoding="utf-8")
    require("void-gh-runner ALL=(root) NOPASSWD: VOID_PRECISION_HOST_CONTROL_V1" in sudoers, "narrow sudo alias")
    require("inspect_custodian_binding" in sudoers and "daemon_reload_custodian_binding" in sudoers, "two sudo operations")
