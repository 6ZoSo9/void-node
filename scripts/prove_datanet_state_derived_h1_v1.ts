...
async function main(): Promise<void> {
  ...
  try {
    ...
    const first = await acquireDataNetAdmissionCapabilityV1(root, rootIdentity, tuple);
    assert.equal(first.key, key);
    assert.equal(first.addressDigest, capabilityDigest);
    ...
    const empty = classifyDataNetStateDerivedH1V1(first);
    ...
    fsyncDirectory(root);
    const e0 = classifyDataNetStateDerivedH1V1(first);
