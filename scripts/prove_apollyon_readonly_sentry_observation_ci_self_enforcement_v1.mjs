...
const WORKFLOW = path.join(ROOT, ".github", "workflows", "apollyon-readonly-sentry-observation-v1.yml");
const source = fs.readFileSync(WORKFLOW, "utf8");

const requiredPaths = [
  ".github/workflows/apollyon-readonly-sentry-observation-v1.yml",
  "docs/security/apollyon-readonly-sentry-observation-v1.md",
  "src/security/apollyon_readonly_sentry_observation_v1.ts",
  "scripts/prove_apollyon_readonly_sentry_observation_v1.ts",
  "scripts/prove_apollyon_readonly_sentry_observation_ci_self_enforcement_v1.mjs",
  ...
  "scripts/prove_chain2050_role_authority_registry_binding_v1.ts",
...
for (const item of requiredPaths) {
  assert.equal(source.includes(`- \"${item}\"`) || source.includes(`- '${item}'`), true, `workflow must watch ${item}`);
}
...
assert.match(
  source,
  /npx --no-install tsx scripts\/prove_chain2050_role_authority_registry_binding_v1\.ts/,
);
assert.match(
  source,
  /node scripts\/prove_apollyon_readonly_sentry_observation_ci_self_enforcement_v1\.mjs/,
);
...
const node24OnlySteps = [
  "Re-prove Chain-2050 registry/binding",
  "Typecheck repository",
  "Prove sentry CI self-enforcement",
  "Prove committed-range hygiene contract",
];
for (const name of node24OnlySteps) {
  const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  assert.match(source, new RegExp(`- name: ${escaped}\\n\\s*if: matrix\\.node == 24`));
}

console.log("VOID_APOLLYON_READONLY_SENTRY_OBSERVATION_CI_SELF_ENFORCEMENT_V1_GREEN");
console.log("workflow_self_watched=true");
console.log("node_matrix_22_24_26=true");
console.log("registry_binding_dependency_watched=true");
