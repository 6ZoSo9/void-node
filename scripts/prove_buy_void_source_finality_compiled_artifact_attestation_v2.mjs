...
const MARKER = "VOID_BUY_VOID_SOURCE_FINALITY_COMPILED_ARTIFACT_ATTESTATION_V2";
const SOURCE_STACK_HEAD = "9202f3ce11664873f2316b08cbdbe2b98fd77fb4";
const EXPECTED_TYPESCRIPT_VERSION = "5.9.3";
const MANIFEST_PATH = "docs/architecture/buy-void-source-finality-compiled-artifact-attestation-v2.json";
const DERIVATION_NODE_MAJORS = Object.freeze([22, 24, 26]);
const PREDECESSOR_MARKER =
  "VOID_BUY_VOID_SOURCE_FINALITY_COMPILED_ARTIFACT_ATTESTATION_V1";
...
const PREDECESSOR_MANIFEST_GIT_BLOB_SHA1 =
  "24ea833fbfacab1f38311e4e6d5f401625291882";
const PREDECESSOR_COMPILED_ARTIFACT_SET_SHA256 =
...
function verifyBuildInputs() {
  ...
  let predecessorPackageJson;
  try {
    predecessorPackageJson = JSON.parse(
      execFileSync("git", ["show", `${SOURCE_STACK_HEAD}:package.json`], {
        cwd: ROOT,
        encoding: "utf8",
    ...
    fail("predecessor_package_json_unavailable");
  }

  if (packageJson?.dependencies?.pg !== "8.23.0") {
    fail("postgres_runtime_dependency_rollover_mismatch");
  }
  if (packageJson?.devDependencies?.["@types/pg"] !== "8.23.1") {
    fail("postgres_types_dependency_rollover_mismatch");
  }

  const comparable = JSON.parse(JSON.stringify(packageJson));
  delete comparable.dependencies.pg;
  delete comparable.devDependencies["@types/pg"];
  if (canonical(comparable) !== canonical(predecessorPackageJson)) {
    fail("package_json_rollover_scope_mismatch");
  }
...
function verifyPredecessor(artifacts) {
  const bytes = readRegularFile(PREDECESSOR_MANIFEST_PATH, 1024 * 1024);
  ...
  if (predecessor?.marker !== PREDECESSOR_MARKER) {
    fail("predecessor_manifest_marker_mismatch");
  }
  if (
    predecessor?.compiled_artifact_set_sha256 !==
    PREDECESSOR_COMPILED_ARTIFACT_SET_SHA256
  ) {
    fail("predecessor_compiled_artifact_set_mismatch");
  }
  if (canonical(predecessor?.artifacts) !== canonical(artifacts)) {
    fail("compiled_artifact_bytes_changed_from_predecessor");
  }
  return Object.freeze({
    marker: PREDECESSOR_MARKER,
...
function expectedManifest(
  ...
  return Object.freeze({
    schema: "void_buy_void_source_finality_compiled_artifact_attestation_v2",
    marker: MARKER,
    version: 2,
    ...
    build: Object.freeze({
      ...
      input_rollover: Object.freeze({
        reason: "production_postgres_client_dependency_lock",
        dependencies_added: Object.freeze({ pg: "8.23.0" }),
        dev_dependencies_added: Object.freeze({ "@types/pg": "8.23.1" }),
        source_finality_sources_changed: false,
      }),
    }),
    ...
    compiled_artifact_generation_sha256: generationSha256,
...
const generationSha256 = sha256(Buffer.from(canonical({
  marker: MARKER,
  source_stack_head: SOURCE_STACK_HEAD,
  typescript_version: lockedTypeScript,
  package_json_git_blob_sha1: EXPECTED_INPUT_BLOBS["package.json"],
...
console.log(`compiled_artifact_generation_sha256=${generationSha256}`);
console.log(`artifact_bytes_match_predecessor=true`);
console.log(`predecessor_manifest_git_blob_sha1=${PREDECESSOR_MANIFEST_GIT_BLOB_SHA1}`);
