...
import {
  ...
  type PublicAgentServiceOrderSubmissionV1,
} from "./public_agent_service_order_submission_v1.js";

export const PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_V1" as const;
export const PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_PACKET_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_PACKET_V1" as const;
export const PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_VERSION =
  1 as const;
export const PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_ID_PREFIX =
  "voidawqh1_" as const;
...
export type PublicAgentServiceSubmissionQuoteHandoffV1 = {
  marker: typeof PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_MARKER;
  version: typeof PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_VERSION;
  evidence_mode: "example_fixture" | "external_receiver_receipt";
  created_at_utc: string;
  expires_at_utc: string;
  handoff_nonce: string;
  submission_input: PublicAgentServiceOrderSubmissionV1;
  intake_receipt: Record<string, unknown>;
};

export type PublicAgentServiceSubmissionQuoteHandoffPacketV1 = {
  marker:
    typeof PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_PACKET_MARKER;
  version: typeof PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_VERSION;
  handoff_id: string;
  status: "example_only" | "ready_for_provider_quote";
  created_at_utc: string;
  expires_at_utc: string;
  source: {
  ...
  quote_contract: {
    ...
    provider_supplied_fields: readonly [
      "created_at_utc",
      "expires_at_utc",
...
function validateAllFalseAuthority(
  value: unknown,
  label: string,
): Record<string, false> {
...
function validateIntakeReceipt(
  ...
  assertCondition(
    ...
    "intake receipt marker mismatch",
  );
  ...
  requireString(
    ...
    "intake_receipt.request_payload_sha256",
    SHA256_PATTERN,
    64,
    64,
  );
  const canonicalRequestSha256 = requireString(
    receipt.canonical_request_sha256,
    "intake_receipt.canonical_request_sha256",
    SHA256_PATTERN,
    64,
    64,
  );
  const admissionId = requireString(
    receipt.admission_id,
    "intake_receipt.admission_id",
    ADMISSION_ID_PATTERN,
    74,
    74,
  );
  const receivedAt = requireIsoUtc(
    receipt.received_at_utc,
    "intake_receipt.received_at_utc",
  );

  assertCondition(
    submissionId === expected.submission_id,
    "intake receipt submission_id mismatch",
  );
  assertCondition(
    workOrderId === expected.work_order_id,
    "intake receipt work_order_id mismatch",
  );
  assertCondition(
    canonicalRequestSha256 === expected.request_sha256,
    "intake receipt canonical request SHA mismatch",
  );
  assertCondition(
    receipt.authorization_verified === true,
    "intake receipt authorization was not verified",
  );
  assertCondition(
    receipt.loopback_source === true,
    "intake receipt must be loopback sourced",
  );
  assertCondition(
    typeof receipt.duplicate === "boolean",
    "intake receipt duplicate must be boolean",
  );
  validateAllFalseAuthority(
    receipt.authority,
    "intake_receipt.authority",
  );
  ...
  assertCondition(
    ...
    "quote handoff requires credential-registry authentication",
  );
  requireString(
    authentication.registry_id,
    "intake_receipt.authentication.registry_id",
    REGISTRY_ID_PATTERN,
    75,
    75,
  );
  requireString(
    ...
    "intake_receipt.authentication.credential_id",
  ...
  requireString(
    ...
    "intake_receipt.authentication.agent_id",
  ...
  assertCondition(
    ...
    "intake receipt authentication scope mismatch",
  );
  ...
  assertCondition(
    ...
    "admission marker mismatch",
  );
  assertCondition(admission.version === 1, "admission version mismatch");
  assertCondition(
    admission.admission_id === admissionId,
    "nested admission_id mismatch",
  );
  assertCondition(
    admission.work_order_id === expected.work_order_id,
    "nested admission work_order_id mismatch",
  );
  assertCondition(
    admission.decision === "accepted_for_review",
    "submission was not accepted for review",
  );
  assertCondition(
    Array.isArray(admission.reason_codes)
      && admission.reason_codes.length === 0,
    "accepted admission must have no reason codes",
  );
  validateAllFalseAuthority(
    admission.authority,
    "intake_receipt.admission.authority",
  );
  ...
  assertCondition(
    normalized.capability_id
      === expected.work_order.service.capability_id,
    "normalized capability_id mismatch",
  );
  assertCondition(
    normalized.quote_asset
      === expected.work_order.commercial.quote_asset,
    "normalized quote_asset mismatch",
  );
  assertCondition(
    normalized.max_total
      === expected.work_order.commercial.max_total,
    "normalized max_total mismatch",
  );
  assertCondition(
    normalized.max_runtime_seconds
      === expected.work_order.execution_limits.max_runtime_seconds,
    "normalized max_runtime_seconds mismatch",
  );
  assertCondition(
    normalized.max_output_bytes
      === expected.work_order.execution_limits.max_output_bytes,
    "normalized max_output_bytes mismatch",
  );
  assertCondition(
    normalized.input_ref_count
      === expected.work_order.service.input_refs.length,
    "normalized input_ref_count mismatch",
  );
  assertCondition(
    normalized.expected_output_count
      === expected.work_order.service.expected_outputs.length,
    "normalized expected_output_count mismatch",
  );
  assertCondition(
    normalized.callback_scheme === "https",
    "normalized callback scheme must be https",
  );
  requireString(
    normalized.callback_host,
    "normalized.callback_host",
    /^[A-Za-z0-9.-]{1,253}$/,
    1,
    253,
...
export function validatePublicAgentServiceSubmissionQuoteHandoffV1(
  value: unknown,
): PublicAgentServiceSubmissionQuoteHandoffV1 {
  ...
  assertCondition(
    root.marker === PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_MARKER,
    "quote handoff marker mismatch",
  );
  assertCondition(root.version === 1, "quote handoff version mismatch");
  assertCondition(
    root.evidence_mode === "example_fixture"
      || root.evidence_mode === "external_receiver_receipt",
    "unsupported evidence_mode",
  );
  ...
  assertCondition(
    ...
    "handoff expiry must follow creation",
  );
...
export function materializePublicAgentServiceSubmissionQuoteHandoffV1(
  inputValue: unknown,
  catalogValue: unknown,
): PublicAgentServiceSubmissionQuoteHandoffPacketV1 {
  const input =
  ...
  assertCondition(
    Date.parse(input.created_at_utc)
      >= Date.parse(workOrder.created_at_utc),
    "handoff cannot predate the work order",
  );
  assertCondition(
    Date.parse(input.expires_at_utc)
      <= Date.parse(workOrder.expires_at_utc),
    "handoff cannot outlive the work order",
  );

  const receipt = validateIntakeReceipt(
    input.intake_receipt,
    {
  ...
  const source = {
    catalog_fingerprint_sha256:
      submission.catalog_fingerprint_sha256,
    ...
    authentication_mode: "credential_registry" as const,
    authentication_scope: "agent_paid_work_submit" as const,
    admission_decision: "accepted_for_review" as const,
  };
  ...
  const quoteConstraints = {
    ...
    total_must_not_exceed_max_total: true as const,
    quote_created_no_earlier_than_work_order: true as const,
    quote_expires_no_later_than_work_order: true as const,
  };

  const packetWithoutId = {
    marker:
      PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_PACKET_MARKER,
    version:
      PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_VERSION,
    status:
      input.evidence_mode === "external_receiver_receipt"
        ? "ready_for_provider_quote" as const
    ...
    quote_contract: {
      ...
      schema_path:
        "schemas/agent-paid-work-quote-envelope-v1.schema.json" as const,
      materializer_path:
        "scripts/agent_paid_work_quote_envelope_v1.ts" as const,
      materializer_export:
        "materializeAgentPaidWorkQuote" as const,
  ...
  const handoffId =
    PUBLIC_AGENT_SERVICE_SUBMISSION_QUOTE_HANDOFF_ID_PREFIX
    + sha256Text(
      canonicalJson({
        ...packetWithoutId,
...
export function verifyPublicAgentServiceSubmissionQuoteHandoffV1(
  inputValue: unknown,
  catalogValue: unknown,
  packetValue: unknown,
): PublicAgentServiceSubmissionQuoteHandoffPacketV1 {
  ...
  assertCondition(
    ...
    "quote handoff packet must be an object",
  );
  assertCondition(
    canonicalJson(packetValue) === canonicalJson(expected),
    "quote handoff packet does not match its source evidence",
  );
  return expected;
}
