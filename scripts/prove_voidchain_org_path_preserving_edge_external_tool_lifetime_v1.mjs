...
try {
  ...
  fs.writeFileSync(
    cloudflared,
    `#!/usr/bin/env node\nconst fs = require("node:fs");\nconst mode = fs.readFileSync(${JSON.stringify(modeFile)}, "utf8").trim();\nconst args = process.argv.slice(2);\nconst hang = () => setInterval(() => {}, 1000);\nif (args[0] === "--version") {\n  if (mode === "version-hang") hang();\n  if (mode === "version-flood") { process.stdout.write("x".repeat(128 * 1024)); process.exit(0); }\n  console.log("cloudflared version 2026.8.0 bounded-proof");\n  process.exit(0);\n}\nif (args[0] === "--config" && args[2] === "tunnel" && args[3] === "ingress" && args[4] === "validate") {\n  if (mode === "ingress-hang") hang();\n  if (mode === "ingress-flood") { process.stderr.write("x".repeat(128 * 1024)); process.exit(1); }\n  if (!fs.existsSync(args[1])) process.exit(92);\n  process.exit(0);\n}\nprocess.exit(91);\n`,
    { mode: 0o700 },
  );
