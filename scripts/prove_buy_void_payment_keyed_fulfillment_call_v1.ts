...
for (const [name, value, reason] of [
  ...
  [
    "incomplete_ready",
    finality({ provider_quorum_verified: false }),
    "payment_keyed_fulfillment_source_finality_not_ready",
  ],
  [
    "attempt_mismatch",
...
const oversizedIdentity = buildBuyVoidPaymentKeyedFulfillmentCallV1({
  attempt: attempt(),
  source_finality: finality({
    canonical_payment_identity: "x".repeat(4096),
  }) as any,
  policy,
});
...
const oversizedAmount = buildBuyVoidPaymentKeyedFulfillmentCallV1({
  attempt: attempt({
    unsigned_instruction: {
      ...attempt().reservation.unsigned_instruction,
      void_amount_units: "9".repeat(4096),
    },
  }),
  source_finality: finality(),
  policy,
...
console.log("merged_preflight_ready_shape_required=true");
console.log("authority_scalar_lengths_bounded=true");
