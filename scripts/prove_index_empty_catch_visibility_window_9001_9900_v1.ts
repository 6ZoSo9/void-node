...
const ID = "VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_9001_9900_V1";
const FILE = "src/index.ts";
const START = 9001;
const END = 9900;
const EXPECTED_SHA256 = "be51bd9dc5ee103034987926b2b7862226821fdbdb67f7b1d8d864d34dd3db73";
const EXPECTED_WINDOW_EMPTY_CATCH_COUNT = 0;
const EXPECTED_TOTAL_EMPTY_CATCH_COUNT = 1063;
const EXPECTED_MEASURED_CATCH_CONTEXT_COUNT = 2563;
const MARKER = "VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_9001_9900_V1_VISIBLE";
...
if (sha256 !== EXPECTED_SHA256) {
  fail("index-window-sha256-stable", `sha256=${sha256}, expected=${EXPECTED_SHA256}`);
}
pass("index-window-sha256-stable", `sha256=${sha256}`);

if (windowCount !== EXPECTED_WINDOW_EMPTY_CATCH_COUNT) {
  fail("index-window-empty-catches-closed", `line-window=${START}-${END} exact empty catch count=${windowCount}, expected=${EXPECTED_WINDOW_EMPTY_CATCH_COUNT}`);
}
...
if (totalCount !== EXPECTED_TOTAL_EMPTY_CATCH_COUNT) {
  fail("index-total-empty-catch-count-reduced", `src/index.ts line-based total exact empty catch count=${totalCount}, expected=${EXPECTED_TOTAL_EMPTY_CATCH_COUNT}`);
}
...
if (measuredCount !== EXPECTED_MEASURED_CATCH_CONTEXT_COUNT) {
  fail("index-measured-catch-context-preserved", `src/index.ts measured catch context count=${measuredCount}, expected=${EXPECTED_MEASURED_CATCH_CONTEXT_COUNT}`);
}
...
if (markers < 1) {
  fail("index-window-marker-present", `marker count=${markers}, expected>=1`);
}
