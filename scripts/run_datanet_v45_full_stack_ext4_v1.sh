...
set -euo pipefail
trap 'rc=$?; cmd=$BASH_COMMAND; line=$LINENO; printf "VOID_PR1505_RUNNER_SHELL_ERR_V1 line=%s rc=%s command=%q\n" "$line" "$rc" "$cmd" >&2; exit "$rc"' ERR
...
cleanup() {
  set +e
  [ "$r0_mounted" -eq 0 ] || /usr/bin/sudo -n umount -l "$r0_mount"
  [ "$e0_mounted" -eq 0 ] || /usr/bin/sudo -n umount -l "$e0_mount"
  [ "$r0_suspended" -eq 0 ] || /usr/bin/sudo -n dmsetup resume "$r0_dm_name" 2>/dev/null || true
  [ "$e0_suspended" -eq 0 ] || /usr/bin/sudo -n dmsetup resume "$e0_dm_name" 2>/dev/null || true
  [ "$r0_dm_on" -eq 0 ] || /usr/bin/sudo -n dmsetup remove --force "$r0_dm_name" 2>/dev/null || true
  [ "$e0_dm_on" -eq 0 ] || /usr/bin/sudo -n dmsetup remove --force "$e0_dm_name" 2>/dev/null || true
  [ "$r0_loop_on" -eq 0 ] || /usr/bin/sudo -n losetup -d "$r0_loop" 2>/dev/null || true
  [ "$e0_loop_on" -eq 0 ] || /usr/bin/sudo -n losetup -d "$e0_loop" 2>/dev/null || true
...
e0_loop="$(/usr/bin/sudo -n losetup --direct-io=on --find --show "$e0_image")"; e0_loop_on=1
r0_loop="$(/usr/bin/sudo -n losetup --direct-io=on --find --show "$r0_image")"; r0_loop_on=1
test "$e0_loop" != "$r0_loop"
e0_sectors="$(/usr/bin/sudo -n /usr/sbin/blockdev --getsz "$e0_loop")"
r0_sectors="$(/usr/bin/sudo -n /usr/sbin/blockdev --getsz "$r0_loop")"
/usr/bin/sudo -n dmsetup create "$e0_dm_name" --table "0 $e0_sectors linear $e0_loop 0"; e0_dm_on=1
/usr/bin/sudo -n dmsetup create "$r0_dm_name" --table "0 $r0_sectors linear $r0_loop 0"; r0_dm_on=1
e0_dm_dev_before="$(stat -Lc '%t:%T' "$e0_dm")"
r0_dm_dev_before="$(stat -Lc '%t:%T' "$r0_dm")"
mkdir "$e0_mount" "$r0_mount" "$evidence"
/usr/bin/sudo -n mount -o rw,nosuid,nodev,noexec "$e0_dm" "$e0_mount"; e0_mounted=1
/usr/bin/sudo -n mount -o rw,nosuid,nodev,noexec "$r0_dm" "$r0_mount"; r0_mounted=1
...
/usr/bin/sudo -n mkdir "$e0_mount/store" "$r0_mount/store"
/usr/bin/sudo -n chown "$(id -u):$(id -g)" "$e0_mount/store" "$r0_mount/store"
/usr/bin/sudo -n chmod 0700 "$e0_mount/store" "$r0_mount/store"
...
/usr/bin/sudo -n dmsetup suspend --noflush "$e0_dm_name"; e0_suspended=1
/usr/bin/sudo -n dmsetup suspend --noflush "$r0_dm_name"; r0_suspended=1
...
/usr/bin/sudo -n dmsetup resume "$r0_dm_name"; r0_suspended=0
/usr/bin/sudo -n dmsetup resume "$e0_dm_name"; e0_suspended=0
/usr/bin/sudo -n umount "$r0_mount"; r0_mounted=0
/usr/bin/sudo -n umount "$e0_mount"; e0_mounted=0
/usr/bin/sudo -n dmsetup remove "$r0_dm_name"; r0_dm_on=0
/usr/bin/sudo -n dmsetup remove "$e0_dm_name"; e0_dm_on=0
/usr/bin/sudo -n losetup -d "$r0_loop"; r0_loop_on=0
/usr/bin/sudo -n losetup -d "$e0_loop"; e0_loop_on=0

/usr/bin/sudo -n losetup --direct-io=on "$e0_loop" "$e0_crash"; e0_loop_on=1
/usr/bin/sudo -n losetup --direct-io=on "$r0_loop" "$r0_crash"; r0_loop_on=1
/usr/bin/sudo -n dmsetup create "$e0_dm_name" --table "0 $e0_sectors linear $e0_loop 0"; e0_dm_on=1
/usr/bin/sudo -n dmsetup create "$r0_dm_name" --table "0 $r0_sectors linear $r0_loop 0"; r0_dm_on=1
...
/usr/bin/sudo -n /usr/sbin/tune2fs -l "$e0_dm" >"$e0_pre_super"
/usr/bin/sudo -n /usr/sbin/tune2fs -l "$r0_dm" >"$r0_pre_super"
...
/usr/bin/sudo -n umount "$r0_mount"; r0_mounted=0
/usr/bin/sudo -n umount "$e0_mount"; e0_mounted=0
/usr/bin/sudo -n /usr/sbin/tune2fs -l "$e0_dm" >"$e0_post_super"
/usr/bin/sudo -n /usr/sbin/tune2fs -l "$r0_dm" >"$r0_post_super"
...
sync
read -r r0_loop_offset r0_loop_sizelimit < <(/usr/bin/sudo -n losetup -l -n -O OFFSET,SIZELIMIT "$r0_loop")
test "$r0_loop_offset" = "0"
test "$r0_loop_sizelimit" = "0"
r0_dm_table="$(/usr/bin/sudo -n dmsetup table "$r0_dm_name")"
...
/usr/bin/sudo -n /usr/sbin/tune2fs -l "$r0_dm" >"$v44_super_after"
...
if /usr/bin/sudo -n dmsetup info "$r0_dm_name" >/dev/null 2>&1 || /usr/bin/sudo -n dmsetup info "$e0_dm_name" >/dev/null 2>&1; then
  echo "V45 HOLD: mapper capability remained live" >&2
  exit 1
fi
test -z "$(/usr/bin/sudo -n losetup -j "$r0_crash")"
test -z "$(/usr/bin/sudo -n losetup -j "$e0_crash")"
test ! -e "$r0_dm"
test ! -e "$e0_dm"
