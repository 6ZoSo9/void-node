...
const required=[
  ...
  'ops/release/configure-void-release-publication-solo-v1.sh',
...
for(const needle of ['PREPARE_GREEN','APPROVE_GREEN','SEAL_GREEN','VERIFY_GREEN','RENDER_GREEN','VERIFY_RECORD_GREEN','FINALIZE_COMMAND_GREEN','ABORT_GREEN','manual_publication_action_required','post_publication_canary_and_qualification_still_required_for_stable','solo_time_lock_v1','NO_INDEPENDENT_REVIEW','SOLO_MIN_WAIT_MINUTES']){
...
for(const needle of ['prepare-live','verify-live','render-live','immutable-releases','void-release-publication','publication_executed=false','solo_time_lock_v1','wait_timer_minutes','deployment_branch_main_only'])if(!shell.includes(needle))fail(`live preflight contract missing: ${needle}`);
...
const soloConfig=fs.readFileSync('ops/release/configure-void-release-publication-solo-v1.sh','utf8');
for(const needle of ['wait_timer','reviewers:null','prevent_self_review:false','deployment-branch-policies','CONFIGURE VOID SOLO RELEASE TIME LOCK','independent_review=false'])if(!soloConfig.includes(needle))fail(`solo environment configurator missing: ${needle}`);
if(/gh\s+workflow\s+run|gh\s+release\s+create|git\s+tag\s+-a|git\s+push[^\n]*refs\/tags/.test(soloConfig))fail('solo environment configurator contains a publication mutation');
pass('solo-environment-configurator-no-publication-mutation');
const publicationWorkflow=fs.readFileSync('.github/workflows/public-release-publication-promotion-v1.yml','utf8');
for(const needle of ['launch_record_commit:','launch_packet_sha256:','launch_approval_sha256:','launch_authorization_sha256:','VOID_FIRST_OFFICIAL_RELEASE_LAUNCH_GATE_WORKFLOW_V1','Verify sealed launch-gate record before any publication mutation','void-first-official-release-launch-gate-v1.mjs verify-record','release/launch-gate/records/${LAUNCH_ID}','git merge-base --is-ancestor "$SOURCE_COMMIT" "$LAUNCH_RECORD_COMMIT"','VOID_FIRST_OFFICIAL_RELEASE_SOLO_OPERATOR_ENVIRONMENT_WORKFLOW_V1','solo_time_lock_v1','wait_timer_minutes'])if(!publicationWorkflow.includes(needle))fail(`publication workflow launch gate missing: ${needle}`);
