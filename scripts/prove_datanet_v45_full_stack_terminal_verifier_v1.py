...
def verify_observed_producer_receipt(obj: dict) -> None:
    ...
    demand(helper_profile == (obj.get('phase') in ('v45-static','runtime')), code)
    ...
    if runner_profile:
        ...
        demand(obs.get('isolated_session_process_group') is True
...
def verify_readonly_helper_observation(obs: dict) -> None:
    ...
    rows = plan['rows']
    demand(type(rows) is list and len(rows) == 5 and type(records) is list and len(records) == 5, code)
...
def phase_contract(phase: str, node: int) -> dict:
    ...
    specs = {
        ...
        "runner": {
            ...
            "argv": [
                "/usr/bin/timeout", "--foreground", "--signal=TERM", "--kill-after=60s", "70m", "/usr/bin/sudo", "/usr/bin/strace",
...
def verify_source_execution_receipt(
    ...
    if spec["stdout"] is not None:
        demand(obj.get("stdout_binding") == created_by_role[spec["stdout"]], "HOLD_V45_TERMINAL_SOURCE_EXECUTION_STDOUT")
    elif output_names:
        primary = bound[0]
        demand(
            obj.get("stdout_binding") == {"role": "CUSTODIAN_PIPE", "bytes": primary["bytes"], "sha256": primary["sha256"]},
            "HOLD_V45_TERMINAL_SOURCE_EXECUTION_STDOUT",
        )
    else:
