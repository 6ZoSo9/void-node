...
if (total !== 159) {
  throw new Error(`expected refined tracked raw empty catches to drop to 159, got ${total}`);
}

if ((buckets.src_diag || 0) !== 40) {
  throw new Error(`expected src_diag bucket to drop to 40, got ${buckets.src_diag || 0}`);
}
