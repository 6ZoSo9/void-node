...
const GREEN_MARKER = "VOID_DATANET_H1_S0_S1_PUBLICATION_EXT4_V1_GREEN";
const REJECT_MARKER = "VOID_DATANET_H1_S0_S1_PUBLICATION_EXT4_V1_REJECTED";
...
function requirePublishedSlot(rootStable, slotName, expectedIdentity = null) {
  ...
  if (expectedIdentity !== null) {
    assert.deepEqual(identity(st), expectedIdentity, `${slotName}: identity drift`);
  }
  return st;
}
...
function expectedNamespaceBefore(targetSlot) {
  return targetSlot === 0 ? [] : [fixture.slot_names["0"]];
}

function expectedNamespaceAfter(targetSlot) {
  return targetSlot === 0
    ? [fixture.slot_names["0"]]
    : [fixture.slot_names["0"], fixture.slot_names["1"]].sort();
}
...
function main() {
  ...
  try {
    ...
    assert.deepEqual(beforeNames, expectedNamespaceBefore(requestedSlot), "noncanonical namespace before publication");
    ...
    // No payload-allocation mutation occurs before the complete S0/S1 classifier above.
    ...
    if (existingS0 !== null) {
      assert.notDeepEqual(payloadIdentity, existingS0.identity, "S1 anonymous candidate aliases S0");
    }
    ...
    // Revalidate the complete canonical state immediately before the sole publication helper.
    ...
    // Campaign-success mode deliberately performs no second/collision ln helper.
    ...
    assert.deepEqual(afterNames, expectedNamespaceAfter(requestedSlot), "noncanonical terminal namespace");
    assertSlotMissing(rootStable, fixture.forbidden_slot_name);
