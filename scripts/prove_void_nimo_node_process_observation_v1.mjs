...
async function supervisorHooks() {
  ...
  for (const mode of ["off", "on", "prepare-failure", "diagnostic-failure"]) {
    ...
    await entry.link(specifier => {
      ...
      // Current #1458 supervisor imports. The hook keeps restore disabled and
      // supplies only inert boundaries needed to execute the real supervisor.
      if (specifier === "node:fs") return synthetic({ default: fs });
      ...
      if (specifier === "./lib/void_public_checkpoint_capability_timing_v1.mjs") return synthetic({
        parseVoidPublicCheckpointCapabilityTimingConfigV1() { return null; },
        observeVoidPublicCheckpointCapabilityTimingV1() { return Promise.resolve(null); },
      });
