...
const baseline = JSON.parse(fs.readFileSync("docs/security/index-empty-catch-visibility-window-0001-0900-v1-baseline.json", "utf8")) as {
  marker: string;
  target: string;
...
function lineBasedExactCount(source: string, maxLine?: number): number {
  return source.split(/\n/).reduce((sum, line, idx) => {
    const lineNo = idx + 1;
...
const windowExact = lineBasedExactCount(source, baseline.line_window_max);
...
const findings: Finding[] = [
  {
    ...
    detail: `line-window<=${baseline.line_window_max} exact empty catch count=${windowExact}, expected=0`,
  },
  {
...
console.log(`VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_SHA256=${hash}`);
console.log(`VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_WINDOW_EMPTY_CATCH_COUNT=${windowExact}`);
console.log(`VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_INDEX_TOTAL_EMPTY_CATCH_COUNT=${totalExact}`);
console.log(`VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_INDEX_MEASURED_CATCH_CONTEXT_COUNT=${contexts}`);
console.log(`VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_MARKER_COUNT=${markerCount}`);
...
if (findings.some((finding) => finding.status === "FAIL")) {
  console.error("VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_FAIL");
  process.exit(1);
}

console.log("VOID_INDEX_EMPTY_CATCH_VISIBILITY_WINDOW_0001_0900_V1_GREEN");
