...
import {
  ...
  type PublicAgentServiceProviderQuoteResponseV1,
  type PublicAgentServiceProviderQuoteResponsePacketV1,
} from "./public_agent_service_provider_quote_response_v1.js";

export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_V1" as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_PACKET_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_PACKET_V1" as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_KEY_BINDING_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_KEY_BINDING_V1" as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_EVIDENCE_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_EVIDENCE_V1" as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_VERSION =
  1 as const;
export const PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_SCHEME =
  "ed25519-spki-sha256-v1" as const;
export const PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_DOMAIN =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_V1" as const;
export const PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_CANONICALIZATION =
...
export type ProviderKeyBindingDraftV1 = {
  marker: typeof PUBLIC_AGENT_SERVICE_PROVIDER_KEY_BINDING_MARKER;
  version: 1;
  binding_status:
    | "example_fixture"
...
export type ProviderKeyBindingV1 =
  ProviderKeyBindingDraftV1 & {
    binding_id: string;
  };

export type ProviderQuoteResponseAuthenticationBodyV1 = {
  marker:
    typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_EVIDENCE_MARKER;
  version: 1;
  signature_scheme:
    typeof PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_SCHEME;
  signature_domain:
    typeof PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_DOMAIN;
  canonicalization:
    typeof PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_CANONICALIZATION;
  response_id: string;
  quote_id: string;
  handoff_id: string;
...
export type ProviderQuoteResponseAuthenticationEnvelopeV1 =
  ProviderQuoteResponseAuthenticationBodyV1 & {
    signature_base64: string;
    authentication_id: string;
  };

export type PublicAgentServiceProviderQuoteResponseAuthenticationV1 = {
  marker:
    typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_MARKER;
  version:
    typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_VERSION;
  evidence_mode:
    | "example_fixture"
    | "external_provider_evidence";
  provider_quote_response_input:
    PublicAgentServiceProviderQuoteResponseV1;
  provider_key_binding:
    ProviderKeyBindingV1;
  authentication_envelope:
    ProviderQuoteResponseAuthenticationEnvelopeV1;
};

export type PublicAgentServiceProviderQuoteResponseAuthenticationPacketV1 = {
  marker:
    typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_PACKET_MARKER;
  version:
    typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_VERSION;
  authentication_id: string;
  status:
  ...
  provider_key_binding: ProviderKeyBindingV1;
  authentication_envelope:
    ProviderQuoteResponseAuthenticationEnvelopeV1;
  response_packet:
    PublicAgentServiceProviderQuoteResponsePacketV1;
  verification: {
    signature_scheme:
      typeof PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_SCHEME;
    signature_domain:
...
function requireCanonicalPublicKeyPem(
  ...
  assertCondition(
    !value.includes("\r"),
    `${label} must use LF line endings`,
  );
  assertCondition(
...
function publicKeyFromPem(
  ...
  assertCondition(
    ...
    "provider public key must be Ed25519",
  );
  return key;
}

export function providerQuoteResponseAuthenticationKeyIdV1(
  publicKeyPem: string,
): string {
...
export function providerKeyBindingIdV1(
  draft: ProviderKeyBindingDraftV1,
): string {
  return `voidapkb1_${sha256Hex(canonicalJson(draft))}`;
}

function exactProviderQuoteResponseAuthenticationBodyV1(
  value: ProviderQuoteResponseAuthenticationBodyV1,
): ProviderQuoteResponseAuthenticationBodyV1 {
  return {
    marker: value.marker,
...
export function providerQuoteResponseAuthenticationSigningBytesV1(
  body: ProviderQuoteResponseAuthenticationBodyV1,
): Buffer {
  const exactBody =
    exactProviderQuoteResponseAuthenticationBodyV1(body);
  return Buffer.from(
    `${PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_DOMAIN}\n`
      + canonicalJson(exactBody),
    "utf8",
  );
}

export function providerQuoteResponseAuthenticationIdV1(
  envelopeWithoutId:
    ProviderQuoteResponseAuthenticationBodyV1 & {
      signature_base64: string;
    },
): string {
  return `voidawqa1_${sha256Hex(
...
function validateProviderKeyBinding(
  ...
  assertCondition(
    ...
    "provider key binding marker mismatch",
  );
  ...
  assertCondition(
    ...
    "provider key binding status is invalid",
  );

  const providerId = requireString(
    root.provider_id,
    "provider_key_binding.provider_id",
    SAFE_ID_PATTERN,
    3,
    128,
  );
  assertCondition(
    ...
    "provider key binding authority scope mismatch",
  );
  const keyId = requireString(
    root.key_id,
    "provider_key_binding.key_id",
    KEY_ID_PATTERN,
    72,
    72,
  );
  const publicKeyPem = requireCanonicalPublicKeyPem(
    root.public_key_pem,
    "provider_key_binding.public_key_pem",
  );
  const validFrom = requireIsoUtc(
    root.valid_from_utc,
    "provider_key_binding.valid_from_utc",
  );
  const expiresAt = requireIsoUtc(
    root.expires_at_utc,
    "provider_key_binding.expires_at_utc",
  );
  assertCondition(
    Date.parse(expiresAt) > Date.parse(validFrom),
    "provider key binding expiry must follow activation",
  );
  ...
  if (root.revoked_at_utc !== null) {
    revokedAt = requireIsoUtc(
      root.revoked_at_utc,
      "provider_key_binding.revoked_at_utc",
    );
    assertCondition(
      Date.parse(revokedAt) >= Date.parse(validFrom),
      "provider key revocation cannot predate activation",
    );
  }

  const bindingNonce = requireString(
    root.binding_nonce,
    "provider_key_binding.binding_nonce",
    NONCE_PATTERN,
    8,
    128,
  );
  const bindingId = requireString(
    root.binding_id,
    "provider_key_binding.binding_id",
    BINDING_ID_PATTERN,
    74,
    74,
  );
  ...
  assertCondition(
    keyId === derivedKeyId,
    "provider key_id does not match Ed25519 SPKI public key",
  );
  ...
  assertCondition(
    ...
    "provider key binding_id does not match canonical binding",
  );
...
function validateAuthenticationEnvelope(
  ...
  assertCondition(
    ...
    "provider authentication evidence marker mismatch",
  );
  assertCondition(root.version === 1, "authentication evidence version mismatch");
  assertCondition(
    root.signature_scheme
      === PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_SCHEME,
    "authentication signature scheme mismatch",
  );
  assertCondition(
    root.signature_domain
      === PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_SIGNATURE_DOMAIN,
    "authentication signature domain mismatch",
  );
  assertCondition(
    root.canonicalization
      === PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_CANONICALIZATION,
    "authentication canonicalization mismatch",
  );

  const body: ProviderQuoteResponseAuthenticationBodyV1 = {
    marker:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_EVIDENCE_MARKER,
    version: 1,
    signature_scheme:
    ...
    canonicalization:
      PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_CANONICALIZATION,
    response_id: requireString(
      root.response_id,
      "authentication_envelope.response_id",
      RESPONSE_ID_PATTERN,
      74,
      74,
    ),
    quote_id: requireString(
      root.quote_id,
      "authentication_envelope.quote_id",
      QUOTE_ID_PATTERN,
      73,
      73,
    ),
    handoff_id: requireString(
      root.handoff_id,
      "authentication_envelope.handoff_id",
      HANDOFF_ID_PATTERN,
      74,
      74,
    ),
    work_order_id: requireString(
      root.work_order_id,
      "authentication_envelope.work_order_id",
    ...
    submission_id: requireString(
      root.submission_id,
      "authentication_envelope.submission_id",
      SUBMISSION_ID_PATTERN,
      74,
      74,
    ),
    request_sha256: requireString(
      root.request_sha256,
      "authentication_envelope.request_sha256",
      SHA256_PATTERN,
      64,
      64,
    ),
    receipt_id: requireString(
      root.receipt_id,
      "authentication_envelope.receipt_id",
      RECEIPT_ID_PATTERN,
      74,
      74,
    ),
    provider_id: requireString(
      root.provider_id,
      "authentication_envelope.provider_id",
      SAFE_ID_PATTERN,
      3,
      128,
    ),
    catalog_fingerprint_sha256: requireString(
      ...
      "authentication_envelope.catalog_fingerprint_sha256",
      SHA256_PATTERN,
      64,
      64,
    ),
    provider_key_binding_id: requireString(
      root.provider_key_binding_id,
      "authentication_envelope.provider_key_binding_id",
      BINDING_ID_PATTERN,
      74,
      74,
    ),
    authentication_nonce: requireString(
      root.authentication_nonce,
      "authentication_envelope.authentication_nonce",
      NONCE_PATTERN,
      8,
      128,
    ),
    created_at_utc: requireIsoUtc(
      root.created_at_utc,
      "authentication_envelope.created_at_utc",
    ),
    expires_at_utc: requireIsoUtc(
      root.expires_at_utc,
      "authentication_envelope.expires_at_utc",
    ),
  };
  assertCondition(
    Date.parse(body.expires_at_utc) > Date.parse(body.created_at_utc),
    "authentication expiry must follow creation",
  );
  ...
  assertCondition(
    ...
    "authentication signature must be canonical 64-byte base64",
  );
...
export function validatePublicAgentServiceProviderQuoteResponseAuthenticationV1(
  value: unknown,
  ...
  assertCondition(
    root.marker
      === PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_MARKER,
    "provider authentication input marker mismatch",
  );
  ...
  assertCondition(
    ...
    "provider authentication evidence mode is invalid",
  );
  ...
  return {
    marker:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_MARKER,
    version:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_VERSION,
    evidence_mode: root.evidence_mode,
    provider_quote_response_input:
      validatePublicAgentServiceProviderQuoteResponseV1(
        root.provider_quote_response_input,
      ),
    provider_key_binding:
      validateProviderKeyBinding(root.provider_key_binding),
    authentication_envelope:
...
function verifyBindings(
  ...
  assertCondition(
    binding.provider_id === responsePacket.provider_claim.provider_id,
    "provider key binding does not match response provider claim",
  );
  assertCondition(
    evidence.provider_id === binding.provider_id,
    "authentication provider_id does not match key binding",
  );
  assertCondition(
    evidence.provider_key_binding_id === binding.binding_id,
    "authentication provider key binding ID mismatch",
  );
  assertCondition(
    evidence.response_id === responsePacket.response_id,
    "authentication response_id mismatch",
  );
  assertCondition(
    evidence.quote_id === responsePacket.source.quote_id,
    "authentication quote_id mismatch",
  );
  assertCondition(
    evidence.handoff_id === responsePacket.source.handoff_id,
    "authentication handoff_id mismatch",
  );
  assertCondition(
    evidence.work_order_id === responsePacket.source.work_order_id,
    "authentication work_order_id mismatch",
  );
  assertCondition(
    evidence.submission_id === responsePacket.source.submission_id,
    "authentication submission_id mismatch",
  );
  assertCondition(
    evidence.request_sha256 === responsePacket.source.request_sha256,
    "authentication request_sha256 mismatch",
  );
  assertCondition(
    evidence.receipt_id === responsePacket.source.receipt_id,
    "authentication receipt_id mismatch",
  );
  assertCondition(
    evidence.catalog_fingerprint_sha256
      === responsePacket.source.catalog_fingerprint_sha256,
    "authentication catalog fingerprint mismatch",
  );
  ...
  const quoteCreated = Date.parse(
    responsePacket.quote_envelope.created_at_utc,
  );
  const quoteExpires = Date.parse(
    responsePacket.quote_envelope.expires_at_utc,
  );

  assertCondition(
    created >= bindingStart && created < bindingEnd,
    "authentication creation is outside provider key binding window",
  );
  assertCondition(
    expires <= bindingEnd,
    "authentication outlives provider key binding",
  );
  assertCondition(
    created >= quoteCreated,
    "authentication cannot predate quote creation",
  );
  assertCondition(
    expires <= quoteExpires,
    "authentication cannot outlive quote",
  );
  if (binding.revoked_at_utc !== null) {
    assertCondition(
      created < Date.parse(binding.revoked_at_utc),
      "provider key was revoked before authentication",
    );
  }
  ...
  const verified = crypto.verify(
    null,
    providerQuoteResponseAuthenticationSigningBytesV1(evidence),
    publicKey,
    Buffer.from(evidence.signature_base64, "base64"),
  );
  assertCondition(
    verified,
    "provider quote-response authentication signature is invalid",
  );
}

export function materializePublicAgentServiceProviderQuoteResponseAuthenticationV1(
  inputValue: unknown,
  ...
  const responsePacket =
    materializePublicAgentServiceProviderQuoteResponseV1(
      input.provider_quote_response_input,
      catalogValue,
    );
  ...
  if (input.evidence_mode === "example_fixture") {
    assertCondition(
      responsePacket.status === "example_only",
      "example authentication must bind an example-only response",
    );
    assertCondition(
      input.provider_key_binding.binding_status === "example_fixture",
      "example authentication requires example key binding",
    );
  } else {
    assertCondition(
      responsePacket.status === "provider_authentication_required",
      "external authentication must bind an external response",
    );
    assertCondition(
      input.provider_key_binding.binding_status
        === "operator_approved_snapshot",
      "external authentication requires approved provider key binding",
    );
  }
  ...
  return {
    marker:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_PACKET_MARKER,
    version:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_AUTHENTICATION_VERSION,
    authentication_id:
    ...
    source: {
      ...
      provider_key_binding_id:
        input.provider_key_binding.binding_id,
...
export function verifyPublicAgentServiceProviderQuoteResponseAuthenticationV1(
  inputValue: unknown,
  catalogValue: unknown,
  packetValue: unknown,
): PublicAgentServiceProviderQuoteResponseAuthenticationPacketV1 {
  ...
  assertCondition(
    ...
    "provider authentication packet must be an object",
  );
  assertCondition(
    canonicalJson(packetValue) === canonicalJson(expected),
    "provider authentication packet does not match source evidence",
  );
  return expected;
}
...
function main(): void {
  ...
  if (mode === "materialize") {
    ...
    console.log(
      `eligible_for_acceptance=${packet.acceptance_gate.eligible_for_acceptance}`,
    );
  ...
  if (mode === "verify") {
    ...
    console.log("provider_authentication_verified=true");
    console.log(
      `eligible_for_acceptance=${result.acceptance_gate.eligible_for_acceptance}`,
    );
    console.log("provider_selection=false");
    console.log("provider_key_binding_creation=false");
    console.log("provider_key_registry_write=false");
