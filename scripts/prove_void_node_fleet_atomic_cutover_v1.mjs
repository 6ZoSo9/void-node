...
for(const bad of [
  ...
  "scripts/run_void_public_bootstrap_supervisor_v1.mjs","contracts/Foo.sol","config/mainnet.json","integrations/a.mjs","src/index.ts",
]){
...
const stage=parseStageInspectionV1([
  `head\t${TARGET}`,"branch\t","status\t","common\t/tmp/live/.git","live_common\t/tmp/live/.git","nm\t1","p0\t1","p1\t1",
].join("\n"));
...
console.log("stage_runtime_proofs=2");
