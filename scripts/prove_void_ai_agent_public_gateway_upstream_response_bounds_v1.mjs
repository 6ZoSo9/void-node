...
const teardownObservationMs = 1000;
...
function startGateway(upstreamPort, cancellationSettlementMode = "") {
  return new Promise((resolve, reject) => {
...
async function waitForRecordClosed(record, route, mode) {
  const deadline = Date.now() + teardownObservationMs;
  while (!record.closed && Date.now() < deadline) {
    await new Promise((resolve) => setTimeout(resolve, 10));
  }
  assert.equal(
    record.closed,
    true,
    `${route} ${mode} upstream response remained live after bounded rejection teardown`,
  );
}
...
async function assertPromptBoundedFailure(gatewayPort, route, mode, expectedError) {
  ...
  assert(
    elapsed < promptFailureMs,
    `${route} ${mode} waited ${elapsed.toFixed(1)}ms instead of rejecting within the bounded teardown window and before the ${upstreamTimeoutMs}ms upstream timeout`,
  );

  const record = [...responseRecords].reverse().find(
    (entry) => entry.path === route && entry.mode === mode,
  );
  assert(record, `missing upstream record for ${route} ${mode}`);
  await waitForRecordClosed(record, route, mode);
}

async function runGatewayScenario(upstreamPort, cancellationSettlementMode) {
  const gateway = await startGateway(upstreamPort, cancellationSettlementMode);
  try {
    for (const route of [operatorRoute, paidWorkRoute]) {
      if (cancellationSettlementMode === "") {
        ...
        assert.equal(
          small.status,
          route === operatorRoute ? 202 : 201,
          `${route} small response status changed`,
        );
        assert.equal(
          small.headers["x-upstream-proof"],
          "small",
          `${route} filtered small response header missing`,
        );
    ...
    const stderr = gateway.getStderr();
    if (cancellationSettlementMode === "never") {
      assert(
        stderr.includes("_response_cancel_timeout=1"),
        "never-settling cancellation did not exercise bounded teardown timeout",
      );
    }
    if (cancellationSettlementMode === "reject") {
      assert(
        stderr.includes("_proof_cancel_rejected"),
        "rejecting cancellation did not exercise cleanup-failure preservation",
      );
    }
  } finally {
...
try {
  ...
  await runGatewayScenario(upstreamPort, "");
  await runGatewayScenario(upstreamPort, "never");
  await runGatewayScenario(upstreamPort, "reject");
  ...
  console.log("owned_abort_on_rejection=1");
  console.log("bounded_rejected_teardown=1");
  console.log("never_settling_cancel_bounded=1");
  console.log("rejecting_cancel_preserves_primary_failure=1");
  console.log("rejected_upstream_connection_closed=1");
  ...
  if (upstreamListening) await close(upstream);
}
