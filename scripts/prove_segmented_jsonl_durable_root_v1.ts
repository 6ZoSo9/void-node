...
try {
  ...
  assert.deepEqual(fs.readdirSync(durableDir).sort(), [path.basename(slot0), path.basename(slot1)].sort());
  ...
  assert.equal(fs.readdirSync(durableDir).length, 2, "durable authority must remain lifetime-bounded to two slot files");
