...
assert.match(cutover, /void_public_frontdoor_cutover_v2\.py/);
assert.doesNotMatch(cutover, /tailscale funnel --|systemctl/);
const conditionalProof = execFileSync("python3", [resolve(ROOT, "scripts/prove_void_public_frontdoor_cutover_v2.py")], {
  cwd: ROOT, encoding: "utf8", timeout: 30000,
});
assert.match(conditionalProof, /VOID_PUBLIC_FRONTDOOR_CUTOVER_V2_GREEN/);
assert.match(conditionalProof, /final_compare_set_foreign_content_rejected=true/);
assert.match(conditionalProof, /retained_directory_and_kernel_lock_proved=true/);
assert.match(conditionalProof, /rollback_requires_new_confirmation=true/);
assert.match(conditionalProof, /service_generation_mutation_paths=0/);
