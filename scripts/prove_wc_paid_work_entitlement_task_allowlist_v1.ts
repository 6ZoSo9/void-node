...
import {
  VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_TASK as PRIMARY_TASK_FOR_ALLOWLIST_PROOF,
  VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_TASKS,
  isVoidWcPaidWorkEntitlementAcceptanceTask,
} from "../src/economic/wc_verified_receipt_acceptance_v1.js";


assert.deepEqual([...VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_TASKS], [
  PRIMARY_TASK_FOR_ALLOWLIST_PROOF,
  VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_SECONDARY_TASK,
]);
assert.equal(isVoidWcPaidWorkEntitlementAcceptanceTask(PRIMARY_TASK_FOR_ALLOWLIST_PROOF), true);
assert.equal(isVoidWcPaidWorkEntitlementAcceptanceTask(VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_SECONDARY_TASK), true);
assert.equal(isVoidWcPaidWorkEntitlementAcceptanceTask("void-unsupported-paid-work-task-v1"), false);
assert.equal(isVoidWcPaidWorkEntitlementAcceptanceTask(""), false);
...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "void-wc-paid-work-entitlement-task-allowlist-v1-"),
);
...
try {
  ...
  const review = signRecord({
    ...
    task_id: VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_SECONDARY_TASK,
    agent_id: account,
  ...
  const options = {
    ...
    expectedTaskId: VOID_WC_PAID_WORK_ENTITLEMENT_ACCEPTANCE_SECONDARY_TASK,
  ...
  await assert.rejects(
    () =>
      inspectPaidWorkEntitlementAcceptance(authority, {
        ...options,
        expectedTaskId: "void-unsupported-paid-work-task-v1",
      }),
    (error: any) =>
      error instanceof PaidWorkEntitlementAcceptanceError &&
      error.code === "expected_task_id_mismatch",
  );
  ...
  console.log("VOID_WC_PAID_WORK_ENTITLEMENT_TASK_ALLOWLIST_V1_PROOF_GREEN");
} finally {
