...
function proveExistingEquivalentDataFsyncConverges(
  storePath: string,
  outputPath: string,
  expectedBytes: Buffer,
): void {
  ...
  try {
    (mutableFs as any).fsyncSync = (fd: number) => {
      ...
      const visible = fs.statSync(outputPath, { bigint: true } as any);
      if (
        !injected &&
        opened.isFile() &&
        ...
        injected = true;
        const error: any = new Error("injected existing-output data fsync uncertainty");
        error.code = "EIO";
        throw error;
      }
      return originalFsyncSync(fd);
  ...
  assert.equal(injected, true, "existing equivalent data-fsync uncertainty must be injected");
  const uncertain = fs.statSync(outputPath, { bigint: true } as any);
  assert.equal(uncertain.dev, survivor.dev);
  assert.equal(uncertain.ino, survivor.ino);
  assert.deepEqual(fs.readFileSync(outputPath), expectedBytes);

  let fileDataFenced = false;
  let parentFencedAfterFile = false;
  try {
    (mutableFs as any).fsyncSync = (fd: number) => {
      const opened = fs.fstatSync(fd, { bigint: true } as any);
      const visible = fs.statSync(outputPath, { bigint: true } as any);
      if (
        ...
        fileDataFenced = true;
      } else if (opened.isDirectory() && fileDataFenced) {
        parentFencedAfterFile = true;
      }
      return originalFsyncSync(fd);
    };
    syncBuiltinESMExports();
  ...
  assert.equal(fileDataFenced, true, "existing exact file data must be fsynced before reuse");
  assert.equal(parentFencedAfterFile, true, "parent directory must be fsynced after file data");
  const durable = fs.statSync(outputPath, { bigint: true } as any);
  assert.equal(durable.dev, survivor.dev);
  assert.equal(durable.ino, survivor.ino);
  assert.deepEqual(fs.readFileSync(outputPath), expectedBytes);
...
try {
  ...
  proveExistingEquivalentDataFsyncConverges(
    store,
    path.join(tmp, "reconstruct-existing-data-unfenced-retry.jsonl"),
    body,
  );
