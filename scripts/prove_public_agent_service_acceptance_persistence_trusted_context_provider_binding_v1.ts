...
assert.equal(bindingCalls[0].arguments[0].getText(indexAst), "process.env");
assert.equal(
  bindingCalls[0].arguments[1].getText(indexAst),
  "globalThis as any",
);
assert.equal(enclosingFunctionName(bindingCalls[0]), "__main__");
...
assert.ok(
  bindingCalls[0].getStart(indexAst) < callsiteCalls[0].getStart(indexAst),
);
...
assert.deepEqual(
  publicAgentServiceAcceptancePersistenceTrustedContextProviderBindingSourceTopologyV1(),
  {
    ...
    provider_result_global:
      PUBLIC_AGENT_SERVICE_ACCEPTANCE_PERSISTENCE_TRUSTED_CONTEXT_PROVIDER_BINDING_RESULT_GLOBAL,
    bundle_shape: ["catalog", "work_order", "quote"],
...
assert.equal(disabled.authority.trusted_context_bundle_read, false);
...
const temporaryRoot = mkdtempSync(
  path.join(os.tmpdir(), "void-acceptance-context-provider-proof-"),
);
try {
  ...
  const bundle = {
    marker:
      PUBLIC_AGENT_SERVICE_ACCEPTANCE_PERSISTENCE_TRUSTED_CONTEXT_BUNDLE_MARKER,
    version: 1,
    catalog: { marker: "trusted-catalog", nested: { value: 1 } },
    work_order: { marker: "trusted-work-order" },
    quote: { marker: "trusted-quote" },
  };
  ...
  assert.equal(dry.bundle_path_fingerprint_sha256, sha256(bundlePath));
  ...
  assert.equal(applied.authority.trusted_context_bundle_read, false);
  ...
  const context = (descriptor.value as () => unknown)() as Record<
    string,
    unknown
  >;
  ...
  assert.equal(
    Object.isFrozen((context.catalog as Record<string, unknown>).nested),
    true,
  );
  ...
  writeJson(bundlePath, { ...bundle, marker: "WRONG" });
  assert.throws(
    () => readPublicAgentServiceAcceptancePersistenceTrustedContextBundleV1(
      bundlePath,
    ),
  ...
  writeJson(bundlePath, { ...bundle, extra: true });
  assert.throws(
    () => readPublicAgentServiceAcceptancePersistenceTrustedContextBundleV1(
      bundlePath,
    ),
  ...
  writeFileSync(bundlePath, "{invalid-json", {
    encoding: "utf8",
    mode: 0o600,
  });
  chmodSync(bundlePath, 0o600);
...
console.log(JSON.stringify({
  ...
  provider_installed_before_route_callsite: true,
  disabled_before_path_validation: true,
  exact_apply_confirmation_required: true,
  provider_global_non_replaceable: true,
  bundle_read_deferred_until_provider_invocation: true,
  bundle_symlink_rejected: true,
  bundle_group_other_write_rejected: true,
  bundle_top_level_keys_exact: true,
  provider_context_deep_frozen: true,
