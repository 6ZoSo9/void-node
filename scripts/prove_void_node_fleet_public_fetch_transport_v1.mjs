...
try {
  ...
  const replacementRepo = makeRepo();
  repos.push(replacementRepo);
  const replacementSnapshot = inspectRepositoryTransportV1(replacementRepo);
  const replacementPlan = buildTransportPlanV1(replacementSnapshot);
  const unchangedReplacementPlan = buildTransportPlanV1(inspectRepositoryTransportV1(replacementRepo));
  assert.equal(replacementPlan.plan_id_sha256, unchangedReplacementPlan.plan_id_sha256);
  const replacementBackup = `${replacementRepo}-original`;
  repos.push(replacementBackup);
  renameSync(replacementRepo, replacementBackup);
  cpSync(replacementBackup, replacementRepo, { recursive: true });
  const replacedSnapshot = inspectRepositoryTransportV1(replacementRepo);
  assert.equal(replacementSnapshot.head, replacedSnapshot.head);
  assert.equal(replacementSnapshot.tree, replacedSnapshot.tree);
  assert.equal(replacementSnapshot.worktree_status_sha256, replacedSnapshot.worktree_status_sha256);
  assert.equal(replacementSnapshot.index_sha256, replacedSnapshot.index_sha256);
  assert.equal(replacementSnapshot.refs_sha256, replacedSnapshot.refs_sha256);
  assert.notEqual(replacementSnapshot.repository_identity_sha256, replacedSnapshot.repository_identity_sha256);
  assert.throws(
    () => applyTransportPlanV1(replacementRepo, replacementPlan.plan_id_sha256),
    /transport plan changed before apply/,
  );
  assertNoDedicatedRemote(replacementRepo);
...
`);
  ...
  console.log('same_path_replacement_plan_reuse_rejected=true');
