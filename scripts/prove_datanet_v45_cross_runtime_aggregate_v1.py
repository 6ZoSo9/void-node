...
def verify_readonly_helper_observation(obs: dict) -> None:
    ...
    rows = plan['rows']
    require(type(rows) is list and len(rows) == 5 and type(records) is list and len(records) == 5, code)
    ...
    for i, (row, record, expected) in enumerate(zip(rows, records, requests)):
        ...
        pids.add(record['pid'])
    require(obs.get('observed_subtree_exec_count') == 6 and obs.get('observed_task_count') == 6, code)
