...
def verify_readonly_helper_observation(obs: dict) -> None:
    ...
    for i, (row, record, expected) in enumerate(zip(rows, records, requests)):
        ...
        pids.add(record['pid'])
    hold(obs.get('observed_subtree_exec_count') == len(requests)+1 and obs.get('observed_task_count') == len(requests)+1, code)
...
def verify_source_execution_stdout_binding(
    ...
    code = "HOLD_V45_SOURCE_EXECUTION_STDOUT"
    by_role = {item["role"]: item for item in created}
    if spec["stdout"] is not None:
        hold(obj.get("stdout_binding") == by_role[spec["stdout"]], code)
        ...
        hold(bool(spec["bind"]) and spec["bind"][0] in by_role, code)
