...
import {
  buildVoidBootstrapRecordReleaseRootCandidateV1,
} from "../tools/void-bootstrap-record-release-root-candidate-v1.mjs";
import {
  validateVoidBootstrapRecordReleaseRootV1,
} from "./lib/void_bootstrap_record_release_root_v1.mjs";

const MARKER = "VOID_BOOTSTRAP_RECORD_RELEASE_ROOT_CANDIDATE_V1_PROOF_GREEN";
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const TOOL = path.join(
  ROOT,
  "tools/void-bootstrap-record-release-root-candidate-v1.mjs",
);

function publicSpkiBase64(type = "ed25519") {
  const pair = crypto.generateKeyPairSync(type);
  return Buffer.from(pair.publicKey.export({ type: "spki", format: "der" })).toString(
    "base64",
  );
}
...
const root = buildVoidBootstrapRecordReleaseRootCandidateV1({
  threshold: 2,
  publicKeySpkiBase64: [keyC, keyA, keyB],
});
const validated = validateVoidBootstrapRecordReleaseRootV1(root, {
  allowHold: false,
});
...
assert.equal(
  new Set(validated.root.keys.map((entry) => entry.key_id)).size,
  validated.root.keys.length,
);
...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), "void-release-root-candidate-v1-"));
try {
  ...
  assert.match(first.stdout, /VOID_BOOTSTRAP_RECORD_RELEASE_ROOT_CANDIDATE_V1/);
  ...
  const diskValidated = validateVoidBootstrapRecordReleaseRootV1(diskRoot, {
    allowHold: false,
  });
