...
const moduleFile = thisFile.endsWith(".ts")
  ...
  : path.join(
      ...
      "src",
      "economic",
      "buy_void_native_delivery_runtime_integration_v1.js",
    );
await import(
...
process.env
  .VOID_BUY_VOID_NATIVE_DELIVERY_RUNTIME_INTEGRATION_ENABLED = "1";
process.env.VOID_BUY_VOID_NATIVE_DELIVERY_CHAIN_ID = "2050";
process.env.VOID_BUY_VOID_NATIVE_DELIVERY_ASSET_MODE =
  "native_void";
process.env.VOID_BUY_VOID_NATIVE_DELIVERY_WALLET_ADDRESS =
  wallet.address;
process.env.VOID_BUY_VOID_NATIVE_DELIVERY_MAX_AMOUNT_UNITS =
  "1000000000";
process.env.VOID_BUY_VOID_NATIVE_DELIVERY_MAX_GAS_LIMIT =
  "100000";
process.env.VOID_BUY_VOID_NATIVE_DELIVERY_MAX_FEE_PER_GAS_WEI =
  "3000000000";
process.env
  .VOID_BUY_VOID_NATIVE_DELIVERY_MAX_PRIORITY_FEE_PER_GAS_WEI =
  "2000000000";
...
const nativeValueWei = BigInt(amount) * 1_000_000_000_000n;
const unsigned = {
  type: 2,
  chainId: 2050n,
  nonce: 7,
...
assert.equal(configuredNoDeps.body.asset_mode, "native_void");
assert.equal(
  configuredNoDeps.body.unit_scale.fulfillment_unit_decimals,
  6,
);
assert.equal(
  configuredNoDeps.body.unit_scale.native_unit_decimals,
  18,
);
assert.equal(
  configuredNoDeps.body.unit_scale.multiplier,
  "1000000000000",
);
...
assert.equal(dry.body.decision.transaction_plan.to, recipient);
assert.equal(dry.body.decision.transaction_plan.value, nativeValueWei);
assert.equal(dry.body.decision.transaction_plan.data, "0x");
...
(globalThis as any)
  ...
  broadcaster: {
    async broadcast_signed_transaction(
      ...
      assert.equal(parsed.hash, expectedHash);
      assert.equal(parsed.to?.toLowerCase(), recipient);
      assert.equal(parsed.value, nativeValueWei);
