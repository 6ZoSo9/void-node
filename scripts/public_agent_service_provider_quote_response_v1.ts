...
import {
  ...
  type PublicAgentServiceSubmissionQuoteHandoffV1,
} from "./public_agent_service_submission_quote_handoff_v1.js";

export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_V1" as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_PACKET_MARKER =
  "VOID_PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_PACKET_V1" as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_VERSION =
  1 as const;
export const PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_ID_PREFIX =
  "voidawqr1_" as const;
...
export type AgentPaidWorkQuoteEnvelopeV1 = {
...
export type PublicAgentServiceProviderQuoteResponseV1 = {
  marker: typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_MARKER;
  version: typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_VERSION;
  response_nonce: string;
  quote_handoff_input: PublicAgentServiceSubmissionQuoteHandoffV1;
  quote_envelope: AgentPaidWorkQuoteEnvelopeV1;
};

export type PublicAgentServiceProviderQuoteResponsePacketV1 = {
  marker:
    typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_PACKET_MARKER;
  version: typeof PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_VERSION;
  response_id: string;
  status: "example_only" | "provider_authentication_required";
  response_nonce: string;
  source: {
...
function requireInteger(
  ...
  assertCondition(
    ...
    `${label} must be a safe integer within bounds`,
  );
  return Number(value);
}
...
function validateQuoteEnvelope(
  ...
  handoffPacket: ReturnType<
    typeof materializePublicAgentServiceSubmissionQuoteHandoffV1
  >,
): AgentPaidWorkQuoteEnvelopeV1 {
  ...
  const capabilityId = requireString(
    ...
    "quote_envelope.provider.capability_id",
    CAPABILITY_PATTERN,
    3,
    128,
  );

  const commercial = requireRecord(
    root.commercial,
    "quote_envelope.commercial",
  );
  requireExactKeys(
    commercial,
    "quote_envelope.commercial",
    ["quote_asset", "total", "payment_rail_id"],
  );
  const quoteAsset = requireString(
    commercial.quote_asset,
    "quote_envelope.commercial.quote_asset",
    /^[A-Z][A-Z0-9._:-]{0,31}$/,
    1,
    32,
  );
  const total = requireString(
    commercial.total,
    "quote_envelope.commercial.total",
    DECIMAL_PATTERN,
    1,
    51,
  );
  const paymentRailId = requireString(
    commercial.payment_rail_id,
    "quote_envelope.commercial.payment_rail_id",
    PAYMENT_RAIL_PATTERN,
    3,
    128,
  );
  ...
  const commitment = requireRecord(
    ...
    "quote_envelope.execution_commitment",
  );
  requireExactKeys(
    commitment,
    "quote_envelope.execution_commitment",
  ...
  const maxRuntime = requireInteger(
    commitment.max_runtime_seconds,
    "quote_envelope.execution_commitment.max_runtime_seconds",
    1,
    604800,
  );
  const maxOutput = requireInteger(
    commitment.max_output_bytes,
    "quote_envelope.execution_commitment.max_output_bytes",
    1,
    1073741824,
  );
  assertCondition(
    Array.isArray(commitment.output_labels)
      && commitment.output_labels.length >= 1
      && commitment.output_labels.length <= 64,
    "quote output_labels must be a bounded array",
  );
  const outputLabels = commitment.output_labels.map(
    (entry, index) =>
      requireString(
        entry,
        `quote_envelope.execution_commitment.output_labels[${index}]`,
  ...
  for (const key of [
    ...
    assertCondition(
      terms[key] === true,
      `quote_envelope.terms.${key} must be true`,
    );
  }

  const nonce = requireString(
    root.nonce,
    "quote_envelope.nonce",
    NONCE_PATTERN,
    1,
    128,
  );
  ...
  const draft = {
    ...
    execution_commitment: {
      ...
      external_side_effects_allowed: false as const,
      wallet_access_allowed: false as const,
      money_movement_allowed: false as const,
    },
    terms: {
      separate_acceptance_required: true as const,
      payment_required_before_execution: true as const,
      quote_grants_no_execution_authority: true as const,
      provider_authentication_required: true as const,
      quote_is_not_payment_instruction: true as const,
    },
    nonce,
  };
  const expectedQuoteId =
    `voidawq1_${sha256Text(canonicalJson(draft))}`;
  assertCondition(
    quoteId === expectedQuoteId,
    "quote_id does not match canonical quote payload",
  );
  ...
  assertCondition(
    workOrderId === handoffPacket.source.work_order_id,
    "quote work_order_id does not match the quote handoff",
  );
  assertCondition(
    capabilityId === workOrder.service.capability_id,
    "quote capability does not match the work order",
  );
  assertCondition(
    capabilityId === handoffPacket.quote_constraints.capability_id,
    "quote capability does not match the handoff constraints",
  );
  assertCondition(
    quoteAsset === workOrder.commercial.quote_asset,
    "quote asset does not match the work-order quote asset",
  );
  assertCondition(
    decimalUnits(total, "quote total")
      <= decimalUnits(
        workOrder.commercial.max_total,
        "work-order maximum total",
      ),
    "quote total exceeds the work-order maximum",
  );
  assertCondition(
    maxRuntime <= workOrder.execution_limits.max_runtime_seconds,
    "quote runtime exceeds the work-order maximum",
  );
  assertCondition(
    maxOutput <= workOrder.execution_limits.max_output_bytes,
    "quote output bytes exceed the work-order maximum",
  );
  assertCondition(
    JSON.stringify(outputLabels)
      === JSON.stringify(workOrder.service.expected_outputs),
    "quote output labels do not match the work order",
  );
  assertCondition(
    Date.parse(createdAt) >= Date.parse(workOrder.created_at_utc),
    "quote cannot predate the work order",
  );
  assertCondition(
    Date.parse(createdAt) >= Date.parse(handoffPacket.created_at_utc),
    "quote cannot predate the quote handoff",
  );
  assertCondition(
    Date.parse(expiresAt) <= Date.parse(workOrder.expires_at_utc),
    "quote cannot outlive the work order",
  );
  assertCondition(
    Date.parse(expiresAt) <= Date.parse(handoffPacket.expires_at_utc),
    "quote cannot outlive the quote handoff",
  );
...
export function validatePublicAgentServiceProviderQuoteResponseV1(
  value: unknown,
  ...
  assertCondition(
    root.marker === PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_MARKER,
    "provider quote response marker mismatch",
  );
  assertCondition(root.version === 1, "provider quote response version mismatch");
  const responseNonce = requireString(
    root.response_nonce,
    "response_nonce",
    /^[A-Za-z0-9._:-]{8,128}$/,
    8,
  ...
  return {
    ...
    quote_envelope: root.quote_envelope as AgentPaidWorkQuoteEnvelopeV1,
  };
}

export function materializePublicAgentServiceProviderQuoteResponseV1(
  inputValue: unknown,
  catalogValue: unknown,
): PublicAgentServiceProviderQuoteResponsePacketV1 {
  const input =
    validatePublicAgentServiceProviderQuoteResponseV1(inputValue);
  const handoffPacket =
    materializePublicAgentServiceSubmissionQuoteHandoffV1(
      input.quote_handoff_input,
      catalogValue,
    );
  const submission =
    materializePublicAgentServiceOrderSubmissionV1(
      input.quote_handoff_input.submission_input,
      catalogValue,
    );
  const workOrder = submission.request.work_order;
  const quote = validateQuoteEnvelope(
    input.quote_envelope,
    workOrder,
    handoffPacket,
  );
  ...
  const packetWithoutId = {
    marker:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_PACKET_MARKER,
    version: PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_VERSION,
    status:
      handoffPacket.status === "example_only"
        ? "example_only" as const
    ...
    authentication: {
      mode: "unverified_declarative_provider" as const,
      provider_authentication_verified: false as const,
      separately_authenticated_transport_required: true as const,
    },
    acceptance_gate: {
      eligible_for_acceptance: false as const,
      reason: "provider_authentication_required" as const,
      separate_acceptance_required: true as const,
      requester_authentication_required: true as const,
      provider_authentication_required: true as const,
    },
    authority: {
      provider_selection: false as const,
      provider_authentication: false as const,
      quote_generation: false as const,
      quote_submission: false as const,
      quote_acceptance: false as const,
      payment_authorization: false as const,
      payment_execution: false as const,
      work_execution_authorization: false as const,
  ...
  return {
    ...packetWithoutId,
    response_id:
      PUBLIC_AGENT_SERVICE_PROVIDER_QUOTE_RESPONSE_ID_PREFIX
      + sha256Text(canonicalJson(packetWithoutId)),
  };
}

export function verifyPublicAgentServiceProviderQuoteResponseV1(
  inputValue: unknown,
  catalogValue: unknown,
  packetValue: unknown,
): PublicAgentServiceProviderQuoteResponsePacketV1 {
  ...
  assertCondition(
    ...
    "provider quote response packet must be an object",
  );
  assertCondition(
    canonicalJson(packetValue) === canonicalJson(expected),
    "provider quote response packet does not match its source evidence",
  );
  assertCondition(
    RESPONSE_ID_PATTERN.test(expected.response_id),
    "provider quote response ID format mismatch",
  );
  return expected;
}
