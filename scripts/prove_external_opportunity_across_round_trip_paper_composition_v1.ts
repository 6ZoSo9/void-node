...
function refreshValuationDigest(input: JsonObject): void {
  const valuation = input.valuation as JsonObject;
  valuation.valuation_sha256 = hashAcrossTokenValuationDocumentV1({
...
const guardSource = await readFile(guardSourcePath, "utf8");
...
assert.doesNotMatch(guardSource, /private[_ -]?key|seed phrase/i);
