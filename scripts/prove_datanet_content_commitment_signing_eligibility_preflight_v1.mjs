...
import {
  ...
  runDatanetContentCommitmentSigningEligibilityPreflightAgainstFingerprintV1,
  runDatanetContentCommitmentSigningEligibilityPreflightV1,
} from "../tools/datanet-content-commitment-signing-eligibility-preflight-v1.mjs";
import {
  ...
  const transport=async(call)=>{
    ...
    switch(call.method){
          ...
          if(data.startsWith(RPC_VIEWS.getFunction(name).selector.toLowerCase())){
            ...
            if(name==="isCommitted"){
              ...
              return encodeResult(name,[
                ...
                (options.committedEligibilityFinalPass===true&&committedReads>4),
              ]);
            }
          }
        }
      ...
      case "eth_getTransactionCount":
        nonceReads+=1;
        if(options.eligibilitySecondPassNonce===true&&nonceReads>3)return "0x8";
...
function withPublisherCredential(
  ...
  const root=fs.mkdtempSync(
    path.join(os.tmpdir(),"void-datanet-signing-eligibility-v1-"),
  );
  ...
  fs.writeFileSync(credentialPath,privateKey+"\n",{mode});
...
function eligibilityInput(transport,credentialsDirectory,overrides={}){
  return {
    ...preSignInput(transport),
  ...
  try{
    const result=
      await runDatanetContentCommitmentSigningEligibilityPreflightAgainstFingerprintV1(
        eligibilityInput(f.transport,credential.root),
        sovereignFingerprint,
      );
    if(result.ok===false){
    ...
    assert.match(
      result.signing_eligibility_preflight_id,
      /^voiddccsep1_[0-9a-f]{64}$/,
    );
    assert.match(
      result.publisher_credential_binding_id,
      /^voiddccpcb1_[0-9a-f]{64}$/,
    );
    ...
    assert.equal(
      result.transaction_candidate_sha256,
      sha256(canonicalJson(result.transaction_candidate)),
    );
    assert.equal(
      result.stability.complete_revalidation_before_credential_binding,
      true,
    );
    assert.equal(
      result.stability.complete_revalidation_after_credential_binding,
      true,
    );
    assert.equal(
      result.stability.exact_unsigned_candidate_stable_across_credential_binding,
      true,
    );
    assert.equal(
    ...
    assert.equal(
      result.eligibility.eligible_for_separate_sovereign_one_shot_signing_authorization,
      true,
    );
    assert.equal(
      result.eligibility.separate_sovereign_authorization_still_required,
      true,
    );
    assert.equal(
      result.eligibility.this_receipt_is_bearer_signing_authority,
      false,
    );
...
{
  const f=transportFixture();
  const credential=withPublisherCredential("0x"+"22".repeat(32));
  try{
    const result=
      await runDatanetContentCommitmentSigningEligibilityPreflightAgainstFingerprintV1(
        eligibilityInput(f.transport,credential.root),
    ...
    assert.match(
      result.reason,
      /^signing_eligibility_publisher_credential_binding_held:/,
    );
    assert.equal(
      f.calls.filter(x=>x.method==="eth_getTransactionCount").length,
      3,
...
{
  const f=transportFixture({eligibilitySecondPassNonce:true});
  const credential=withPublisherCredential();
  try{
    const result=
      await runDatanetContentCommitmentSigningEligibilityPreflightAgainstFingerprintV1(
...
{
  const f=transportFixture({committedEligibilityFinalPass:true});
  const credential=withPublisherCredential();
  try{
    const result=
      await runDatanetContentCommitmentSigningEligibilityPreflightAgainstFingerprintV1(
    ...
    assert.match(
      result.reason,
      /^signing_eligibility_final_revalidation_held:/,
    );
    assert.equal(result.signing_performed,false);
  }finally{
    credential.cleanup();
...

{
  const f=transportFixture();
  const credential=withPublisherCredential(
    SYNTHETIC_PUBLISHER_PRIVATE_KEY,
    0o644,
  );
  try{
    const result=
...
for(const [key,expected] of Object.entries({
  ...
  assert.equal(
    VOID_DATANET_CONTENT_COMMITMENT_SIGNING_ELIGIBILITY_PREFLIGHT_AUTHORITY_V1[key],
    expected,
    key,
  );
}
...
const eligibilitySource=fs.readFileSync(
  path.join(
    ROOT,
    "tools/datanet-content-commitment-signing-eligibility-preflight-v1.mjs",
  ),
  "utf8",
);
for(const forbidden of [
  ...
  assert.equal(
    eligibilitySource.includes(forbidden),
    false,
    "eligibility source contains "+forbidden,
  );
}

console.log(
  "VOID_DATANET_CONTENT_COMMITMENT_SIGNING_ELIGIBILITY_PREFLIGHT_V1_PROOF_GREEN",
);
console.log("complete_revalidation_before_credential_binding=true");
console.log("fixed_publisher_credential_identity_bound=true");
