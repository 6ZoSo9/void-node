...
for (const body of [null, [], {}, { ok: false }, { ok: "true" }]) {
  assert.equal(parseVoidUiWave2HomeHealthOkV1(body), false);
}
...
assert.equal(
  parseVoidUiWave2HomePeerCountV1({ ok: true, connected: [] }),
  0
);
assert.equal(
  parseVoidUiWave2HomePeerCountV1({
...
for (const body of [
  ...
  { ok: false, connected: [] },
  { ok: true },
  { ok: true, connected: "bad" },
  { ok: true, connected: [null] },
  { ok: true, connected: [{ ...validPeer, unexpected: true }] },
]) {
...
): VoidUiWave2HomeSourceResultV1 => ({ ok, status, body });
...
assert.deepEqual(evaluateVoidUiWave2HomeOperationalEvidenceV1(canonical), {
...
const malformedHead = evaluateVoidUiWave2HomeOperationalEvidenceV1({
  ...canonical,
  head: result(200, {}),
});
assert.equal(malformedHead.source_available, false);
assert.equal(malformedHead.operational_ready, false);
assert.equal(malformedHead.chain_head, null);

const failedHealth = evaluateVoidUiWave2HomeOperationalEvidenceV1({
  ...canonical,
  health: result(200, { ok: false }),
});
assert.equal(failedHealth.source_available, false);
...
for (const readinessBody of [
  ...
  const malformedReady = evaluateVoidUiWave2HomeOperationalEvidenceV1({
    ...canonical,
    ready: result(200, readinessBody),
  });
  assert.equal(malformedReady.source_available, false);
  assert.equal(malformedReady.operational_ready, false);
}
...
for (const readinessBody of [
  ...
  assert.equal(notReady.source_available, true);
  assert.equal(notReady.operational_ready, false);
}

const malformedPeers = evaluateVoidUiWave2HomeOperationalEvidenceV1({
  ...canonical,
  peers: result(200, { ok: true, connected: [null] }),
});
assert.equal(malformedPeers.source_available, false);
assert.equal(malformedPeers.operational_ready, false);
assert.equal(malformedPeers.peer_count, null);
