...
export const AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_MARKER =
  "VOID_AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_V1" as const;
export const AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_PACKET_MARKER =
  "VOID_AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_PACKET_V1" as const;
export const AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_ID_PREFIX =
  "voidawqapa1_" as const;
...
export interface AuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityInputV1 {
  marker:
    typeof AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_MARKER;
  version: 1;
...
export interface AuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityPacketV1 {
  marker:
    typeof AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_PACKET_MARKER;
  version: 1;
  ...
  prepared_artifacts: {
    acceptance_envelope: AgentPaidWorkAcceptanceEnvelope;
    payment_intent_envelope: AgentPaidWorkPaymentIntentEnvelope;
  };
  acceptance_gate: {
  ...
  next_gate: {
    reason:
      "requester_provider_authentication_replay_consumption_and_atomic_persistence_required";
    next_action:
      "authenticate_then_atomically_consume_and_persist_acceptance_and_payment_intent";
...
function requireDecimal(value: unknown, label: string): string {
  ...
  assertCondition(
    ...
    `${label} must be a bounded non-negative decimal string`,
  );
  return text;
}
...
function validateInputShape(
  ...
  assertCondition(
    ...
    `marker must be ${AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_MARKER}`,
  );
  ...
  const acceptancePlan = requireRecord(
  ...
  const acceptanceCreatedAt = requireUtc(
    acceptancePlan.created_at_utc,
    "acceptance_plan.created_at_utc",
  );
  const acceptanceExpiresAt = requireUtc(
    acceptancePlan.expires_at_utc,
    "acceptance_plan.expires_at_utc",
  );
  const acceptanceNonce = requirePattern(
    acceptancePlan.nonce,
  ...
  const paymentPlan = requireRecord(
  ...
  const paymentCreatedAt = requireUtc(
    paymentPlan.created_at_utc,
    "payment_authority_plan.created_at_utc",
  );
  const paymentExpiresAt = requireUtc(
    paymentPlan.expires_at_utc,
    "payment_authority_plan.expires_at_utc",
  );
  const maxFeeTotal = requireDecimal(
    paymentPlan.max_fee_total,
    "payment_authority_plan.max_fee_total",
  );
  const paymentNonce = requirePattern(
    paymentPlan.nonce,
...
function buildAcceptanceDraft(
  input: AuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityInputV1,
...
function buildPaymentIntentDraft(
  input: AuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityInputV1,
...
export function authenticatedPaidWorkQuoteAcceptancePaymentAuthorityIdV1(
  packet: PacketWithoutId,
): string {
...
export function materializeAuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityV1(
  inputValue: unknown,
): AuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityPacketV1 {
  const input = validateInputShape(inputValue);
  const acceptance = materializeAgentPaidWorkAcceptance(
    input.work_order,
    input.quote,
    buildAcceptanceDraft(input),
  );
  validateAgentPaidWorkAcceptanceEnvelope(
    input.work_order,
    input.quote,
  ...
  const paymentIntent = materializeAgentPaidWorkPaymentIntent(
    input.work_order,
    input.quote,
    acceptance,
    buildPaymentIntentDraft(input, acceptance),
  );
  validateAgentPaidWorkPaymentIntentEnvelope(
    input.work_order,
    input.quote,
  ...
  const packetWithoutId: PacketWithoutId = {
    marker:
      AUTHENTICATED_PAID_WORK_QUOTE_ACCEPTANCE_PAYMENT_AUTHORITY_PACKET_MARKER,
    version: 1,
    status: "prepared_requires_authenticated_atomic_activation",
    source: {
      work_order_id: input.work_order.work_order_id,
  ...
  return {
    ...packetWithoutId,
    packet_id:
      authenticatedPaidWorkQuoteAcceptancePaymentAuthorityIdV1(
        packetWithoutId,
      ),
  };
}

export function verifyAuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityV1(
  inputValue: unknown,
  packetValue: unknown,
): AuthenticatedPaidWorkQuoteAcceptancePaymentAuthorityPacketV1 {
  const expected =
  ...
  assertCondition(
    typeof packet.packet_id === "string" && ID_PATTERN.test(packet.packet_id),
    "packet_id format mismatch",
  );
  assertCondition(
    canonicalJson(packetValue) === canonicalJson(expected),
    "packet does not match source input",
  );
  return expected;
}
