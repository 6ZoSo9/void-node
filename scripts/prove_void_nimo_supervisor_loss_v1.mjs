...
async function releaseEntry(state, existingData = null) {
  const ticks = await waitFor(() => {
    assert(!state.fault, state.fault);
    assert(!state.exit, `parent exited before entry barrier: ${JSON.stringify(state.exit)}`);
    return state.events.includes("entry-started");
  }, "entry barrier");
...
try {
  ...
  const checkpointBoundary = new Map([
    ["scripts/lib/void_public_checkpoint_restore_supervisor_v1.mjs",
...
`],
    ["scripts/lib/void_public_checkpoint_restore_activation_v1.mjs",
      `export function closeSelectedCheckpointGenerationV1() {}
`],
    ["scripts/lib/void_public_checkpoint_capability_timing_v1.mjs",
...
export async function observeVoidPublicCheckpointCapabilityTimingV1() {
  throw new Error("capability timing fixture unexpectedly activated");
}
`],
  ]);
  for (const [relative, bytes] of checkpointBoundary) {
