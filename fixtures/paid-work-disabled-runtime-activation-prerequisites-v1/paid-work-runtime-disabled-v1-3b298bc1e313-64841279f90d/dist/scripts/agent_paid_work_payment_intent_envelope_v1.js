...
export const AGENT_PAID_WORK_PAYMENT_INTENT_MARKER = "VOID_AGENT_PAID_WORK_PAYMENT_INTENT_ENVELOPE_V1";
export const AGENT_PAID_WORK_PAYMENT_INTENT_ID_PREFIX = "voidawpi1_";
...
function requireTrimmedString(value, label, minimum, maximum) {
    ...
    assertCondition(value.length >= minimum && value.length <= maximum, `${label} length must be ${minimum}..${maximum}`);
    return value;
}
function requirePattern(value, label, pattern, minimum, maximum) {
    const text = requireTrimmedString(value, label, minimum, maximum);
    assertCondition(pattern.test(text), `${label} has invalid format`);
    return text;
}
function requireDecimal(value, label, allowZero) {
...
function parseUtcSeconds(value, label) {
    ...
    assertCondition(new Date(milliseconds).toISOString() === value.replace("Z", ".000Z"), `${label} is not canonical UTC`);
...
function validateDraftShape(value, allowId) {
    ...
    assertCondition(root.marker === AGENT_PAID_WORK_PAYMENT_INTENT_MARKER, `marker must be ${AGENT_PAID_WORK_PAYMENT_INTENT_MARKER}`);
    ...
    const maxFeeTotal = requireDecimal(commercial.max_fee_total, "commercial.max_fee_total", true);
    ...
    const trueKeys = authorizationKeys.filter((key) => key !== "payment_execution_granted" &&
        key !== "work_execution_authorization_granted");
    for (const key of trueKeys) {
        assertCondition(authorization[key] === true, `authorization.${key} must be true`);
    }
    assertCondition(authorization.payment_execution_granted === false, "authorization.payment_execution_granted must be false");
    assertCondition(authorization.work_execution_authorization_granted === false, "authorization.work_execution_authorization_granted must be false");
...
function validateBindings(workOrder, quote, acceptance, intent) {
    ...
    assertCondition(intent.requester.agent_id === acceptance.requester.agent_id, "requester mismatch");
    assertCondition(intent.provider.provider_id === acceptance.provider.provider_id, "provider mismatch");
    assertCondition(intent.commercial.quote_asset === acceptance.commercial.quote_asset, "quote asset mismatch");
    assertCondition(intent.commercial.total === acceptance.commercial.total, "total must exactly match the acceptance");
    assertCondition(intent.commercial.payment_rail_id === acceptance.commercial.payment_rail_id, "payment_rail_id mismatch");
    const combined = addDecimals(intent.commercial.total, intent.commercial.max_fee_total);
    assertCondition(compareDecimals(combined, workOrder.commercial.max_total) <= 0, "total plus max_fee_total exceeds the work-order max_total");
    ...
    assertCondition(intentCreated >= acceptanceCreated, "payment intent cannot be created before acceptance");
    ...
    assertCondition(acceptance.terms.acceptance_replay_protection_required === true, "acceptance must require replay protection");
    assertCondition(acceptance.terms.single_active_acceptance_per_quote_required === true, "acceptance must require one active acceptance per quote");
    assertCondition(acceptance.terms.acceptance_is_not_funds_reservation === true, "acceptance must not reserve funds");
    assertCondition(acceptance.terms.payment_authorization_granted === false, "acceptance must not already grant payment authority");
    assertCondition(acceptance.terms.execution_authorization_granted === false, "acceptance must not grant execution authority");
}
export function computeAgentPaidWorkPaymentIntentId(draft) {
...
export function validateAgentPaidWorkPaymentIntentDraft(workOrderValue, quoteValue, acceptanceValue, intentValue) {
    validateAgentPaidWorkOrderEnvelope(workOrderValue);
    validateAgentPaidWorkQuoteEnvelope(workOrderValue, quoteValue);
    validateAgentPaidWorkAcceptanceEnvelope(workOrderValue, quoteValue, acceptanceValue);
    const intent = validateDraftShape(intentValue, false);
    validateBindings(workOrderValue, quoteValue, acceptanceValue, intent);
}
export function materializeAgentPaidWorkPaymentIntent(workOrderValue, quoteValue, acceptanceValue, intentValue) {
    validateAgentPaidWorkOrderEnvelope(workOrderValue);
    validateAgentPaidWorkQuoteEnvelope(workOrderValue, quoteValue);
    validateAgentPaidWorkAcceptanceEnvelope(workOrderValue, quoteValue, acceptanceValue);
...
export function validateAgentPaidWorkPaymentIntentEnvelope(workOrderValue, quoteValue, acceptanceValue, intentValue) {
    validateAgentPaidWorkOrderEnvelope(workOrderValue);
    validateAgentPaidWorkQuoteEnvelope(workOrderValue, quoteValue);
    validateAgentPaidWorkAcceptanceEnvelope(workOrderValue, quoteValue, acceptanceValue);
    ...
    validateBindings(workOrderValue, quoteValue, acceptanceValue, draft);
    const id = requirePattern(root.payment_intent_id, "payment_intent_id", /^voidawpi1_[0-9a-f]{64}$/, 74, 74);
    assertCondition(id === computeAgentPaidWorkPaymentIntentId(draft), "payment_intent_id does not match the canonical payload");
}
...
function main() {
    const [mode, workPath, quotePath, acceptancePath, intentPath, outputPath, ...extra] = process.argv.slice(2);
    assertCondition(extra.length === 0, "unexpected extra arguments");
    if (mode === "materialize") {
        assertCondition(Boolean(workPath && quotePath && acceptancePath && intentPath && outputPath), "materialize requires work-order, quote, acceptance, draft, and output paths");
        const resolvedOutput = resolve(outputPath);
        assertCondition(!existsSync(resolvedOutput), "refusing to overwrite an existing payment intent");
        const envelope = materializeAgentPaidWorkPaymentIntent(readJson(resolve(workPath)), readJson(resolve(quotePath)), readJson(resolve(acceptancePath)), readJson(resolve(intentPath)));
    ...
    if (mode === "verify") {
        assertCondition(Boolean(workPath && quotePath && acceptancePath && intentPath) &&
            outputPath === undefined, "verify requires work-order, quote, acceptance, and intent-envelope paths");
