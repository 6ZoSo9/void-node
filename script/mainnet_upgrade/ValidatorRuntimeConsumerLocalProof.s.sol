...
contract MockVoidTokenRuntimeConsumerProof {
...
contract ValidatorRuntimeConsumerLocalProof is Script {
    function run() external {
        ...
        MockVoidTokenRuntimeConsumerProof token = new MockVoidTokenRuntimeConsumerProof();
        ...
        uint256 count = consumer.validatorCount();
        uint256 total = consumer.totalPower();
        IValidatorSelectionSource.SelectableValidator memory v = consumer.validatorAt(0);

        console2.log("consumer.count", count);
        console2.log("consumer.totalPower", total);
        console2.log("consumer.selected.reward", v.reward);
        console2.log("consumer.selected.controller", v.controller);
        console2.logBytes32(v.consensusKey);
        console2.log("consumer.selected.effectivePower", v.effectivePower);
    }
}
