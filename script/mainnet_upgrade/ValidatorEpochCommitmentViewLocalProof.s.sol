...
contract MockVoidTokenEpochCommitmentProof {
...
contract ValidatorHelperEpochCommitment {
    function approveToken(address token, address spender, uint256 amount) external {
        MockVoidTokenEpochCommitmentProof(token).approve(spender, amount);
    }
...
contract ValidatorEpochCommitmentViewLocalProof is Script {
    struct Env {
        MockVoidTokenEpochCommitmentProof token;
        ...
        ValidatorHelperEpochCommitment h1;
        ValidatorHelperEpochCommitment h2;
        ValidatorHelperEpochCommitment h3;
    }

    function _deploy(address admin) internal returns (Env memory e) {
        e.token = new MockVoidTokenEpochCommitmentProof();
        ...
        e.h1 = new ValidatorHelperEpochCommitment();
        e.h2 = new ValidatorHelperEpochCommitment();
        e.h3 = new ValidatorHelperEpochCommitment();
    }
    ...
    function run() external {
        ...
        console2.log("=== epoch 1 commitments ===");
        console2.logBytes32(e.commitmentView.validatorSetCommitment(1));
        console2.logBytes32(e.commitmentView.scheduleWindowCommitment(1, 0, 8));
        console2.logBytes32(e.commitmentView.epochWindowCommitment(1, 0, 8));

        e.h2.beginExit(address(e.staking));
        e.snapshot.captureEpoch(2);

        console2.log("=== epoch 2 commitments ===");
        console2.logBytes32(e.commitmentView.validatorSetCommitment(2));
        console2.logBytes32(e.commitmentView.scheduleWindowCommitment(2, 0, 8));
        console2.logBytes32(e.commitmentView.epochWindowCommitment(2, 0, 8));
