...
contract MockVoidTokenSelectionProof {
...
contract ValidatorSelectionAdapterLocalProof is Script {
    function run() external {
        ...
        MockVoidTokenSelectionProof token = new MockVoidTokenSelectionProof();
        ...
        uint256 count = adapter.getSelectableValidatorCount();
        uint256 total = adapter.totalSelectablePower();
        IValidatorSelectionSource.SelectableValidator memory v = adapter.getSelectableValidatorAt(0);

        console2.log("selectable.count", count);
        console2.log("selectable.totalPower", total);
        console2.log("selected.reward", v.reward);
        console2.log("selected.controller", v.controller);
        console2.logBytes32(v.consensusKey);
        console2.log("selected.effectivePower", v.effectivePower);
    }
}
