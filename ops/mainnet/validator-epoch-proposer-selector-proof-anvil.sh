...
ANVIL_PORT="${ANVIL_PORT:-9975}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-epoch-proposer-selector.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded epoch proposer selector proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator epoch proposer selector guarded proof completed on disposable anvil"
