...
ANVIL_PORT="${ANVIL_PORT:-10005}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-epoch-commitment-registry.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded epoch commitment registry proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator epoch commitment registry guarded proof completed on disposable anvil"
