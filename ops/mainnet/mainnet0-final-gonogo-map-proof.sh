...
grep -q 'current_baseline_pointer_commit: 16c20d3f' "$DOC"
grep -q 'current_baseline_pointer_tag: ckpt-public-validator-candidate-only-posture-green-20260522-125742' "$DOC"
grep -q 'cross_box_proven: true' "$DOC"
grep -q 'NO-GO' "$DOC"
grep -q 'Ready signals alone are not launch approval' "$DOC"
grep -q 'Money step remains last' "$DOC"
grep -q 'Current baseline pointer now records the public validator candidate-only posture checkpoint as the canonical rolling baseline.' "$DOC"
...
grep -q '^commit: 16c20d3f$' "$BASELINE"
grep -q '^tag: ckpt-public-validator-candidate-only-posture-green-20260522-125742$' "$BASELINE"
