...
grep -q 'current_baseline_pointer_commit: 56f9c2d2' "$DOC"
grep -q 'current_baseline_pointer_tag: ckpt-final-public-launch-checklist-product-baseline-green-20260522-084710' "$DOC"
grep -q 'cross_box_proven: true' "$DOC"
grep -q 'NO-GO' "$DOC"
grep -q 'Ready signals alone are not launch approval' "$DOC"
grep -q 'Money step remains last' "$DOC"
grep -q 'Current baseline pointer now records final public launch checklist product-baseline as the canonical rolling baseline.' "$DOC"
...
grep -q '^commit: 56f9c2d2$' "$BASELINE"
grep -q '^tag: ckpt-final-public-launch-checklist-product-baseline-green-20260522-084710$' "$BASELINE"
