...
grep -q 'current_baseline_pointer_commit: 654ea54f' "$DOC"
grep -q 'current_baseline_pointer_tag: ckpt-launch-approval-artifact-prep-green-20260523-024355' "$DOC"
grep -q 'cross_box_proven: true' "$DOC"
grep -q 'NO-GO' "$DOC"
grep -q 'Ready signals alone are not launch approval' "$DOC"
grep -q 'Money step remains last' "$DOC"
grep -q 'Current baseline pointer now records the launch approval artifact prep checkpoint as the canonical rolling baseline.' "$DOC"
