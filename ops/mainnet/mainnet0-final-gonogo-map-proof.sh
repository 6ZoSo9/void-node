...
grep -q 'current_baseline_pointer_commit: 152cf74c' "$DOC"
grep -q 'current_baseline_pointer_tag: ckpt-final-checklist-validator-candidate-posture-green-20260523-020915' "$DOC"
grep -q 'cross_box_proven: true' "$DOC"
grep -q 'NO-GO' "$DOC"
grep -q 'Ready signals alone are not launch approval' "$DOC"
grep -q 'Money step remains last' "$DOC"
grep -q 'Current baseline pointer now records the final checklist validator candidate posture checkpoint as the canonical rolling baseline.' "$DOC"
...
grep -q '^commit: 152cf74c$' "$BASELINE"
grep -q '^tag: ckpt-final-checklist-validator-candidate-posture-green-20260523-020915$' "$BASELINE"
