...
grep -q 'current_baseline_pointer_commit: 5b279795' "$DOC"
grep -q 'current_baseline_pointer_tag: ckpt-product-surface-datanet-goldenpath-green-20260522-075118' "$DOC"
grep -q 'cross_box_proven: true' "$DOC"
grep -q 'NO-GO' "$DOC"
grep -q 'Ready signals alone are not launch approval' "$DOC"
grep -q 'Money step remains last' "$DOC"
grep -q 'Current baseline pointer now records product surface/DataNet golden path as the canonical rolling baseline.' "$DOC"
...
grep -q '^commit: 5b279795$' "$BASELINE"
grep -q '^tag: ckpt-product-surface-datanet-goldenpath-green-20260522-075118$' "$BASELINE"
