...
grep -q 'current_baseline_pointer_commit: f8684313' "$DOC"
grep -q 'current_baseline_pointer_tag: ckpt-settings-escape-close-green-20260522-120632' "$DOC"
grep -q 'cross_box_proven: true' "$DOC"
grep -q 'NO-GO' "$DOC"
grep -q 'Ready signals alone are not launch approval' "$DOC"
grep -q 'Money step remains last' "$DOC"
grep -q 'Current baseline pointer now records the Settings escape-close product-surface checkpoint as the canonical rolling baseline.' "$DOC"
...
grep -q '^commit: f8684313$' "$BASELINE"
grep -q '^tag: ckpt-settings-escape-close-green-20260522-120632$' "$BASELINE"
