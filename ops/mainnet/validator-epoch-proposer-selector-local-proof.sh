...
RPC_URL="${RPC_URL:-http://127.0.0.1:9975}"
...
if [ "$ACTUAL_CHAIN_ID" = "2050" ] && [ "$FORCE_REAL_CHAIN" != "1" ]; then
  echo "[ERR] refusing to run validator-epoch-proposer-selector local proof on real chain 2050"
...
echo
echo "=== [3] run epoch proposer selector local proof script ==="
export PRIVATE_KEY
forge script script/mainnet_upgrade/ValidatorEpochProposerSelectorLocalProof.s.sol:ValidatorEpochProposerSelectorLocalProof \
  --rpc-url "$RPC_URL" \
