...
ANVIL_PORT="${ANVIL_PORT:-9945}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-runtime-consumer-multival.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded multival runtime consumer proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator runtime consumer multival proof completed on disposable anvil"
