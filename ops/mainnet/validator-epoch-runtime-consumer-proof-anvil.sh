...
ANVIL_PORT="${ANVIL_PORT:-9965}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-epoch-runtime-consumer.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded epoch runtime consumer proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator epoch runtime consumer guarded proof completed on disposable anvil"
