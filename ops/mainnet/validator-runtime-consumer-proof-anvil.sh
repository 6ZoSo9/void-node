...
ANVIL_PORT="${ANVIL_PORT:-9845}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-runtime-consumer.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded runtime consumer proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator runtime consumer guarded proof completed on disposable anvil"
