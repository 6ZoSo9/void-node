...
ANVIL_PORT="${ANVIL_PORT:-10015}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-epoch-manifest-view.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded epoch manifest view proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator epoch manifest view guarded proof completed on disposable anvil"
