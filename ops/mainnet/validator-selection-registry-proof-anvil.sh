...
ANVIL_PORT="${ANVIL_PORT:-9745}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-selection-registry.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded registry proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator selection registry guarded proof completed on disposable anvil"
