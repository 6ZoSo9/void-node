...
if [ "$ACTUAL_CHAIN_ID" = "2050" ] && [ "$FORCE_REAL_CHAIN" != "1" ]; then
  echo "[ERR] refusing to run validator-staking-v2 local proof on real chain 2050"
...
echo
echo "=== [3] run local staking proof script ==="
forge script script/mainnet_upgrade/ValidatorStakingV2LocalProof.s.sol:ValidatorStakingV2LocalProof \
  --rpc-url "$RPC_URL" \
