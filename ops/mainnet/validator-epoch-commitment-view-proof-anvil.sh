...
ANVIL_PORT="${ANVIL_PORT:-9995}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-epoch-commitment-view.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded epoch commitment view proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator epoch commitment view guarded proof completed on disposable anvil"
