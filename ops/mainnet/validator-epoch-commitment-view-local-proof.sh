...
RPC_URL="${RPC_URL:-http://127.0.0.1:9995}"
...
if [ "$ACTUAL_CHAIN_ID" = "2050" ] && [ "$FORCE_REAL_CHAIN" != "1" ]; then
  echo "[ERR] refusing to run validator-epoch-commitment-view local proof on real chain 2050"
...
echo
echo "=== [3] run epoch commitment view local proof script ==="
export PRIVATE_KEY
forge script script/mainnet_upgrade/ValidatorEpochCommitmentViewLocalProof.s.sol:ValidatorEpochCommitmentViewLocalProof \
  --rpc-url "$RPC_URL" \
