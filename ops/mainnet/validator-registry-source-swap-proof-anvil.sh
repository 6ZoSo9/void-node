...
ANVIL_PORT="${ANVIL_PORT:-9955}"
ANVIL_LOG="${ANVIL_LOG:-/tmp/anvil.validator-registry-source-swap.$(date +%Y%m%d-%H%M%S).log}"
...
echo
echo "=== [3] run guarded registry source swap proof ==="
RPC_URL="$RPC_URL" \
...
echo
echo "=== [4] done ==="
echo "[ok] validator registry source swap proof completed on disposable anvil"
