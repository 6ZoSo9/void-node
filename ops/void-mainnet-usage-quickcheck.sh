...
# If PROM_URL looks bogus (literally '$PROM_URL', 'PROM_URL', or missing http/https),
# force it back to the default local Prometheus.
if [[ "$PROM_URL" == '$PROM_URL' || "$PROM_URL" == 'PROM_URL' || ( "$PROM_URL" != http://* && "$PROM_URL" != https://* ) ]]; then
