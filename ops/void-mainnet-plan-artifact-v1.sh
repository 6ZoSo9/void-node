...
EXPECTED_REVERT=0
if [[ "$RC" -ne 0 ]]; then
  if rg -n "RUN_STUB_ONLY" "$PLAN_TXT" >/dev/null 2>&1; then
    EXPECTED_REVERT=1
  else
    OK=0
  fi
...
# HELP void_mainnet_bootstrap_plan_artifact_expected_revert 1 if PLAN ended in the expected RUN_STUB_ONLY revert.
# TYPE void_mainnet_bootstrap_plan_artifact_expected_revert gauge
void_mainnet_bootstrap_plan_artifact_expected_revert ${EXPECTED_REVERT}

# HELP void_mainnet_bootstrap_plan_artifact_rc Exit code from forge script.
# TYPE void_mainnet_bootstrap_plan_artifact_rc gauge
void_mainnet_bootstrap_plan_artifact_rc ${RC}
