...
if [ -z "${chosen_manifest_days}" ] || [ "${chosen_manifest_days}" = "null" ] || [ "${chosen_manifest_days}" = "error" ]; then
...
if [ "${chosen_manifest_days}" != "-1" ]; then
  if [ "${chosen_manifest_days}" -ge 7 ] 2>/dev/null; then
    ...
    echo "[pillars] ERROR: manifest_days too low (${chosen_manifest_days} < 7)"
  fi
else
...
echo
echo "[pillars] RESULT: OK (safeboot+devnet+mainnet-core healthy; chosen_manifest_days=${chosen_manifest_days})"
