...
OUT_DIR="${OUT_DIR:-/tmp/two-box-reverse-bidirectional-open-proof-$TS_NOW}"
mkdir -p "$OUT_DIR"
...
LOCAL_HEALTH="$(jget "$LOCAL_NODE_BASE/health" 10)"
...
printf '%s\n' "$LOCAL_HEALTH" | tee "$OUT_DIR/local-health-before.json" >/dev/null
...
print("[ok] baseline reverse bidirectional truth aligned")
PY

echo
echo "=== [2] publish dataset on Precision ==="
LOCAL_PUBLISH="$(jpost_json "$LOCAL_NODE_BASE/jobs/submit" "{\"account\":\"$ACCOUNT\",\"kind\":\"datanet_publish\",\"plaintext\":\"$PLAINTEXT_LOCAL\"}" 20)"
printf '%s\n' "$LOCAL_PUBLISH" | tee "$OUT_DIR/local-publish-submit.json" >/dev/null

LOCAL_JOB_ID="$(python3 - "$OUT_DIR/local-publish-submit.json" <<'PY'
import json, sys
...
for i in $(seq 1 20); do
  LOCAL_JOB="$(jget "$LOCAL_NODE_BASE/jobs/$LOCAL_JOB_ID" 20)"
  LOCAL_JOB_FILE="$OUT_DIR/local-publish-job-$i.json"
  printf '%s\n' "$LOCAL_JOB" | tee "$LOCAL_JOB_FILE" >/dev/null
  LOCAL_STATUS="$(python3 - "$LOCAL_JOB_FILE" <<'PY'
import json, sys
j = json.load(open(sys.argv[1]))
print(((j.get("job") or {}).get("status")) or "")
PY
)"
  echo "local_publish_status=$LOCAL_STATUS poll=$i"
  [ "$LOCAL_STATUS" = "completed" ] && break
  sleep 1
done
test "$LOCAL_STATUS" = "completed"
test -n "$LOCAL_JOB_FILE"

LOCAL_DATASET_ID="$(python3 - "$LOCAL_JOB_FILE" <<'PY'
import json, sys
j = json.load(open(sys.argv[1]))
print(((j.get("job") or {}).get("dataset_id")) or "")
PY
)"
LOCAL_RECEIPT_ID="$(python3 - "$LOCAL_JOB_FILE" <<'PY'
import json, sys
j = json.load(open(sys.argv[1]))
...
test -n "$LOCAL_DATASET_ID"
test -n "$LOCAL_RECEIPT_ID"

echo "local_dataset_id=$LOCAL_DATASET_ID"
echo "local_receipt_id=$LOCAL_RECEIPT_ID"

echo
echo "=== [3] verify Precision already visible from Alienware peer surfaces ==="
REMOTE_PEERS_NOW="$(jget "$REMOTE_NODE_BASE/peers" 10)"
printf '%s\n' "$REMOTE_PEERS_NOW" | tee "$OUT_DIR/remote-peers-now.json" >/dev/null

python3 - "$OUT_DIR/remote-peers-now.json" "$OUT_DIR/local-health-before.json" <<'PY'
import json, sys
peers = json.load(open(sys.argv[1]))
lh = json.load(open(sys.argv[2]))
local_id = str(lh.get("nodeId") or "")
connected = peers.get("connected") or []
known = [str(x) for x in (peers.get("knownAddrs") or [])]
assert local_id, "local node id missing"
assert any(str(x.get("id") or "") == local_id for x in connected), f"local node id {local_id} not present in remote connected peers: {connected}"
assert "100.93.2.116:4700" in known, f"local addr not present in remote knownAddrs: {known}"
print("[ok] alienware already sees precision on live peer surface")
PY

echo
echo "=== [4] consume Precision dataset from Alienware ==="
CONSUME_URL="$REMOTE_NODE_BASE/datanet/consume-view/$LOCAL_DATASET_ID?who=$ACCOUNT"
echo "$CONSUME_URL"
curl -fsS --max-time 20 "$CONSUME_URL" > "$OUT_DIR/alienware-consume-view.html"

python3 - "$OUT_DIR/alienware-consume-view.html" "$LOCAL_DATASET_ID" "$PLAINTEXT_LOCAL" <<'PY'
...
echo
echo "=== [5] verify Alienware local materialization ==="
REMOTE_MAT="$(jget "$REMOTE_NODE_BASE/datanet/v1/local-job/$LOCAL_DATASET_ID?who=$ACCOUNT" 20)"
printf '%s\n' "$REMOTE_MAT" | tee "$OUT_DIR/remote-materialized-job.json" >/dev/null

python3 - "$OUT_DIR/remote-materialized-job.json" "$ACCOUNT" "$PLAINTEXT_LOCAL" <<'PY'
import json, sys, hashlib
j = json.load(open(sys.argv[1]))
account = sys.argv[2]
plaintext = sys.argv[3]
want_sha = hashlib.sha256(plaintext.encode()).hexdigest()
assert j.get("ok") is True, f"remote materialized job not ok: {j}"
...
print(json.dumps({
  ...
  "sha256": j.get("sha256"),
  "materialized_on_alienware": True
}, indent=2))
PY
...
python3 - "$OUT_DIR/local-ready-after.json" "$OUT_DIR/remote-ready-after.json" "$OUT_DIR/local-peer-after.json" "$OUT_DIR/remote-peer-after.json" "$OUT_DIR/remote-materialized-job.json" <<'PY'
import json, sys
lr = json.load(open(sys.argv[1]))
rr = json.load(open(sys.argv[2]))
lp = json.load(open(sys.argv[3]))
...
print(json.dumps({
  ...
  "alienware_materialized_precision_dataset": True
}, indent=2))
PY

echo
echo "=== [7] success ==="
echo "[ok] two-box reverse bidirectional open proof green"
echo "out=$OUT_DIR"
