...
for path in paths:
    ...
    if x:
        print(str(x))
        raise SystemExit(0)
raise SystemExit("dataset id not found in runner tick")
PY
)"
...
LOCAL_RECENT_JSON="$(jget "$LOCAL_NODE_BASE/datanet/v1/local-jobs/recent?who=$ACCOUNT&limit=50" 20)"
...
if [ -z "$LOCAL_DATASET_ID" ] || [ -z "$LOCAL_SHA256" ] || [ -z "$PLAINTEXT" ]; then
  echo "[fail] missing dataset id, sha, or plaintext on Precision" >&2
  exit 1
fi
