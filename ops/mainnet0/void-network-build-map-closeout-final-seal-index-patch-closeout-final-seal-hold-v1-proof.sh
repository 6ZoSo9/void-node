...
MARKER="VOID_NETWORK_BUILD_MAP_CLOSEOUT_FINAL_SEAL_INDEX_PATCH_CLOSEOUT_FINAL_SEAL_HOLD_V1"
...
DOC="docs/public-node/void-network-build-map-closeout-final-seal-index-patch-closeout-final-seal-hold-v1.md"
...
SOURCE_PROOF="ops/mainnet0/void-network-build-map-closeout-final-seal-index-patch-closeout-audit-rollup-hold-v1-proof.sh"
PUBLIC_JSON="public/public-node/void-network/build-map-closeout-final-seal-index-patch-closeout-final-seal-hold-v1.json"
PUBLIC_HTML="public/public-node/void-network/build-map-closeout-final-seal-index-patch-closeout-final-seal-hold-v1.html"

echo "== JSON parse / final seal binding =="
python3 - "$PUBLIC_JSON" "$INDEX_JSON" "$ROOT_PUBLIC_INDEX" "$CLOSEOUT_ROLLUP_JSON" "$INDEX_PATCH_JSON" "$FINAL_SEAL_JSON" "$CLOSEOUT_JSON" "$BUILD_MAP_JSON" "$MARKER" "$CLOSEOUT_ROLLUP_MARKER" "$INDEX_PATCH_MARKER" "$FINAL_SEAL_MARKER" "$CLOSEOUT_MARKER" "$BUILD_MAP_PROOF_MARKER" "$BUILD_MAP_FINAL_MARKER" <<'PY'
import json
import sys
...
source = final["source"]
assert source["closeout_rollup_marker"] == closeout_rollup_marker
...
print("void_network_build_map_closeout_final_seal_index_patch_closeout_final_seal_binding_green=true")
PY
...
grep -Fq "$CLOSEOUT_ROLLUP_MARKER" "$CLOSEOUT_ROLLUP_HTML"
...
if grep -RInE '<form|method=|fetch\(|XMLHttpRequest|navigator\.|localStorage|sessionStorage|indexedDB|onclick=|onload=|<script' "$PUBLIC_JSON" "$PUBLIC_HTML" "$CLOSEOUT_ROLLUP_JSON" "$CLOSEOUT_ROLLUP_HTML" "$INDEX_PATCH_JSON" "$INDEX_PATCH_HTML" "$FINAL_SEAL_JSON" "$FINAL_SEAL_HTML" "$CLOSEOUT_JSON" "$CLOSEOUT_HTML" "$BUILD_MAP_JSON" "$BUILD_MAP_HTML" "$INDEX_JSON"; then
...
if grep -RInE '100,000,000[[:space:]]+WC|100000000[[:space:]]+WC|WC[[:space:]]+supply[[:space:]]+cap|work[[:space:]]+credit[[:space:]]+supply[[:space:]]+cap|capped[[:space:]]+at[[:space:]]+100' "$DOC" "$INDEX_JSON" "$PUBLIC_JSON" "$PUBLIC_HTML" "$CLOSEOUT_ROLLUP_JSON" "$INDEX_PATCH_JSON" "$FINAL_SEAL_JSON" "$CLOSEOUT_JSON" "$BUILD_MAP_JSON"; then
...
echo "== result =="
echo "VOID_NETWORK_BUILD_MAP_CLOSEOUT_FINAL_SEAL_INDEX_PATCH_CLOSEOUT_FINAL_SEAL_HOLD_V1_GREEN"
