...
BRICK="datanet-wc-public-earn-loop-first-work-pack-wc-ledger-write-candidate-example-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_LEDGER_WRITE_CANDIDATE_EXAMPLE_HOLD_V1"
KEY="datanet_wc_public_earn_loop_first_work_pack_wc_ledger_write_candidate_example"
...
example_block = record["wc_ledger_write_candidate_example"]
need(example_block["example_candidate_packet_created"] is True, "example packet should be true")
...
need(example_block["example_amount_is_not_approved_here"] is True, "example amount must not be approved here")
need(example_block["example_amount_is_not_supply_limit"] is True, "example amount must not be supply limit")
need(example_block["ledger_write_candidate_instantiated_here"] is False, "candidate instantiated must be false")
...
need(example_block["operator_review_required"] is True, "operator review required")
need(example_block["runtime_ledger_endpoint_created"] is False, "runtime endpoint must be false")

packet = record["wc_ledger_write_candidate_example_packet"]
need(packet["candidate_ledger_entry"]["amount"] == 100, "packet amount should be 100")
need(packet["candidate_ledger_entry"]["accounting_unit"] == "WC", "packet unit should be WC")
need(packet["candidate_ledger_entry"]["amount_is_not_approved_here"] is True, "packet amount not approved")
need(packet["candidate_ledger_entry"]["amount_is_not_supply_limit"] is True, "packet amount not supply limit")
...
need(ledger["ledger_write_candidate_example_only"] is True, "example-only must be true")
...
print("wc_ledger_write_candidate_example_binding_green=true")
PY
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_LEDGER_WRITE_CANDIDATE_EXAMPLE_HOLD_V1_GREEN"
