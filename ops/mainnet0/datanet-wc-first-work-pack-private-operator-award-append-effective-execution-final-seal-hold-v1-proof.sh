...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-effective-execution-final-seal-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_FINAL_SEAL_HOLD_V1"
...
EFFECTIVE_EXECUTION_EXAMPLE_MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_EXAMPLE_HOLD_V1"
...
echo "== JSON parse / private effective execution final seal binding =="
...
assert(record.seal_type === "effective_execution_final_seal", "seal type mismatch");
...
assert(record.chain.private_effective_execution_candidate_marker === "$EFFECTIVE_EXECUTION_CANDIDATE_MARKER", "effective execution candidate binding mismatch");
assert(record.chain.private_effective_execution_example_marker === "$EFFECTIVE_EXECUTION_EXAMPLE_MARKER", "effective execution example binding mismatch");
...
assert(record.final_seal.effective_execution_closeout_bound === true, "effective execution closeout seal mismatch");
assert(record.final_seal.candidate_bound === true, "candidate seal mismatch");
assert(record.final_seal.example_bound === true, "example seal mismatch");
assert(record.final_seal.seal_state === "final_seal_hold_for_review", "seal state mismatch");
...
console.log("private_operator_effective_execution_final_seal_binding_green=true");
NODE

echo "== forbidden public mutation scan =="
if git grep -n "$MARKER" -- public >/tmp/void-private-effective-execution-final-seal-public-leak.txt 2>/dev/null; then
  cat /tmp/void-private-effective-execution-final-seal-public-leak.txt
  echo "private_effective_execution_final_seal_marker_not_in_public_tree_green=false"
  exit 1
fi
rm -f /tmp/void-private-effective-execution-final-seal-public-leak.txt
echo "private_effective_execution_final_seal_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_FINAL_SEAL_HOLD_V1_GREEN"
