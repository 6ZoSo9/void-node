...
DOC="docs/work-credits/void-datanet-wc-availability-ledger-write-preflight-hold-v1.md"
SCHEMA="fixtures/work-credits/void-datanet-wc-availability-ledger-write-preflight-hold-schema-v1.json"
EXAMPLE="fixtures/work-credits/void-datanet-wc-availability-ledger-write-preflight-hold-example-v1.json"
MARKER="VOID_DATANET_WC_AVAILABILITY_LEDGER_WRITE_PREFLIGHT_HOLD_V1"

fail() {
  echo "void_datanet_wc_availability_ledger_write_preflight_hold_v1_proof=FAIL reason=$1"
  exit 1
}
...
grep -Fq "Ledger-write preflight/hold only; no WC issuance and no WC ledger write." "$DOC" || fail "missing_status"
grep -Fq "This artifact does not issue Work Credits, write the WC ledger, create a ledger line, append to a ledger file, or approve execution." "$DOC" || fail "missing_no_mutation_clause"
grep -Fq "A later operator approval packet would still be required before any ledger write can execute." "$DOC" || fail "missing_later_operator_boundary"
...
assert schema["status"] == "ledger_write_preflight_hold_only_no_wc_issuance_no_wc_ledger_write"
...
print("example_ledger_write_preflight_hold_green=true")
PY

if grep -R "issues_work_credits.*true\|writes_wc_ledger.*true\|creates_ledger_line.*true\|appends_to_ledger_file.*true\|allocates_void.*true\|transfers_void.*true\|automatic_reward.*true\|approves_ledger_write.*true\|executes_ledger_write.*true\|activates_public_mutation.*true\|grants_signer_wallet_access.*true\|authorizes_execution.*true\|moves_funds.*true" "$SCHEMA" "$EXAMPLE" "$DOC" >/tmp/void_datanet_wc_ledger_write_preflight_hold_forbidden_true.txt; then
  cat /tmp/void_datanet_wc_ledger_write_preflight_hold_forbidden_true.txt
  fail "forbidden_true_flag"
fi

echo "void_datanet_wc_availability_ledger_write_preflight_hold_v1_proof=GREEN marker=$MARKER"
