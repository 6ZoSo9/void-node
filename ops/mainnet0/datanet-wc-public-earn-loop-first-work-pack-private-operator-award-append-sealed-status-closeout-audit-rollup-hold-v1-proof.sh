...
BRICK="datanet-wc-public-earn-loop-first-work-pack-private-operator-award-append-sealed-status-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_STATUS_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
PUBLIC_JSON="public/public-node/work-credits/datanet-wc-first-work-pack-private-operator-award-append-sealed-status-closeout-audit-rollup-hold-v1.json"
PUBLIC_HTML="public/public-node/work-credits/datanet-wc-first-work-pack-private-operator-award-append-sealed-status-closeout-audit-rollup-hold-v1.html"

echo "== JSON parse / public award append sealed status closeout binding =="
...
assert(record.closeout_type === "award_append_private_operator_sealed_status_closeout_audit_rollup", "closeout type mismatch");
...
assert(record.closeout.sealed_status_rollup_bound === true, "sealed status closeout binding mismatch");
assert(record.closeout.operator_side_award_append_chain === "sealed_for_review", "operator chain status mismatch");
assert(record.closeout.operator_side_index === "sealed", "operator index status mismatch");
assert(record.closeout.public_disclosure_level === "public_safe_no_private_markers", "disclosure level mismatch");
assert(record.closeout.closeout_state === "sealed_status_closed_for_public_review_hold", "closeout state mismatch");
assert(record.closeout.execution_state === "not_executed_by_this_public_closeout", "execution state mismatch");
assert(record.closeout.ledger_append_state === "not_appended_by_this_public_closeout", "ledger append state mismatch");
...
assert(record.closeout.operator_review_required === true, "operator review required mismatch");
...
console.log("public_award_append_sealed_status_closeout_binding_green=true");
NODE
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_STATUS_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
