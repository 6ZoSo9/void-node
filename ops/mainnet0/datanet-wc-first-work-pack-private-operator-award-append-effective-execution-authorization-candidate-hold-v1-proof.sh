...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-effective-execution-authorization-candidate-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_AUTHORIZATION_CANDIDATE_HOLD_V1"
...
EFFECTIVE_PREFLIGHT_MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_PREFLIGHT_HOLD_V1"
EFFECTIVE_PREFLIGHT_EXAMPLE_MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_PREFLIGHT_EXAMPLE_HOLD_V1"
...
echo "== JSON parse / private effective execution authorization candidate binding =="
python3 - <<PY
import json
...
marker = "$MARKER"
required_markers = [
    "$EFFECTIVE_PREFLIGHT_CLOSEOUT_MARKER",
    "$EFFECTIVE_PREFLIGHT_MARKER",
    "$EFFECTIVE_PREFLIGHT_EXAMPLE_MARKER",
...
candidate = record["private_operator_effective_execution_authorization_candidate"]
need(candidate["authorization_candidate_shape_created"] is True, "candidate shape")
need(candidate["authorization_candidate_is_not_authorization"] is True, "candidate not authorization")
need(candidate["private_operator_only"] is True, "private")
need(candidate["requires_private_effective_execution_preflight_closeout"] is True, "preflight closeout required")
need(candidate["requires_private_effective_execution_preflight"] is True, "preflight required")
need(candidate["requires_private_effective_execution_preflight_example"] is True, "preflight example required")
need(candidate["requires_private_final_execution_closeout"] is True, "final closeout required")
need(candidate["requires_private_ledger_append_closeout"] is True, "append closeout required")
need(candidate["requires_private_ledger_line_closeout"] is True, "line closeout required")
need(candidate["requires_private_ledger_write_authorization_closeout"] is True, "auth closeout required")
need(candidate["requires_future_effective_execution_authorization"] is True, "future authorization required")
need(candidate["requires_future_effective_execution"] is True, "future execution required")
need(candidate["requires_future_effective_ledger_append"] is True, "future append required")
need(candidate["requires_future_effective_ledger_line_creation"] is True, "future line required")
need(candidate["requires_future_effective_ledger_write_authorization"] is True, "future ledger auth required")
need(candidate["requires_operator_final_review"] is True, "operator review required")
need(candidate["example_amount_wc"] == 100, "example amount")
need(candidate["example_amount_is_not_approved_here"] is True, "amount not approved")
need(candidate["example_amount_is_not_supply_limit"] is True, "amount not supply limit")
...
for key in [
    ...
    need(candidate[key] is False, f"candidate false expected: {key}")

template = record["effective_execution_authorization_candidate_template"]
need(template["authorization_candidate_shape_created_here"] is True, "template shape")
need(template["authorization_candidate_is_not_authorization"] is True, "template not authorization")
for key in [
    "effective_execution_authorized_here",
    "effective_execution_performed_here",
    "effective_ledger_append_performed_here",
    ...
    need(template[key] is False, f"template false expected: {key}")
...
need(execution["effective_execution_authorization_candidate_shape_only"] is True, "authorization candidate only")
for key in [
...
print("private_operator_effective_execution_authorization_candidate_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_effective_execution_authorization_candidate_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_effective_execution_authorization_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_AUTHORIZATION_CANDIDATE_HOLD_V1_GREEN"
