...
BRICK="datanet-wc-held-redacted-settlement-receipt-candidate-hold-v1"
...
assert entry["scope"] == "datanet_held_redacted_settlement_receipt_candidate"
assert entry["redacted_settlement_receipt_candidate_only"] is True
assert entry["settlement_receipt_created"] is False
assert entry["settlement_receipt_published"] is False
assert entry["worker_receipt_created"] is False
assert entry["public_receipt_created"] is False
...
assert card["schema"] == "void.public_node.work_credits.held_redacted_settlement_receipt_candidate.v1"
assert card["id"] == brick
assert card["status"] == "hold"
assert card["marker"] == marker
assert card["redacted_settlement_receipt_candidate_only"] is True
assert card["candidate_amounts"]["wc_amount"] == 0
assert card["candidate_amounts"]["void_amount"] == 0

state = card["receipt_candidate_state"]
...
assert state["receipt_status"] == "not_created_hold"
...
assert redaction["redacted_public_receipt_only"] is True
...
assert source["transfer_execution_result_candidate"] == "/public-node/work-credits/datanet-wc-held-transfer-execution-result-candidate-hold-v1.json"

print("held_redacted_settlement_receipt_candidate_binding_green=true")
PY
...
paths = [
    ...
    Path("public/public-node/work-credits/datanet-wc-held-redacted-settlement-receipt-candidate-hold-v1.json"),
    Path("docs/public-node/work-credits/datanet-wc-held-redacted-settlement-receipt-candidate-hold-v1.md"),
]
...
echo "== result =="
echo "VOID_DATANET_WC_HELD_REDACTED_SETTLEMENT_RECEIPT_CANDIDATE_HOLD_V1_GREEN"
