...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-manual-reviewer-share-intent-record-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_manual_reviewer_share_intent_record_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_MANUAL_REVIEWER_SHARE_INTENT_RECORD_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_MANUAL_REVIEWER_SHARE_INTENT_RECORD_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=8022eedf \
  manual_reviewer_share_readiness_packet_cross_box_green=true \
  manual_reviewer_share_intent_recorded_now=true \
  manual_operator_intent_recorded_now=true \
...
echo "${pfx}manual_reviewer_share_intent_recorded_now=true"
...
echo "${pfx}proof_scope=tiny_static_doc_only_intent_record_no_contact_no_packet_share_no_approval_no_mutation_no_ledger_write_no_wc_award"
echo "${u}_MANUAL_REVIEWER_SHARE_INTENT_RECORD_PROOF_V1_GREEN"
