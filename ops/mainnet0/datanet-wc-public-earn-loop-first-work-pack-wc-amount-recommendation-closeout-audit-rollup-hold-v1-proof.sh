...
BRICK="datanet-wc-public-earn-loop-first-work-pack-wc-amount-recommendation-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_AMOUNT_RECOMMENDATION_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
KEY="datanet_wc_public_earn_loop_first_work_pack_wc_amount_recommendation_closeout_audit_rollup"
...
need(closeout["amount_recommendation_candidate_exists"] is True, "candidate should exist")
need(closeout["amount_recommendation_example_exists"] is True, "example should exist")
need(closeout["requires_accepted_decision"] is True, "accepted decision requirement must be true")
need(closeout["example_recommended_amount_wc"] == 100, "example amount should be 100")
need(closeout["example_amount_is_not_supply_limit"] is True, "example amount must not be supply limit")
need(closeout["amount_recommendation_fixture_created"] is False, "recommendation fixture must be false")
...
need(closeout["ledger_write_candidate_created"] is False, "ledger candidate must be false")
need(closeout["ledger_append_enabled"] is False, "ledger append must be false")
need(closeout["void_transfer_created"] is False, "VOID transfer must be false")
need(closeout["runtime_recommendation_endpoint_created"] is False, "runtime endpoint must be false")

amount = record["amount_boundary"]
need(amount["amount_recommendation_closeout_only"] is True, "amount closeout only must be true")
...
need(amount["wc_issuance_created"] is False, "WC issuance must be false")
need(amount["wc_ledger_write_created"] is False, "WC ledger write must be false")
need(amount["ledger_write_candidate_created"] is False, "ledger candidate must be false")
need(amount["ledger_append_enabled"] is False, "ledger append must be false")
need(amount["void_transfer_created"] is False, "VOID transfer must be false")
...
print("wc_amount_recommendation_closeout_binding_green=true")
PY
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_AMOUNT_RECOMMENDATION_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
