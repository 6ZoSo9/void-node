...
BRICK="datanet-wc-public-earn-loop-first-work-pack-wc-ledger-write-approval-candidate-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_LEDGER_WRITE_APPROVAL_CANDIDATE_HOLD_V1"
KEY="datanet_wc_public_earn_loop_first_work_pack_wc_ledger_write_approval_candidate"
...
candidate = record["wc_ledger_write_approval_candidate"]
need(candidate["approval_candidate_shape_created"] is True, "approval candidate shape should be true")
...
need(candidate["example_amount_is_not_approved_here"] is True, "example amount not approved here")
need(candidate["example_amount_is_not_supply_limit"] is True, "example amount not supply limit")
need(candidate["approval_decision_created_here"] is False, "approval decision must be false")
need(candidate["ledger_write_authorized_here"] is False, "ledger authorization must be false")
need(candidate["ledger_write_candidate_instantiated_here"] is False, "ledger candidate instantiated must be false")
...
need(approval["approval_candidate_shape_only"] is True, "approval shape-only must be true")
...
template = record["wc_ledger_write_approval_candidate_template"]
need(template["approval_candidate_shape_created_here"] is True, "template shape created should be true")
need(template["approval_decision_created_here"] is False, "template approval decision must be false")
need(template["ledger_write_authorized_here"] is False, "template ledger authorization must be false")
...
need(template["wc_ledger_write_created_here"] is False, "template WC ledger write must be false")
need(template["reward_created_here"] is False, "template reward must be false")
...
print("wc_ledger_write_approval_candidate_binding_green=true")
PY
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_LEDGER_WRITE_APPROVAL_CANDIDATE_HOLD_V1_GREEN"
