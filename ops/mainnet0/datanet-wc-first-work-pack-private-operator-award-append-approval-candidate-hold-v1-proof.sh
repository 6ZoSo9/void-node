...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-approval-candidate-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_APPROVAL_CANDIDATE_HOLD_V1"
...
echo "== JSON parse / private approval candidate binding =="
python3 - <<PY
import json
...
candidate = record["private_operator_approval_candidate"]
need(candidate["approval_candidate_shape_created"] is True, "approval candidate shape must be true")
need(candidate["private_operator_only"] is True, "private operator only must be true")
need(candidate["requires_private_decision_closeout"] is True, "private decision closeout required")
need(candidate["requires_private_preflight_closeout"] is True, "private preflight closeout required")
need(candidate["requires_future_effective_operator_approval"] is True, "future effective approval required")
need(candidate["requires_future_ledger_write_authorization"] is True, "future ledger authorization required")
need(candidate["requires_future_ledger_write_candidate"] is True, "future ledger candidate required")
...
need(candidate["ledger_write_authorized_here"] is False, "ledger authorization must be false")
...
need(candidate["reward_created_here"] is False, "reward created must be false")
...
template = record["approval_candidate_template"]
need(template["approval_candidate_shape_created_here"] is True, "template candidate shape created")
...
need(template["ledger_write_authorized_here"] is False, "template ledger authorization false")
...
need(approval["approval_candidate_shape_only"] is True, "approval candidate shape only must be true")
need(approval["effective_operator_approval_created"] is False, "effective approval must be false")
need(approval["operator_append_decision_created"] is False, "append decision must be false")
need(approval["award_created"] is False, "award created must be false")
need(approval["award_approved"] is False, "award approved must be false")
need(approval["ledger_write_authorized"] is False, "ledger authorized must be false")
...
need(approval["wc_issuance_created"] is False, "WC issuance must be false")
need(approval["wc_ledger_write_created"] is False, "WC ledger write must be false")
need(approval["reward_created"] is False, "reward must be false")
need(approval["void_transfer_created"] is False, "VOID transfer must be false")
...
print("private_operator_award_append_approval_candidate_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_approval_candidate_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_approval_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_APPROVAL_CANDIDATE_HOLD_V1_GREEN"
