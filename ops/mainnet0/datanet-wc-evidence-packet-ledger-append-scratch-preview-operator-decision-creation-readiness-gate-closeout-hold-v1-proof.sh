...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate-closeout"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_CLOSEOUT_HOLD_V1"

SRC_LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate"
SRC_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_HOLD_V1"
...
BAD_BOUNDARY="$(mktemp)"
BAD_AUTH="$(mktemp)"
trap 'rm -f "$OUT1" "$OUT2" "$DISPLAY_OUT" "$VALIDATOR" "$BAD_MARKER" "$BAD_HASH" "$BAD_BOUNDARY" "$BAD_AUTH"' EXIT
...
if (data.artifact.status !== 'read_only_operator_decision_creation_readiness_gate_closeout_ready') fail('status mismatch');
if (data.closeout_hash !== hashCanonical(data.artifact)) fail('closeout hash mismatch');

const closeout = data.artifact.readiness_gate_closeout ?? {};
...
echo "== operator decision creation readiness gate closeout deterministic / binding =="
echo "operator_decision_creation_readiness_gate_closeout_deterministic_green=true"
echo "operator_decision_creation_readiness_gate_closeout_binding_green=true"
...
const data = JSON.parse(readFileSync(process.argv[2], 'utf8'));
const field = process.argv[4];
data.artifact.readiness_gate_closeout[field] = true;
data.closeout_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE
