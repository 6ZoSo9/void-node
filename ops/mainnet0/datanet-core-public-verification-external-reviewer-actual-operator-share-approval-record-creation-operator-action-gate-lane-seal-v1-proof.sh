...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-creation-operator-action-gate-lane-seal-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_creation_operator_action_gate_lane_seal_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_CREATION_OPERATOR_ACTION_GATE_LANE_SEAL_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_CREATION_OPERATOR_ACTION_GATE_LANE_SEAL_DOC_V1" "$doc"
...
echo "${u}_CREATION_OPERATOR_ACTION_GATE_LANE_SEAL_PROOF_V1_GREEN"
