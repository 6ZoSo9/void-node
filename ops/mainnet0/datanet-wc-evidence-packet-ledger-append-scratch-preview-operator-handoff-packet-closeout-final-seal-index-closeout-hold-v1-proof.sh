...
SOURCE_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_PACKET_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1"
SOURCE_CLOSEOUT_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_PACKET_CLOSEOUT_HOLD_V1"
...
TOOL="tools/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-handoff-packet-closeout-final-seal-index-closeout.mjs"
DOC="docs/datanet/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-handoff-packet-closeout-final-seal-index-closeout-hold-v1.md"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-handoff-packet-closeout-final-seal-index-hold-v1-proof.sh"
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

INPUT="$TMP_DIR/operator-handoff-packet-closeout-final-seal-index.json"
OUT_A="$TMP_DIR/final-seal-index-closeout-a.json"
OUT_B="$TMP_DIR/final-seal-index-closeout-b.json"
...
echo "== operator handoff packet closeout final seal index source proof =="
bash "$PREV_PROOF"

echo "== create fixture operator handoff packet closeout final seal index =="
python3 - "$INPUT" "$SOURCE_MARKER" "$SOURCE_CLOSEOUT_MARKER" "$OPERATOR_HANDOFF_MARKER" "$TERMINAL_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_MARKER" "$TERMINAL_CLOSEOUT_FINAL_SEAL_INDEX_MARKER" "$TERMINAL_CLOSEOUT_MARKER" "$TERMINAL_ROLLUP_MARKER" "$PRIOR_FINAL_CLOSEOUT_MARKER" <<'PYFIX'
import hashlib, json, sys
out, source_marker, source_closeout_marker, handoff_marker, terminal_closeout_final_closeout_marker, terminal_closeout_final_marker, terminal_closeout_marker, terminal_rollup_marker, prior_final_closeout_marker = sys.argv[1:]
def h(label):
...
record = {
    ...
    "reason": "seal scratch preview operator handoff packet closeout for manual review only",
    "source_operator_handoff_packet_closeout": {
        "path": "/tmp/operator-handoff-packet-closeout.json",
        "marker": source_closeout_marker,
...
with open(out, "w", encoding="utf-8") as f:
    ...
    f.write("\n")
PYFIX
fixture_operator_handoff_packet_closeout_final_seal_index_green=true

echo "== operator handoff packet closeout final seal index closeout =="
VOID_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_PACKET_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$TOOL" \
  --operator-handoff-packet-closeout-final-seal-index "$INPUT" \
  --out "$OUT_A" \
  --closer "operator-handoff-closeout-final-seal-index-closeout" \
  --reason "close out scratch preview operator handoff packet closeout final seal index for manual review only" >/dev/null
VOID_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_PACKET_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$TOOL" \
  --operator-handoff-packet-closeout-final-seal-index "$INPUT" \
  --out "$OUT_B" \
  --closer "operator-handoff-closeout-final-seal-index-closeout" \
  --reason "close out scratch preview operator handoff packet closeout final seal index for manual review only" >/dev/null
cmp -s "$OUT_A" "$OUT_B"
echo "operator_handoff_packet_closeout_final_seal_index_closeout_deterministic_green=true"

echo "== operator handoff packet closeout final seal index closeout binding =="
...
assert data["status"] == "scratch_preview_operator_handoff_packet_closeout_final_seal_index_closed_for_operator_review"
assert len(data["scratch_preview_operator_handoff_packet_closeout_final_seal_index_closeout_id"]) == 64
assert data["source_final_seal_index"]["marker"] == source_marker
summary = data["closeout_summary"]
assert summary["operator_handoff_packet_closeout_final_seal_index_bound"] is True
assert summary["scratch_preview_operator_handoff_packet_closeout_final_seal_index_closeout_only"] is True
...
assert boundary["scratch_preview_operator_handoff_packet_closeout_final_seal_index_closeout_only"] is True
...
PYCHECK
echo "operator_handoff_packet_closeout_final_seal_index_closeout_binding_green=true"

echo "== bad final seal index rejection =="
...
with open(dst, "w", encoding="utf-8") as f:
    ...
    f.write("\n")
PYBAD
if VOID_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_PACKET_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$TOOL" --operator-handoff-packet-closeout-final-seal-index "$BAD_SOURCE" --out "$TMP_DIR/bad-out.json" --closer "operator" --reason "bad" >/dev/null 2>&1; then
  echo "bad_final_seal_index_rejection_failed"
  exit 1
fi
...
with open(dst, "w", encoding="utf-8") as f:
    ...
    f.write("\n")
PYMARK
if VOID_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_PACKET_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$TOOL" --operator-handoff-packet-closeout-final-seal-index "$BAD_MARKER" --out "$TMP_DIR/bad-marker-out.json" --closer "operator" --reason "bad" >/dev/null 2>&1; then
...
grep -qi "operator handoff packet closeout final seal index closeout" "$DOC"
