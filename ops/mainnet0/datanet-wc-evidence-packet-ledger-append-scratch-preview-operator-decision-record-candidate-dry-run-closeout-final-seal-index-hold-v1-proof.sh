...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-closeout-final-seal-index"
...
SOURCE_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CLOSEOUT_HOLD_V1"
TOOL="tools/${LANE}.mjs"
DOC="docs/datanet/${LANE}-hold-v1.md"
PROOF="ops/mainnet0/${LANE}-hold-v1-proof.sh"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-closeout-hold-v1-proof.sh"
...
printf '== operator decision record candidate dry-run closeout source proof ==\n'
bash "${PREV_PROOF}"

TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT
SOURCE="${TMP}/operator-decision-record-candidate-dry-run-closeout.json"
OUT_A="${TMP}/operator-decision-record-candidate-dry-run-closeout-final-seal-index-a.json"
OUT_B="${TMP}/operator-decision-record-candidate-dry-run-closeout-final-seal-index-b.json"
...
printf '== create fixture operator decision record candidate dry-run closeout ==\n'
cat > "${SOURCE}" <<JSON
{
  "marker": "${SOURCE_MARKER}",
  "status": "closed",
  "closeout_id": "operator-decision-record-candidate-dry-run-closeout-fixture-v1",
  ...
  "source": {
    ...
    "path_basename": "operator-decision-record-candidate-dry-run.json",
    "sha256": "747b108a6dd0e4bc960ff2eda993272cd387d823f69da163b6eb426e214d2d7d",
  ...
  "closeout": {
    "source_status": "candidate_dry_run_only",
    "source_candidate_outcome": "prepare_manual_operator_decision_record",
  ...
  "closeout_hash": "b6f0d3651be9d362b3461884f37815ed55577622fd6afbf64dfc554f0a1266de"
}
JSON

printf '== operator decision record candidate dry-run closeout final seal index ==\n'
node "${TOOL}" --source "${SOURCE}" --out "${OUT_A}" --sealed-at "2026-07-03T00:00:00.000Z" --index-id "operator-decision-record-candidate-dry-run-closeout-final-seal-index-fixture-v1"
node "${TOOL}" --source "${SOURCE}" --out "${OUT_B}" --sealed-at "2026-07-03T00:00:00.000Z" --index-id "operator-decision-record-candidate-dry-run-closeout-final-seal-index-fixture-v1"

printf '== final seal index deterministic / binding ==\n'
cmp "${OUT_A}" "${OUT_B}"
echo "operator_decision_record_candidate_dry_run_closeout_final_seal_index_deterministic_green=true"
...
const expectedMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1';
const expectedSourceMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CLOSEOUT_HOLD_V1';
...
assert(out.source?.candidate_outcome === source.source.candidate_outcome, 'candidate outcome binding mismatch');
assert(out.entries?.length === 1, 'expected one index entry');
assert(out.entries[0]?.kind === 'operator_decision_record_candidate_dry_run_closeout', 'entry kind mismatch');
assert(out.entries[0]?.closeout_hash === source.closeout_hash, 'entry closeout hash mismatch');
assert(out.entries[0]?.candidate_dry_run_hash === source.source.candidate_dry_run_hash, 'entry candidate hash mismatch');
assert(out.index_guardrails?.source_closeout_only === true, 'source closeout guard missing');
...
assert(out.boundary?.operator_decision_record_candidate_closeout_final_seal_index_only === true, 'candidate closeout final seal index boundary missing');
...
NODE_BINDING_EOF
echo "operator_decision_record_candidate_dry_run_closeout_final_seal_index_binding_green=true"

printf '== bad marker rejection ==\n'
cat > "${BAD}" <<JSON
{"marker":"VOID_BAD_MARKER"}
JSON
if node "${TOOL}" --source "${BAD}" --out "${BAD_OUT}" >/tmp/void-bad-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-bad-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log
...
data['closeout']['operator_decision_record_created'] = True
json.dump(data, open(out, 'w'), indent=2)
PY_CREATED_EOF
if node "${TOOL}" --source "${CREATED}" --out "${CREATED_OUT}" >/tmp/void-created-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-created-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log
...
src, out = sys.argv[1:3]
data = json.load(open(src))
data['closeout']['operator_decision_signed'] = True
json.dump(data, open(out, 'w'), indent=2)
PY_SIGNED_EOF
if node "${TOOL}" --source "${SIGNED}" --out "${SIGNED_OUT}" >/tmp/void-signed-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-signed-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log
...
src, out = sys.argv[1:3]
data = json.load(open(src))
data['closeout']['canonical_ledger_append_ready_for_execution'] = True
json.dump(data, open(out, 'w'), indent=2)
PY_EXEC_READY_EOF
if node "${TOOL}" --source "${EXEC_READY}" --out "${EXEC_READY_OUT}" >/tmp/void-exec-ready-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-exec-ready-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log
...
src, out = sys.argv[1:3]
data = json.load(open(src))
data['closeout']['approval_execution_authorized'] = True
json.dump(data, open(out, 'w'), indent=2)
PY_AUTH_EXEC_EOF
if node "${TOOL}" --source "${AUTH_EXEC}" --out "${AUTH_EXEC_OUT}" >/tmp/void-auth-exec-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-auth-exec-operator-decision-record-candidate-dry-run-closeout-final-seal-index.log
