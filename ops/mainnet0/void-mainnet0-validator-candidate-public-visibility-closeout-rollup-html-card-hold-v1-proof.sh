...
echo "== source rollup presence =="
grep -F "$ROLLUP_MARKER" "$ROLLUP_JSON" >/dev/null
grep -F '"closeout_rollup_only": true' "$ROLLUP_JSON" >/dev/null
grep -F '"public_submit_enabled": false' "$ROLLUP_JSON" >/dev/null
grep -F '"active_admission_enabled": false' "$ROLLUP_JSON" >/dev/null
echo "source_rollup_green=true"
...
assert card["source_closeout_rollup_route"] == rollup_route
assert card["source_closeout_rollup_marker"] == rollup_marker
...
print("validator_candidate_public_visibility_closeout_rollup_html_binding_green=true")
PYCHECK
...
paths = [
    ...
    Path("public/public-node/validators/mainnet0-validator-candidate-public-visibility-closeout-rollup-html-card-hold-v1.json"),
