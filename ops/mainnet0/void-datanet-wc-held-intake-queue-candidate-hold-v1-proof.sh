...
BRICK="datanet-wc-held-intake-queue-candidate-hold-v1"
...
PROOF="ops/mainnet0/void-datanet-wc-held-intake-queue-candidate-hold-v1-proof.sh"
...
marker = "VOID_DATANET_WC_HELD_INTAKE_QUEUE_CANDIDATE_HOLD_V1"
entry_id = "datanet-wc-held-intake-queue-candidate-hold-v1"
...
assert entry["path"] == "/public-node/work-credits/datanet-wc-held-intake-queue-candidate-hold-v1.json"
assert entry["json"] == "datanet-wc-held-intake-queue-candidate-hold-v1.json"
assert entry["lane"] == "work_credits"
assert entry["scope"] == "datanet_held_intake_queue_candidate"
assert entry["queue_candidate_only"] is True
assert entry["live_intake_queue"] is False
...
card = json.loads(Path("public/public-node/work-credits/datanet-wc-held-intake-queue-candidate-hold-v1.json").read_text())
assert card["schema"] == "void.public_node.work_credits.datanet_wc_held_intake_queue_candidate_hold.v1"
assert card["id"] == entry_id
assert card["status"] == "hold"
assert card["marker"] == marker
assert card["candidate_status"] == "held_intake_queue_candidate_only"

shape = card["candidate_queue_shape"]
assert shape["queue_kind"] == "datanet_wc_held_intake_queue"
...
print("held_intake_queue_candidate_binding_green=true")
PY
...
files = [
    ...
    Path("public/public-node/work-credits/datanet-wc-held-intake-queue-candidate-hold-v1.json"),
    Path("docs/public-node/work-credits/datanet-wc-held-intake-queue-candidate-hold-v1.md"),
]
