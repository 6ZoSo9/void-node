...
MARKER = "VOID_REALMS_TRISCALE_BUILDING_HOST_READINESS_V1"
...
def main() -> int:
    ...
    triscale = inspect_text(
        ...
        [
            ...
            "client_material_cost_trusted",
        ],
    )
    luanti = inspect_text(
        repo / "integrations/luanti/void_realms_triscale_build/init.lua",
        [
    ...
    source_ready = all(
        item["all_observed"] for item in (parent_world, triscale, luanti)
    )
    ...
    result = {
        ...
        "version": 1,
        "status": (
            "source_contract_ready_for_preview_adapter_plan"
            if source_ready
            else "source_contract_incomplete"
        ),
        "hostname": socket.gethostname(),
        "repo_realpath": str(repo),
        "stacked_parent_commit": "3220ac6cd1493275c4677a4ce1aa26e0a439ecbd",
        "stacked_parent_pr": 948,
        "luanti_runtime_present": runtime_present,
        "luanti_binaries": luanti_bins,
        "boundaries": {
            "single_world_parent": parent_world,
            "triscale_contract": triscale,
