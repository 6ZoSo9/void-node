...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-effective-execution-preflight-hold-v1"
...
echo "== JSON parse / private effective execution preflight binding =="
python3 - <<PY
import json
...
preflight = record["private_operator_effective_execution_preflight"]
need(preflight["effective_execution_preflight_shape_created"] is True, "preflight shape")
...
need(preflight["requires_future_effective_execution"] is True, "future execution required")
need(preflight["requires_future_effective_ledger_append"] is True, "future append required")
need(preflight["requires_future_effective_ledger_line_creation"] is True, "future line required")
need(preflight["requires_future_effective_ledger_write_authorization"] is True, "future auth required")
need(preflight["requires_operator_final_review"] is True, "operator final review")
...
template = record["effective_execution_preflight_template"]
need(template["effective_execution_preflight_shape_created_here"] is True, "template shape")
for key in [
    "effective_execution_authorized_here",
    "effective_execution_performed_here",
    "effective_ledger_append_performed_here",
...
need(execution["effective_execution_preflight_shape_only"] is True, "preflight only")
for key in [
...
print("private_operator_effective_execution_preflight_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_effective_execution_preflight_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_effective_execution_preflight_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_PREFLIGHT_HOLD_V1_GREEN"
