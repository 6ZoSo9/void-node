...
PREV_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_TERMINAL_ROLLUP_CLOSEOUT_HOLD_V1"
...
TOOL="tools/datanet-wc-evidence-packet-ledger-append-scratch-preview-terminal-rollup-closeout-final-seal-index.mjs"
DOC="docs/datanet/datanet-wc-evidence-packet-ledger-append-scratch-preview-terminal-rollup-closeout-final-seal-index-hold-v1.md"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-terminal-rollup-closeout-hold-v1-proof.sh"
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

INPUT="$TMP_DIR/terminal-rollup-closeout.json"
OUT_A="$TMP_DIR/terminal-rollup-closeout-final-seal-index-a.json"
OUT_B="$TMP_DIR/terminal-rollup-closeout-final-seal-index-b.json"
...
printf '== terminal rollup closeout source proof ==\n'
bash "$PREV_PROOF"

printf '== create fixture terminal rollup closeout ==\n'
python3 - "$INPUT" "$PREV_MARKER" "$TERMINAL_ROLLUP_MARKER" "$FINAL_SEAL_INDEX_CLOSEOUT_MARKER" "$FINAL_SEAL_INDEX_MARKER" "$REVIEW_INDEX_CLOSEOUT_MARKER" "$REVIEW_INDEX_MARKER" "$RECEIPT_CLOSEOUT_MARKER" "$RECEIPT_MARKER" "$SCRATCH_APPLY_MARKER" "$EXECUTE_PACKET_MARKER" "$DRY_RUN_MARKER" "$LEDGER_PACKET_MARKER" <<'PYFIXTURE'
import json, sys
(out, prev_marker, terminal_marker, final_close_marker, final_marker, review_close_marker, review_marker, receipt_close_marker, receipt_marker, scratch_marker, execute_marker, dry_marker, ledger_marker) = sys.argv[1:]
hexes = {
    "terminal_rollup_closeout_id": "0" * 64,
    "terminal_rollup_id": "a" * 64,
    "final_seal_index_closeout_id": "b" * 64,
    "final_seal_index_id": "c" * 64,
    "review_index_closeout_id": "d" * 64,
    "review_index_id": "e" * 64,
    "receipt_closeout_id": "f" * 64,
    "receipt_id": "1" * 64,
    "scratch_apply_id": "2" * 64,
    "current_scratch_ledger_hash": "3" * 64,
    "scratch_ledger_out_hash": "4" * 64,
    "appended_line_hash": "5" * 64,
    "logical_candidate_next_ledger_hash": "6" * 64,
    "execute_packet_id": "7" * 64,
    "dry_run_id": "8" * 64,
    "packet_id": "9" * 64,
    "evidence_hash": "a" * 64,
}
...
print("fixture_terminal_rollup_closeout_green=true")
PYFIXTURE

printf '== terminal rollup closeout final seal index ==\n'
VOID_LEDGER_APPEND_SCRATCH_PREVIEW_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_CREATED_AT="$CREATED_AT" node "$TOOL" \
  --terminal-rollup-closeout "$INPUT" \
  --out "$OUT_A" \
  --indexer "operator-terminal-rollup-closeout-final-seal-index" \
  --reason "fixture final seal index binds scratch preview terminal rollup closeout only" >/tmp/void-scratch-preview-terminal-rollup-closeout-final-seal-index-a.out

VOID_LEDGER_APPEND_SCRATCH_PREVIEW_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_CREATED_AT="$CREATED_AT" node "$TOOL" \
  --terminal-rollup-closeout "$INPUT" \
  --out "$OUT_B" \
  --indexer "operator-terminal-rollup-closeout-final-seal-index" \
  --reason "fixture final seal index binds scratch preview terminal rollup closeout only" >/tmp/void-scratch-preview-terminal-rollup-closeout-final-seal-index-b.out

printf '== terminal rollup closeout final seal index deterministic / binding ==\n'
...
assert record["schema"] == "void.datanet.wc.evidence_packet_ledger_append_scratch_preview_terminal_rollup_closeout_final_seal_index.v1"
assert record["marker"] == marker
assert record["status"] == "scratch_preview_terminal_rollup_closeout_final_seal_indexed"
assert re.fullmatch(r"[0-9a-f]{64}", record["scratch_preview_terminal_rollup_closeout_final_seal_index_id"])
assert record["indexer"] == "operator-terminal-rollup-closeout-final-seal-index"
assert record["terminal_rollup_closeout"]["path"] == str(pathlib.Path(input_path).resolve())
assert record["terminal_rollup_closeout"]["marker"] == prev_marker
assert record["terminal_rollup_closeout"]["scratch_preview_terminal_rollup_closeout_id"] == source["scratch_preview_terminal_rollup_closeout_id"]
...
assert summary["terminal_rollup_closeout_bound"] is True
assert summary["scratch_preview_chain_fully_closed_rolled_up_closed_and_final_seal_indexed_for_operator_review"] is True
assert summary["terminal_rollup_closeout_final_seal_index_only"] is True
...
assert boundary["scratch_preview_terminal_rollup_closeout_final_seal_index_only"] is True
...
print("scratch_preview_terminal_rollup_closeout_final_seal_index_deterministic_green=true")
print("scratch_preview_terminal_rollup_closeout_final_seal_index_binding_green=true")
PYCHECK

printf '== bad terminal rollup closeout rejection ==\n'
...
PYBADSTATUS
if node "$TOOL" \
  --terminal-rollup-closeout "$BAD_STATUS" \
  --out "$TMP_DIR/bad-status-final-seal-index.json" \
  --indexer "operator-terminal-rollup-closeout-final-seal-index" \
  --reason "bad terminal rollup closeout should fail" >/tmp/void-terminal-rollup-closeout-final-seal-index-bad-status-out 2>/tmp/void-terminal-rollup-closeout-final-seal-index-bad-status-err; then
  echo "bad_terminal_rollup_closeout_rejection_green=false"
  cat /tmp/void-terminal-rollup-closeout-final-seal-index-bad-status-out
  exit 1
fi
grep -Fq "terminal_rollup_closeout_status_mismatch" /tmp/void-terminal-rollup-closeout-final-seal-index-bad-status-err
echo "bad_terminal_rollup_closeout_rejection_green=true"
...
PYBADMARKER
if node "$TOOL" \
  --terminal-rollup-closeout "$BAD_MARKER" \
  --out "$TMP_DIR/bad-marker-final-seal-index.json" \
  --indexer "operator-terminal-rollup-closeout-final-seal-index" \
  --reason "bad marker should fail" >/tmp/void-terminal-rollup-closeout-final-seal-index-bad-marker-out 2>/tmp/void-terminal-rollup-closeout-final-seal-index-bad-marker-err; then
  echo "bad_marker_rejection_green=false"
  cat /tmp/void-terminal-rollup-closeout-final-seal-index-bad-marker-out
  exit 1
fi
grep -Fq "terminal_rollup_closeout_marker_mismatch" /tmp/void-terminal-rollup-closeout-final-seal-index-bad-marker-err
...
grep -Fq "$MARKER" "$DOC"
grep -Fq "$MARKER" "$0"
grep -Fq "terminal rollup closeout final seal index" "$DOC"
grep -Fq "scratch_preview_terminal_rollup_closeout_final_seal_index" "$TOOL"
