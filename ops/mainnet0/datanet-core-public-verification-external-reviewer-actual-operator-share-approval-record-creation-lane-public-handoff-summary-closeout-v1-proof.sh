...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-creation-lane-public-handoff-summary-closeout-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_creation_lane_public_handoff_summary_closeout_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_CREATION_LANE_PUBLIC_HANDOFF_SUMMARY_CLOSEOUT_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_CREATION_LANE_PUBLIC_HANDOFF_SUMMARY_CLOSEOUT_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=2340eb4b \
  public_handoff_summary_cross_box_green=true \
  public_handoff_summary_closeout_recorded=true \
...
echo "${u}_CREATION_LANE_PUBLIC_HANDOFF_SUMMARY_CLOSEOUT_PROOF_V1_GREEN"
