...
BRICK="datanet-wc-public-earn-loop-first-work-pack-wc-ledger-write-approval-example-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_LEDGER_WRITE_APPROVAL_EXAMPLE_HOLD_V1"
KEY="datanet_wc_public_earn_loop_first_work_pack_wc_ledger_write_approval_example"
...
example_block = record["wc_ledger_write_approval_example"]
need(example_block["example_approval_packet_created"] is True, "example approval packet should be true")
need(example_block["requires_future_ledger_write_candidate"] is True, "future ledger candidate required")
need(example_block["requires_future_approved_wc_amount"] is True, "future approved amount required")
need(example_block["requires_operator_final_review"] is True, "operator final review required")
need(example_block["example_amount_wc"] == 100, "example amount should be 100")
need(example_block["example_amount_is_not_approved_here"] is True, "example amount not approved here")
need(example_block["example_amount_is_not_supply_limit"] is True, "example amount not supply limit")
need(example_block["approval_decision_created_here"] is False, "approval decision must be false")
need(example_block["ledger_write_authorized_here"] is False, "ledger authorization must be false")
need(example_block["ledger_write_candidate_instantiated_here"] is False, "ledger candidate instantiated must be false")
need(example_block["ledger_line_created_here"] is False, "ledger line must be false")
need(example_block["ledger_append_performed_here"] is False, "ledger append must be false")
...
need(example_block["reward_created_here"] is False, "reward must be false")
need(example_block["void_transfer_created_here"] is False, "VOID transfer must be false")
need(example_block["runtime_ledger_endpoint_created"] is False, "runtime endpoint must be false")

packet = record["wc_ledger_write_approval_example_packet"]
need(packet["example_only"] is True, "packet example-only must be true")
need(packet["example_amount_wc"] == 100, "packet amount should be 100")
need(packet["example_amount_is_not_approved_here"] is True, "packet amount not approved")
need(packet["example_amount_is_not_supply_limit"] is True, "packet amount not supply limit")
need(packet["approval_decision_created_here"] is False, "packet approval decision must be false")
need(packet["ledger_write_authorized_here"] is False, "packet ledger authorization must be false")
need(packet["ledger_write_candidate_instantiated_here"] is False, "packet candidate instantiated must be false")
need(packet["ledger_line_created_here"] is False, "packet ledger line must be false")
need(packet["ledger_append_performed_here"] is False, "packet ledger append must be false")
...
need(packet["reward_created_here"] is False, "packet reward must be false")
need(packet["void_transfer_created_here"] is False, "packet VOID transfer must be false")

approval = record["approval_boundary"]
need(approval["approval_example_only"] is True, "approval example-only must be true")
...
print("wc_ledger_write_approval_example_binding_green=true")
PY
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_WC_LEDGER_WRITE_APPROVAL_EXAMPLE_HOLD_V1_GREEN"
