...
DOC="docs/work-credits/datanet-wc-agent-attestation-envelope-schema-candidate-index-patch-closeout-audit-rollup-hold-v1.md"
...
PUBLIC_JSON="public/public-node/work-credits/datanet-wc-agent-attestation-envelope-schema-candidate-index-patch-closeout-audit-rollup-hold-v1.json"
PUBLIC_HTML="public/public-node/work-credits/datanet-wc-agent-attestation-envelope-schema-candidate-index-patch-closeout-audit-rollup-hold-v1.html"

echo "== JSON parse / closeout audit rollup binding =="
python3 - "$PUBLIC_JSON" "$INDEX_JSON" "$INDEX_PATCH_JSON" "$SCHEMA_JSON" "$EXAMPLE_JSON" "$SOURCE_JSON" "$MARKER" "$INDEX_PATCH_MARKER" "$SCHEMA_CANDIDATE_MARKER" <<'PY'
import json
import sys

public_path, index_path, index_patch_path, schema_path, example_path, source_path, marker, index_patch_marker, schema_candidate_marker = sys.argv[1:10]
...
assert closeout["source"]["index_patch_marker"] == index_patch_marker
assert closeout["source"]["schema_candidate_marker"] == schema_candidate_marker
...
print("agent_attestation_envelope_schema_candidate_index_patch_closeout_binding_green=true")
PY
...
if grep -RInE '<form|method=|fetch\(|XMLHttpRequest|navigator\.|localStorage|sessionStorage|indexedDB|onclick=|onload=|<script' "$PUBLIC_JSON" "$PUBLIC_HTML" "$INDEX_PATCH_JSON" "$INDEX_PATCH_HTML" "$SCHEMA_JSON" "$EXAMPLE_JSON" "$SOURCE_JSON" "$SOURCE_HTML"; then
...
echo "== result =="
echo "VOID_DATANET_WC_AGENT_ATTESTATION_ENVELOPE_SCHEMA_CANDIDATE_INDEX_PATCH_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
