...
BRICK="datanet-wc-first-work-pack-private-operator-ledger-line-closeout-audit-rollup-hold-v1"
...
echo "== JSON parse / private ledger-line closeout binding =="
python3 - <<PY
import json
...
need(closeout["private_ledger_line_candidate_exists"] is True, "ledger-line candidate should exist")
need(closeout["private_ledger_line_example_exists"] is True, "ledger-line example should exist")
...
need(closeout["example_ledger_line_is_not_effective"] is True, "example ledger line not effective")
need(closeout["example_ledger_line_is_not_created"] is True, "example ledger line not created")
need(closeout["example_ledger_line_is_not_appended"] is True, "example ledger line not appended")
...
need(closeout["requires_future_ledger_append"] is True, "future ledger append required")
...
need(ll["private_ledger_line_closeout_only"] is True, "private ledger-line closeout only must be true")
...
need(ll["award_created"] is False, "award must be false")
...
print("private_operator_ledger_line_closeout_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_ledger_line_closeout_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_ledger_line_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_LINE_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
