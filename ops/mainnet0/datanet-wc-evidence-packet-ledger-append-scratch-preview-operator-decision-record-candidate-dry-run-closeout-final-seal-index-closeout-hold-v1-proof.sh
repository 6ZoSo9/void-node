...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout"
...
SOURCE_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1"
TOOL="tools/${LANE}.mjs"
DOC="docs/datanet/${LANE}-hold-v1.md"
PROOF="ops/mainnet0/${LANE}-hold-v1-proof.sh"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-closeout-final-seal-index-hold-v1-proof.sh"
...
printf '== operator decision record candidate dry-run closeout final seal index source proof ==\n'
bash "${PREV_PROOF}"

TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT
SOURCE="${TMP}/operator-decision-record-candidate-dry-run-closeout-final-seal-index.json"
...
printf '== create fixture operator decision record candidate dry-run closeout final seal index ==\n'
cat > "${SOURCE}" <<JSON
{
  ...
  "source": {
    ...
    "path_basename": "operator-decision-record-candidate-dry-run-closeout.json",
    "sha256": "327361227a0a39a9f6da54b12bfd051443022627be897712a4b2588fca48cb6c",
...
printf '== operator decision record candidate dry-run closeout final seal index closeout ==\n'
node "${TOOL}" --source "${SOURCE}" --out "${OUT_A}" --closed-at "2026-07-03T00:00:00.000Z" --closeout-id "operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout-fixture-v1"
node "${TOOL}" --source "${SOURCE}" --out "${OUT_B}" --closed-at "2026-07-03T00:00:00.000Z" --closeout-id "operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout-fixture-v1"

printf '== final seal index closeout deterministic / binding ==\n'
cmp "${OUT_A}" "${OUT_B}"
echo "operator_decision_record_candidate_dry_run_closeout_final_seal_index_closeout_deterministic_green=true"
...
const expectedMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_HOLD_V1';
const expectedSourceMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1';
...
assert(out.boundary?.operator_decision_record_candidate_closeout_final_seal_index_closeout_only === true, 'candidate closeout final seal index closeout boundary missing');
...
NODE_BINDING_EOF
echo "operator_decision_record_candidate_dry_run_closeout_final_seal_index_closeout_binding_green=true"

printf '== bad marker rejection ==\n'
cat > "${BAD}" <<JSON
{"marker":"VOID_BAD_MARKER"}
JSON
if node "${TOOL}" --source "${BAD}" --out "${BAD_OUT}" >/tmp/void-bad-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log 2>&1; then
  cat /tmp/void-bad-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log
...
PY_CREATED_EOF
if node "${TOOL}" --source "${CREATED}" --out "${CREATED_OUT}" >/tmp/void-created-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log 2>&1; then
  cat /tmp/void-created-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log
...
json.dump(data, open(out, 'w'), indent=2)
PY_SIGNED_EOF
if node "${TOOL}" --source "${SIGNED}" --out "${SIGNED_OUT}" >/tmp/void-signed-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log 2>&1; then
  cat /tmp/void-signed-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log
...
json.dump(data, open(out, 'w'), indent=2)
PY_EXEC_READY_EOF
if node "${TOOL}" --source "${EXEC_READY}" --out "${EXEC_READY_OUT}" >/tmp/void-exec-ready-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log 2>&1; then
  cat /tmp/void-exec-ready-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log
...
json.dump(data, open(out, 'w'), indent=2)
PY_AUTH_EXEC_EOF
if node "${TOOL}" --source "${AUTH_EXEC}" --out "${AUTH_EXEC_OUT}" >/tmp/void-auth-exec-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log 2>&1; then
  cat /tmp/void-auth-exec-operator-decision-record-candidate-dry-run-closeout-final-seal-index-closeout.log
