...
grep -Fq "expected_route_count=15" ops/mainnet0/public-node-self-check-snapshot-proof.sh
grep -Fq "route_count=15" ops/mainnet0/public-node-route-manifest-proof.sh
...
ok(manifest.route_count === 15, "manifest route count 14");
...
ok(snap.expected_route_count === 15, "self-check route count 14");
...
echo "route_manifest_route_count=15"
echo "self_check_expected_route_count=15"
