...
DOC="docs/work-credits/void-datanet-wc-availability-operator-approval-hold-v1.md"
SCHEMA="fixtures/work-credits/void-datanet-wc-availability-operator-approval-hold-schema-v1.json"
EXAMPLE="fixtures/work-credits/void-datanet-wc-availability-operator-approval-hold-example-v1.json"
MARKER="VOID_DATANET_WC_AVAILABILITY_OPERATOR_APPROVAL_HOLD_V1"

fail() {
  echo "void_datanet_wc_availability_operator_approval_hold_v1_proof=FAIL reason=$1"
  exit 1
}
...
grep -Fq "Operator-approval hold only; no WC issuance and no WC ledger write." "$DOC" || fail "missing_status"
grep -Fq "This artifact does not approve a ledger write by itself." "$DOC" || fail "missing_no_approval_clause"
grep -Fq "A later operator approval packet would still be required before any ledger write packet can be executed." "$DOC" || fail "missing_later_operator_packet_boundary"
...
assert schema["status"] == "operator_approval_hold_only_no_wc_issuance_no_wc_ledger_write"
...
assert example["approval_status"] in schema["allowed_approval_status"]
...
print("example_operator_approval_hold_green=true")
PY

if grep -R "issues_work_credits.*true\|writes_wc_ledger.*true\|creates_ledger_line.*true\|allocates_void.*true\|transfers_void.*true\|automatic_reward.*true\|approves_ledger_write.*true\|creates_future_ledger_packet.*true\|activates_public_mutation.*true\|grants_signer_wallet_access.*true\|authorizes_execution.*true\|moves_funds.*true" "$SCHEMA" "$EXAMPLE" "$DOC" >/tmp/void_datanet_wc_operator_approval_hold_forbidden_true.txt; then
  cat /tmp/void_datanet_wc_operator_approval_hold_forbidden_true.txt
  fail "forbidden_true_flag"
fi

echo "void_datanet_wc_availability_operator_approval_hold_v1_proof=GREEN marker=$MARKER"
