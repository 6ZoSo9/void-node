...
BRICK="datanet-wc-public-earn-loop-first-work-pack-operator-intake-review-packet-example-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_OPERATOR_INTAKE_REVIEW_PACKET_EXAMPLE_HOLD_V1"
KEY="datanet_wc_public_earn_loop_first_work_pack_operator_intake_review_packet_example"

CANDIDATE_MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_OPERATOR_INTAKE_REVIEW_PACKET_CANDIDATE_HOLD_V1"
...
op = record["operator_intake_review_example"]
need(op["example_packet_created"] is True, "example packet should be true")
...
need(review["operator_review_packet_example_only"] is True, "example-only must be true")
...
outcome = record["operator_review_packet_example"]["review_outcome_candidate"]
need(outcome["decision_created_here"] is False, "example outcome decision must be false")
need(outcome["accepted"] is False, "accepted must be false")
need(outcome["rejected"] is False, "rejected must be false")
need(outcome["needs_more_info"] is False, "needs_more_info must be false")
need(outcome["suggested_wc_amount"] is None, "suggested WC amount must be null")
need(outcome["ledger_write_candidate_created_here"] is False, "ledger write candidate must be false")
...
print("operator_intake_review_packet_example_binding_green=true")
PY
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_OPERATOR_INTAKE_REVIEW_PACKET_EXAMPLE_HOLD_V1_GREEN"
