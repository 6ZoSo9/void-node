...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-effective-execution-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
echo "== JSON parse / private effective execution closeout binding =="
...
assert(record.closeout_type === "effective_execution_closeout_audit_rollup", "closeout type mismatch");
...
assert(record.closeout.effective_execution_status === "closed_for_review_hold", "effective execution closeout status mismatch");
...
console.log("private_operator_effective_execution_closeout_binding_green=true");
NODE

echo "== forbidden public mutation scan =="
if git grep -n "$MARKER" -- public >/tmp/void-private-effective-execution-closeout-public-leak.txt 2>/dev/null; then
  cat /tmp/void-private-effective-execution-closeout-public-leak.txt
  echo "private_effective_execution_closeout_marker_not_in_public_tree_green=false"
  exit 1
fi
rm -f /tmp/void-private-effective-execution-closeout-public-leak.txt
echo "private_effective_execution_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
