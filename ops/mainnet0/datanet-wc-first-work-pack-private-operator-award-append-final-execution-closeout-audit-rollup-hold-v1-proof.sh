...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-final-execution-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_FINAL_EXECUTION_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
echo "== JSON parse / private final execution closeout binding =="
python3 - <<PY
import json
...
need(record["visibility"] == "private_operator_only", "visibility must be private")
need(record["public_route_created"] is False, "public route false")

closeout = record["closeout_assertions"]
need(closeout["private_final_execution_candidate_exists"] is True, "candidate exists")
need(closeout["private_final_execution_example_exists"] is True, "example exists")
need(closeout["private_final_execution_pair_closed_out"] is True, "pair closed")
need(closeout["private_operator_only"] is True, "private operator")
...
need(closeout["example_execution_is_not_effective"] is True, "example not effective")
need(closeout["example_execution_is_not_performed"] is True, "example not performed")
need(closeout["example_append_is_not_performed"] is True, "example append not performed")
need(closeout["example_ledger_line_is_not_created"] is True, "example line not created")
...
need(closeout["requires_future_effective_final_execution"] is True, "future execution")
need(closeout["requires_future_effective_ledger_append"] is True, "future append")
need(closeout["requires_future_effective_ledger_line_creation"] is True, "future line")
need(closeout["requires_future_effective_ledger_write_authorization"] is True, "future auth")
need(closeout["requires_operator_final_review"] is True, "operator review")

for key in [
    "effective_final_execution_performed_here",
...
need(execution["final_execution_closeout_only"] is True, "closeout only")
for key in [
    "effective_final_execution_performed",
...
print("private_operator_final_execution_closeout_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_final_execution_closeout_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_final_execution_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_FINAL_EXECUTION_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
