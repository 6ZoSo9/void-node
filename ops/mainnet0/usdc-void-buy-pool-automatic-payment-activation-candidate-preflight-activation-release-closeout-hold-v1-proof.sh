...
n="usdc-void-buy-pool-automatic-payment-activation-candidate-preflight-activation-release-closeout-hold-v1"
marker="VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_CANDIDATE_PREFLIGHT_ACTIVATION_RELEASE_CLOSEOUT_HOLD_V1"
...
echo "activation_release_closeout_hold_files_and_marker_green=true"
...
assert j["id"] == n
assert j["status"] == "activation_candidate_preflight_activation_release_closeout_held"
assert j["scope"] == "private_operator_only_activation_release_closeout_hold_not_release_not_activation"
assert j["precision_source_of_truth"] is True
assert j["activation_release_gate_status"] == "activation_candidate_preflight_activation_release_gate_held"
assert j["next_required_gate"] == "activation_candidate_preflight_final_rollup_hold_v1"

gate = j["closeout_gate"]
assert gate["gate_state"] == "held_closed"
assert gate["closeout_record_written"] is False
assert gate["activation_release_closed_out"] is False
...
print("activation_release_closeout_hold_fixture_green=true")
print("activation_release_closeout_gate_held_closed_green=true")
print("activation_release_closeout_hold_boundary_green=true")
print("activation_release_closeout_hold_no_closeout_record=true")
print("activation_release_closeout_hold_no_activation_release_closeout=true")
print("activation_release_closeout_hold_no_activation_release=true")
print("activation_release_closeout_hold_no_activation_enablement=true")
PY

if grep -RInF 'git status --short' "$doc" "$fixture"; then
  echo "activation_release_closeout_hold_command_contamination=red"
  exit 1
fi

if grep -RInF 'zoso@' "$doc" "$fixture"; then
  echo "activation_release_closeout_hold_prompt_contamination=red"
  exit 1
fi

echo "activation_release_closeout_hold_contamination_absent=true"

grep -Fq "not activation release" "$doc"
grep -Fq "not activation closeout" "$doc"
grep -Fq "not activation enablement" "$doc"

echo "activation_release_closeout_hold_no_signing=true"
echo "activation_release_closeout_hold_no_signature=true"
echo "activation_release_closeout_hold_no_private_key_access=true"
echo "activation_release_closeout_hold_no_void_transfer=true"
echo "activation_release_closeout_hold_no_transaction_broadcast=true"
