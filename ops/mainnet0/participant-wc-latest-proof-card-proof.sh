...
OUT="/tmp/participant-wc-latest-proof-card-v1-proof-$STAMP"
mkdir -p "$OUT"

echo "=== Participant WC latest proof card v1 proof ==="
echo "base=$BASE"
echo "out=$OUT"
...
expect_grep "latest proof card id" "participantWcLatestProofCard" src/index.ts
expect_grep "latest proof title" "Latest Work Credit Proof" src/index.ts
expect_grep "latest proof status id" "participantWcLatestProofStatus" src/index.ts
expect_grep "latest proof meta id" "participantWcLatestProofMeta" src/index.ts
expect_grep "latest proof open link id" "participantWcLatestProofOpenLink" src/index.ts
expect_grep "latest proof raw link id" "participantWcLatestProofRawLink" src/index.ts
expect_grep "latest proof history link id" "participantWcLatestProofHistoryLink" src/index.ts
expect_grep "latest proof endpoint" "/wc-proofs/latest?limit=1" src/index.ts
expect_grep "participant generate card preserved" "participantWcProofGenerateCard" src/index.ts
expect_grep "participant history link preserved" "participantWcProofHistoryLink" src/index.ts
expect_grep "public proofs actions preserved" "VOID_WC_PROOFS_HISTORY_ACTIONS_V1" src/index.ts
echo "[ok] source markers/card"
...
echo
echo "=== [4] generate fresh proof for latest card source ==="
WHO="participant-latest-proof-card-$STAMP"
...
echo
echo "=== [5] latest endpoint source for card + generated proof discovery ==="
...
if not dataset:
    raise SystemExit("latest limit=1 missing dataset")
if not viewer:
    raise SystemExit("latest limit=1 missing viewer path")
if not raw:
    raise SystemExit("latest limit=1 missing raw path")
if not task:
    raise SystemExit("latest limit=1 missing task class")
...
LATESTJSONPY
echo "[ok] latest one parseable for participant latest card"
...
echo "[ok] latest endpoint is valid for card and generated proof is discoverable"

echo
echo "=== [6] participant renders latest proof card shell ==="
curl -fsS "$BASE/participant" > "$OUT/participant.html"
expect_grep "participant generate card" "participantWcProofGenerateCard" "$OUT/participant.html"
...
expect_grep "participant latest status" "participantWcLatestProofStatus" "$OUT/participant.html"
expect_grep "participant latest meta" "participantWcLatestProofMeta" "$OUT/participant.html"
...
expect_grep "participant latest history route" "href='/proofs'" "$OUT/participant.html"
expect_grep "participant proof history link preserved" "participantWcProofHistoryLink" "$OUT/participant.html"
expect_grep "participant proof history copy preserved" "View proof history" "$OUT/participant.html"
echo "[ok] participant renders latest proof card shell"
...
if timeout 45s make mainnet0-status-smoke; then
  ...
  echo "[warn] status smoke timed out/nonfatal for participant latest proof card after route/health checks"
...
echo "participant_latest_card_title=Latest Work Credit Proof"
echo "participant_latest_status_id=participantWcLatestProofStatus"
echo "participant_latest_meta_id=participantWcLatestProofMeta"
...
echo "buy_void_fulfillment=false"
echo "validator_mutation=false"
echo "VOID_WC_LATEST_PROOF_CARD_V1_GREEN"
