...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-handoff-terminal-rollup-closeout-final-seal-index-closeout"
...
SOURCE_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1"
TOOL="tools/${LANE}.mjs"
DOC="docs/datanet/${LANE}-hold-v1.md"
PROOF="ops/mainnet0/${LANE}-hold-v1-proof.sh"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-handoff-terminal-rollup-closeout-final-seal-index-hold-v1-proof.sh"
...
printf '== operator handoff terminal rollup closeout final seal index source proof ==\n'
bash "${PREV_PROOF}"

TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT
SOURCE="${TMP}/operator-handoff-terminal-rollup-closeout-final-seal-index.json"
...
printf '== create fixture operator handoff terminal rollup closeout final seal index ==\n'
cat > "${SOURCE}" <<JSON
{
  "marker": "${SOURCE_MARKER}",
  "status": "indexed",
  "final_seal_index_id": "operator-handoff-terminal-rollup-closeout-final-seal-index-fixture-v1",
  ...
  "checks": {
    ...
    "closeout_hash_bound_green": true,
...
printf '== operator handoff terminal rollup closeout final seal index closeout ==\n'
node "${TOOL}" --source "${SOURCE}" --out "${OUT_A}" --closed-at "2026-07-03T00:00:00.000Z" --closeout-id "operator-handoff-terminal-rollup-closeout-final-seal-index-closeout-fixture-v1"
node "${TOOL}" --source "${SOURCE}" --out "${OUT_B}" --closed-at "2026-07-03T00:00:00.000Z" --closeout-id "operator-handoff-terminal-rollup-closeout-final-seal-index-closeout-fixture-v1"

printf '== final seal index closeout deterministic / binding ==\n'
cmp "${OUT_A}" "${OUT_B}"
echo "operator_handoff_terminal_rollup_closeout_final_seal_index_closeout_deterministic_green=true"
...
const expectedMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_HOLD_V1';
const expectedSourceMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_HANDOFF_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1';
...
assert(out.checks?.final_seal_index_hash_bound_green === true, 'missing final seal index hash bound check');
...
NODE_BINDING_EOF
echo "operator_handoff_terminal_rollup_closeout_final_seal_index_closeout_binding_green=true"

printf '== bad marker rejection ==\n'
cat > "${BAD}" <<JSON
{"marker":"VOID_BAD_MARKER"}
JSON
if node "${TOOL}" --source "${BAD}" --out "${BAD_OUT}" >/tmp/void-bad-final-seal-index-closeout.log 2>&1; then
  cat /tmp/void-bad-final-seal-index-closeout.log
