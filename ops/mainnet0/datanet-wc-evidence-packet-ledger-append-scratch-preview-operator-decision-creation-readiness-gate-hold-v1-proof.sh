...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_HOLD_V1"

SRC_LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-terminal-closeout-index-closeout-final-seal-index-closeout"
SRC_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_TERMINAL_CLOSEOUT_INDEX_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_HOLD_V1"
...
if (data.artifact.status !== 'read_only_operator_decision_creation_readiness_gate_ready') fail('status mismatch');
if (data.readiness_gate_hash !== hashCanonical(data.artifact)) fail('readiness gate hash mismatch');

const gate = data.artifact.readiness_gate ?? {};
if (gate.dry_run_chain_terminally_closed !== true) fail('dry-run chain must be terminally closed');
if (gate.manual_operator_review_required !== true) fail('manual operator review must be required');
...
echo "== operator decision creation readiness gate deterministic / binding =="
echo "operator_decision_creation_readiness_gate_deterministic_green=true"
echo "operator_decision_creation_readiness_gate_binding_green=true"
...
data.readiness_gate_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE
...
const data = JSON.parse(readFileSync(process.argv[2], 'utf8'));
const field = process.argv[4];
data.artifact.readiness_gate[field] = true;
data.readiness_gate_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE
