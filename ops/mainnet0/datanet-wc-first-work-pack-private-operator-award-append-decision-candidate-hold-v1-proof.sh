...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-decision-candidate-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_DECISION_CANDIDATE_HOLD_V1"
...
echo "== JSON parse / private decision candidate binding =="
python3 - <<PY
import json
...
candidate = record["private_operator_decision_candidate"]
need(candidate["decision_candidate_shape_created"] is True, "decision candidate shape must be true")
...
need(candidate["requires_future_operator_decision"] is True, "future operator decision required")
need(candidate["requires_future_amount_approval"] is True, "future amount approval required")
...
need(decision["decision_candidate_shape_only"] is True, "decision candidate shape only must be true")
...
template = record["decision_candidate_template"]
need(template["decision_candidate_shape_created_here"] is True, "template shape created must be true")
...
print("private_operator_award_append_decision_candidate_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_decision_candidate_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_decision_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_DECISION_CANDIDATE_HOLD_V1_GREEN"
