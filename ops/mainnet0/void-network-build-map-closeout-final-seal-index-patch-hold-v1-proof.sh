...
MARKER="VOID_NETWORK_BUILD_MAP_CLOSEOUT_FINAL_SEAL_INDEX_PATCH_HOLD_V1"
SOURCE_MARKER="VOID_NETWORK_BUILD_MAP_CLOSEOUT_FINAL_SEAL_HOLD_V1"
...
DOC="docs/public-node/void-network-build-map-closeout-final-seal-index-patch-hold-v1.md"
...
SOURCE_JSON="public/public-node/void-network/build-map-closeout-final-seal-hold-v1.json"
SOURCE_HTML="public/public-node/void-network/build-map-closeout-final-seal-hold-v1.html"
...
SOURCE_PROOF="ops/mainnet0/void-network-build-map-closeout-final-seal-hold-v1-proof.sh"
PUBLIC_JSON="public/public-node/void-network/build-map-closeout-final-seal-index-patch-hold-v1.json"
PUBLIC_HTML="public/public-node/void-network/build-map-closeout-final-seal-index-patch-hold-v1.html"

echo "== JSON parse / final seal index patch binding =="
python3 - "$INDEX_JSON" "$ROOT_PUBLIC_INDEX" "$PUBLIC_JSON" "$SOURCE_JSON" "$CLOSEOUT_JSON" "$BUILD_MAP_JSON" "$MARKER" "$SOURCE_MARKER" "$CLOSEOUT_MARKER" "$BUILD_MAP_PROOF_MARKER" "$BUILD_MAP_FINAL_MARKER" <<'PY'
import json
import sys

index_path, root_path, patch_path, source_path, closeout_path, build_map_path, marker, source_marker, closeout_marker, proof_marker, build_final_marker = sys.argv[1:12]
...
print("void_network_build_map_closeout_final_seal_index_patch_binding_green=true")
PY
...
if grep -RInE '<form|method=|fetch\(|XMLHttpRequest|navigator\.|localStorage|sessionStorage|indexedDB|onclick=|onload=|<script' "$PUBLIC_JSON" "$PUBLIC_HTML" "$SOURCE_JSON" "$SOURCE_HTML" "$CLOSEOUT_JSON" "$CLOSEOUT_HTML" "$BUILD_MAP_JSON" "$BUILD_MAP_HTML" "$INDEX_JSON"; then
...
if grep -RInE '100,000,000[[:space:]]+WC|100000000[[:space:]]+WC|WC[[:space:]]+supply[[:space:]]+cap|work[[:space:]]+credit[[:space:]]+supply[[:space:]]+cap|capped[[:space:]]+at[[:space:]]+100' "$DOC" "$INDEX_JSON" "$PUBLIC_JSON" "$PUBLIC_HTML" "$SOURCE_JSON" "$CLOSEOUT_JSON" "$BUILD_MAP_JSON"; then
...
echo "== result =="
echo "VOID_NETWORK_BUILD_MAP_CLOSEOUT_FINAL_SEAL_INDEX_PATCH_HOLD_V1_GREEN"
