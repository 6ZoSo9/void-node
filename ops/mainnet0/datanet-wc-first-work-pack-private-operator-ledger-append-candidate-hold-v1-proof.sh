...
BRICK="datanet-wc-first-work-pack-private-operator-ledger-append-candidate-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_APPEND_CANDIDATE_HOLD_V1"
...
echo "== JSON parse / private ledger-append candidate binding =="
python3 - <<PY
import json
...
candidate = record["private_operator_ledger_append_candidate"]
need(candidate["ledger_append_candidate_shape_created"] is True, "ledger-append candidate shape must be true")
...
need(candidate["effective_ledger_append_performed_here"] is False, "effective ledger append must be false")
...
need(candidate["award_created_here"] is False, "award must be false")
...
template = record["ledger_append_candidate_template"]
need(template["ledger_append_candidate_shape_created_here"] is True, "template candidate shape created")
...
need(template["effective_ledger_write_authorization_created_here"] is False, "template ledger auth false")
...
need(template["award_created_here"] is False, "template award false")
...
need(template["candidate_append"]["amount"] == 100, "template amount must be 100")
need(template["candidate_append"]["amount_is_not_approved_here"] is True, "template amount not approved")
need(template["candidate_append"]["amount_is_not_supply_limit"] is True, "template amount not supply limit")
...
need(append["ledger_append_candidate_shape_only"] is True, "ledger-append candidate shape only must be true")
...
need(append["ledger_append_performed"] is False, "ledger append must be false")
...
print("private_operator_ledger_append_candidate_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_ledger_append_candidate_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_ledger_append_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_APPEND_CANDIDATE_HOLD_V1_GREEN"
