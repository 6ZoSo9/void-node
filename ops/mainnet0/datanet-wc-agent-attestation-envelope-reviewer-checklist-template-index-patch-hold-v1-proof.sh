...
MARKER="VOID_DATANET_WC_AGENT_ATTESTATION_ENVELOPE_REVIEWER_CHECKLIST_TEMPLATE_INDEX_PATCH_HOLD_V1"
SOURCE_MARKER="VOID_DATANET_WC_AGENT_ATTESTATION_ENVELOPE_REVIEWER_CHECKLIST_TEMPLATE_HOLD_V1"
...
DOC="docs/work-credits/datanet-wc-agent-attestation-envelope-reviewer-checklist-template-index-patch-hold-v1.md"
INDEX_JSON="public/public-node/work-credits/index.json"
CHECKLIST_JSON="public/public-node/work-credits/datanet-wc-agent-attestation-envelope-reviewer-checklist-template-hold-v1.json"
CHECKLIST_HTML="public/public-node/work-credits/datanet-wc-agent-attestation-envelope-reviewer-checklist-template-hold-v1.html"
...
PUBLIC_JSON="public/public-node/work-credits/datanet-wc-agent-attestation-envelope-reviewer-checklist-template-index-patch-hold-v1.json"
PUBLIC_HTML="public/public-node/work-credits/datanet-wc-agent-attestation-envelope-reviewer-checklist-template-index-patch-hold-v1.html"

echo "== JSON parse / reviewer checklist index patch binding =="
python3 - "$INDEX_JSON" "$PUBLIC_JSON" "$CHECKLIST_JSON" "$VALIDATION_FINAL_SEAL_JSON" "$VALIDATION_RECEIPT_JSON" "$SCHEMA_JSON" "$EXAMPLE_JSON" "$MARKER" "$SOURCE_MARKER" "$VALIDATION_FINAL_SEAL_MARKER" "$VALIDATION_RECEIPT_MARKER" "$SCHEMA_CANDIDATE_MARKER" <<'PY'
import json
import sys

index_path, patch_path, checklist_path, final_seal_path, validation_path, schema_path, example_path, marker, source_marker, final_seal_marker, validation_marker, schema_marker = sys.argv[1:13]
...
assert checklist["marker"] == source_marker
assert checklist["read_only"] is True
assert checklist["source"]["validation_final_seal_marker"] == final_seal_marker
assert checklist["source"]["validation_receipt_marker"] == validation_marker
assert checklist["source"]["schema_candidate_marker"] == schema_marker
...
for name in [
    "datanet-wc-agent-attestation-envelope-reviewer-checklist-template-hold-v1.json",
    "datanet-wc-agent-attestation-envelope-reviewer-checklist-template-hold-v1.html",
    "datanet-wc-agent-attestation-envelope-validation-receipt-index-patch-closeout-final-seal-hold-v1.json",
    ...
    "datanet-wc-agent-attestation-envelope-reviewer-checklist-template-index-patch-hold-v1.json",
    "datanet-wc-agent-attestation-envelope-reviewer-checklist-template-index-patch-hold-v1.html"
]:
...
print("agent_attestation_envelope_reviewer_checklist_template_index_patch_binding_green=true")
PY
...
grep -Fq "$SOURCE_MARKER" "$CHECKLIST_JSON"
grep -Fq "$SOURCE_MARKER" "$CHECKLIST_HTML"
grep -Fq "$VALIDATION_FINAL_SEAL_MARKER" "$INDEX_JSON"
...
if grep -RInE '<form|method=|fetch\(|XMLHttpRequest|navigator\.|localStorage|sessionStorage|indexedDB|onclick=|onload=|<script' "$PUBLIC_JSON" "$PUBLIC_HTML" "$CHECKLIST_JSON" "$CHECKLIST_HTML" "$VALIDATION_FINAL_SEAL_JSON" "$VALIDATION_RECEIPT_JSON" "$SCHEMA_JSON" "$EXAMPLE_JSON"; then
...
echo "== result =="
echo "VOID_DATANET_WC_AGENT_ATTESTATION_ENVELOPE_REVIEWER_CHECKLIST_TEMPLATE_INDEX_PATCH_HOLD_V1_GREEN"
