...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-final-execution-candidate-hold-v1"
...
echo "== JSON parse / private final execution candidate binding =="
python3 - <<PY
import json
...
candidate = record["private_operator_final_execution_candidate"]
need(candidate["final_execution_candidate_shape_created"] is True, "final execution candidate shape must be true")
need(candidate["private_operator_only"] is True, "private operator only must be true")
need(candidate["requires_private_ledger_append_closeout"] is True, "ledger append closeout required")
need(candidate["requires_private_ledger_line_closeout"] is True, "ledger line closeout required")
...
need(candidate["requires_future_effective_final_execution"] is True, "future final execution required")
need(candidate["requires_future_effective_ledger_append"] is True, "future ledger append required")
...
need(candidate["effective_final_execution_performed_here"] is False, "effective execution false")
need(candidate["effective_ledger_append_performed_here"] is False, "effective append false")
need(candidate["effective_ledger_line_created_here"] is False, "effective ledger line false")
need(candidate["effective_ledger_write_authorization_created_here"] is False, "effective ledger auth false")
need(candidate["effective_operator_approval_created_here"] is False, "effective approval false")
need(candidate["operator_append_decision_created_here"] is False, "append decision false")
need(candidate["award_created_here"] is False, "award false")
need(candidate["award_approved_here"] is False, "award approved false")
need(candidate["ledger_write_authorized_here"] is False, "ledger auth false")
need(candidate["ledger_line_created_here"] is False, "ledger line false")
need(candidate["ledger_append_performed_here"] is False, "ledger append false")
need(candidate["ledger_mutation_performed_here"] is False, "ledger mutation false")
need(candidate["wc_issuance_created_here"] is False, "WC issuance false")
need(candidate["wc_ledger_write_created_here"] is False, "WC ledger write false")
need(candidate["reward_created_here"] is False, "reward false")
need(candidate["void_transfer_created_here"] is False, "VOID transfer false")
need(candidate["runtime_append_endpoint_created"] is False, "runtime endpoint false")

template = record["final_execution_candidate_template"]
need(template["final_execution_candidate_shape_created_here"] is True, "template candidate shape")
need(template["effective_final_execution_performed_here"] is False, "template execution false")
need(template["effective_ledger_append_performed_here"] is False, "template append false")
need(template["effective_ledger_line_created_here"] is False, "template ledger line false")
need(template["ledger_mutation_performed_here"] is False, "template ledger mutation false")
...
need(template["candidate_execution"]["amount"] == 100, "template amount 100")
need(template["candidate_execution"]["amount_is_not_approved_here"] is True, "template amount not approved")
need(template["candidate_execution"]["amount_is_not_supply_limit"] is True, "template amount not supply limit")
...
need(execution["final_execution_candidate_shape_only"] is True, "candidate shape only")
...
print("private_operator_final_execution_candidate_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_final_execution_candidate_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_final_execution_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_FINAL_EXECUTION_CANDIDATE_HOLD_V1_GREEN"
