...
BRICK="datanet-wc-first-work-pack-private-operator-ledger-line-example-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_LINE_EXAMPLE_HOLD_V1"
...
echo "== JSON parse / private ledger-line example binding =="
python3 - <<PY
import json
...
example_record = record["private_operator_ledger_line_example"]
need(example_record["ledger_line_example_packet_created"] is True, "ledger-line example packet must be true")
need(example_record["private_operator_only"] is True, "private operator only must be true")
need(example_record["requires_private_ledger_line_candidate"] is True, "ledger-line candidate required")
...
need(example_record["example_ledger_line_is_not_effective"] is True, "example ledger line not effective")
need(example_record["example_ledger_line_is_not_created"] is True, "example ledger line not created")
need(example_record["example_ledger_line_is_not_appended"] is True, "example ledger line not appended")
...
need(example_record["award_created_here"] is False, "award created must be false")
...
packet = record["ledger_line_example_packet"]
need(packet["ledger_line_example_created_here"] is True, "packet example created")
need(packet["example_ledger_line_is_not_effective"] is True, "packet example not effective")
need(packet["example_ledger_line_is_not_created"] is True, "packet example not created")
need(packet["example_ledger_line_is_not_appended"] is True, "packet example not appended")
need(packet["effective_ledger_line_created_here"] is False, "packet effective ledger line false")
need(packet["effective_ledger_write_authorization_created_here"] is False, "packet effective auth false")
...
need(packet["example_ledger_line"]["amount"] == 100, "packet amount must be 100")
need(packet["example_ledger_line"]["amount_is_not_approved_here"] is True, "packet amount not approved")
need(packet["example_ledger_line"]["amount_is_not_supply_limit"] is True, "packet amount not supply limit")
...
need(ll["ledger_line_example_only"] is True, "ledger-line example only must be true")
...
print("private_operator_ledger_line_example_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_ledger_line_example_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_ledger_line_example_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_LINE_EXAMPLE_HOLD_V1_GREEN"
