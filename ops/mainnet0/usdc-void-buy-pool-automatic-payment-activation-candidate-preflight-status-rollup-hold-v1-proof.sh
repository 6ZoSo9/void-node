...
n="usdc-void-buy-pool-automatic-payment-activation-candidate-preflight-status-rollup-hold-v1"
marker="VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_CANDIDATE_PREFLIGHT_STATUS_ROLLUP_HOLD_V1"
...
echo "status_rollup_hold_files_and_marker_green=true"
...
assert j["id"] == n
assert j["status"] == "activation_candidate_preflight_status_rollup_held"
assert j["scope"] == "private_operator_only_preflight_status_rollup_hold_not_activation_not_release_not_execution"
assert j["precision_source_of_truth"] is True
assert j["preflight_complete_closeout_status"] == "activation_candidate_preflight_complete_closeout_held"
assert j["next_required_gate"] == "activation_candidate_preflight_status_rollup_closeout_hold_v1"

hold = j["status_rollup_hold"]
assert hold["rollup_state"] == "held_closed"
assert hold["status_rollup_generated"] is True
assert hold["preflight_chain_status"] == "complete_as_hold_only"
...
rollup = j["rollup"]
assert rollup["held_chain_count"] >= 19
...
chain = j["status_rollup_chain"]
assert isinstance(chain, list)
assert len(chain) >= 19
assert chain[-1] == "preflight_status_rollup_hold"
...
print("status_rollup_hold_fixture_green=true")
print("status_rollup_hold_held_closed_green=true")
print("status_rollup_hold_chain_green=true")
print("status_rollup_hold_boundary_green=true")
print("status_rollup_hold_all_preflight_gates_held=true")
print("status_rollup_hold_all_execution_authority_false=true")
print("status_rollup_hold_all_signing_authority_false=true")
print("status_rollup_hold_all_transfer_authority_false=true")
print("status_rollup_hold_all_public_mutation_false=true")
print("status_rollup_hold_no_activation_finalized=true")
print("status_rollup_hold_no_activation_enablement=true")
print("status_rollup_hold_no_execution=true")
PY

if grep -RInF 'git status --short' "$doc" "$fixture"; then
  echo "status_rollup_hold_command_contamination=red"
  exit 1
fi

if grep -RInF 'zoso@' "$doc" "$fixture"; then
  echo "status_rollup_hold_prompt_contamination=red"
  exit 1
fi

echo "status_rollup_hold_contamination_absent=true"
...
grep -Fq "activation_candidate_preflight_status_rollup_closeout_hold_v1" "$doc"
grep -Fq "activation_candidate_preflight_status_rollup_closeout_hold_v1" "$fixture"

echo "status_rollup_hold_no_signing=true"
echo "status_rollup_hold_no_signature=true"
echo "status_rollup_hold_no_private_key_access=true"
echo "status_rollup_hold_no_void_transfer=true"
echo "status_rollup_hold_no_transaction_broadcast=true"
