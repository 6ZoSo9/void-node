...
echo "VOID_WC_TO_VOID_PUBLIC_SMOKE_RECEIPT_MARKER_FIX_V1_PROOF_BEGIN"

good_marker="VOID_WC_TO_VOID_REDACTED_SETTLEMENT_RECEIPT_V1"
bad_marker="VOID_WC_TO_VOID_REDACTED_SETTLEMENT_RECEIPT_RUNTIME_V1"
receipt_route="/public-node/wc-to-void/redacted-settlement-receipt-v1.json"
...
grep -F "$receipt_route" "$script" >/dev/null
grep -F "$receipt_route" "$proof" >/dev/null

if grep -F "$bad_marker" "$script" "$proof" >/dev/null; then
  echo "STOP: stale receipt runtime marker remains in smoke files."
  exit 1
fi
...
if grep -F "VOID_WC_TO_VOID_PUBLIC_REVIEWER_VERIFY_PACK_RUNTIME_V1" "$script" "$proof" >/dev/null; then
...
echo "VOID_WC_TO_VOID_PUBLIC_SMOKE_RECEIPT_MARKER_FIX_V1_ASSERT_GREEN"
echo "VOID_WC_TO_VOID_PUBLIC_SMOKE_RECEIPT_MARKER_FIX_V1_PROOF_GREEN"
