...
n="usdc-void-buy-pool-automatic-payment-activation-candidate-preflight-complete-hold-v1"
marker="VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_CANDIDATE_PREFLIGHT_COMPLETE_HOLD_V1"
...
echo "complete_hold_files_and_marker_green=true"
...
assert j["id"] == n
assert j["status"] == "activation_candidate_preflight_complete_held"
assert j["scope"] == "private_operator_only_preflight_complete_hold_not_activation_not_release_not_execution"
assert j["precision_source_of_truth"] is True
assert j["final_seal_status"] == "activation_candidate_preflight_final_seal_held"
assert j["next_required_gate"] == "activation_candidate_preflight_complete_closeout_hold_v1"

hold = j["complete_hold"]
assert hold["completion_state"] == "held_closed"
assert hold["preflight_chain_complete_as_hold"] is True
...
chain = j["complete_hold_chain"]
assert isinstance(chain, list)
assert len(chain) >= 17
assert chain[-1] == "preflight_complete_hold"
...
print("complete_hold_fixture_green=true")
print("complete_hold_held_closed_green=true")
print("complete_hold_chain_green=true")
print("complete_hold_boundary_green=true")
print("complete_hold_no_preflight_complete_record=true")
print("complete_hold_no_activation_finalized=true")
print("complete_hold_no_activation_release=true")
print("complete_hold_no_activation_enablement=true")
print("complete_hold_no_execution=true")
PY

if grep -RInF 'git status --short' "$doc" "$fixture"; then
  echo "complete_hold_command_contamination=red"
  exit 1
fi

if grep -RInF 'zoso@' "$doc" "$fixture"; then
  echo "complete_hold_prompt_contamination=red"
  exit 1
fi

echo "complete_hold_contamination_absent=true"
...
echo "complete_hold_no_signing=true"
echo "complete_hold_no_signature=true"
echo "complete_hold_no_private_key_access=true"
echo "complete_hold_no_void_transfer=true"
echo "complete_hold_no_transaction_broadcast=true"
