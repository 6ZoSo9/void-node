...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-chain-closeout-audit-rollup-hold-v1"
...
echo "== JSON parse / private award append chain closeout binding =="
...
assert(record.closeout_type === "private_operator_award_append_full_chain_closeout_audit_rollup", "closeout type mismatch");
...
assert(record.closeout.private_effective_execution_final_seal_bound === true, "effective final seal closeout mismatch");
assert(record.closeout.chain_state === "closed_for_review_hold", "chain state mismatch");
...
console.log("private_operator_award_append_chain_closeout_binding_green=true");
NODE

echo "== forbidden public mutation scan =="
if git grep -n "$MARKER" -- public >/tmp/void-private-award-append-chain-closeout-public-leak.txt 2>/dev/null; then
  cat /tmp/void-private-award-append-chain-closeout-public-leak.txt
  echo "private_award_append_chain_closeout_marker_not_in_public_tree_green=false"
  exit 1
fi
rm -f /tmp/void-private-award-append-chain-closeout-public-leak.txt
echo "private_award_append_chain_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_CHAIN_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
