...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-preflight-hold-v1"
...
echo "== JSON parse / private binding =="
python3 - <<PY
import json
...
private = record["private_operator_preflight"]
need(private["preflight_template_created"] is True, "preflight template must be true")
...
need(append["preflight_only"] is True, "append preflight only must be true")
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_PREFLIGHT_HOLD_V1_GREEN"
