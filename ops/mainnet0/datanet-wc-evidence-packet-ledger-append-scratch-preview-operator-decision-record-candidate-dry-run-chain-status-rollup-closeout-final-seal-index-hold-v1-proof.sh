...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_STATUS_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1"
SOURCE_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_STATUS_ROLLUP_CLOSEOUT_HOLD_V1"
TOOL="tools/${LANE}.mjs"
DOC="docs/datanet/${LANE}-hold-v1.md"
PROOF="ops/mainnet0/${LANE}-hold-v1-proof.sh"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-hold-v1-proof.sh"
...
printf '== operator decision record candidate dry-run chain status rollup closeout source proof ==\n'
bash "${PREV_PROOF}"

TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT
SOURCE="${TMP}/operator-decision-record-candidate-dry-run-chain-status-rollup-closeout.json"
OUT_A="${TMP}/operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index-a.json"
OUT_B="${TMP}/operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index-b.json"
BAD="${TMP}/bad-source.json"
BAD_OUT="${TMP}/bad-index.json"
CREATED="${TMP}/created-source.json"
CREATED_OUT="${TMP}/created-index.json"
SIGNED="${TMP}/signed-source.json"
SIGNED_OUT="${TMP}/signed-index.json"
EXEC_READY="${TMP}/exec-ready-source.json"
EXEC_READY_OUT="${TMP}/exec-ready-index.json"
AUTH_EXEC="${TMP}/auth-exec-source.json"
AUTH_EXEC_OUT="${TMP}/auth-exec-index.json"

printf '== create fixture operator decision record candidate dry-run chain status rollup closeout ==\n'
cat > "${SOURCE}" <<JSON
{
  ...
  "source": {
    ...
    "path_basename": "operator-decision-record-candidate-dry-run-chain-status-rollup.json",
...
printf '== operator decision record candidate dry-run chain status rollup closeout final seal index ==\n'
node "${TOOL}" --source "${SOURCE}" --out "${OUT_A}" --sealed-at "2026-07-03T00:00:00.000Z" --index-id "operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index-fixture-v1"
node "${TOOL}" --source "${SOURCE}" --out "${OUT_B}" --sealed-at "2026-07-03T00:00:00.000Z" --index-id "operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index-fixture-v1"

printf '== final seal index deterministic / binding ==\n'
cmp "${OUT_A}" "${OUT_B}"
echo "operator_decision_record_candidate_dry_run_chain_status_rollup_closeout_final_seal_index_deterministic_green=true"
...
const expectedMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_STATUS_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1';
const expectedSourceMarker = 'VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_STATUS_ROLLUP_CLOSEOUT_HOLD_V1';
const sourceHash = crypto.createHash('sha256').update(sourceRaw).digest('hex');
function assert(condition, message) {
  if (!condition) throw new Error(message);
}
assert(out.marker === expectedMarker, 'index marker mismatch');
assert(out.status === 'indexed', 'index status mismatch');
...
assert(out.source?.rollup_hash === source.source.rollup_hash, 'rollup hash binding mismatch');
...
assert(out.entries?.length === 1, 'expected one index entry');
assert(out.entries[0]?.kind === 'operator_decision_record_candidate_dry_run_chain_status_rollup_closeout', 'entry kind mismatch');
assert(out.entries[0]?.closeout_hash === source.closeout_hash, 'entry closeout hash mismatch');
assert(out.entries[0]?.rollup_hash === source.source.rollup_hash, 'entry rollup hash mismatch');
assert(out.index_guardrails?.source_chain_status_rollup_closeout_only === true, 'source chain status rollup closeout guard missing');
assert(out.index_guardrails?.no_signature_material_requested === true, 'signature guard missing');
assert(out.index_guardrails?.no_automatic_promotion_to_operator_decision === true, 'automatic promotion guard missing');
assert(out.index_guardrails?.no_execution_authority === true, 'execution authority guard missing');
assert(out.index_guardrails?.no_canonical_ledger_target_path_written === true, 'canonical ledger target path guard missing');
assert(out.checks?.source_closeout_hash_bound_green === true, 'missing closeout hash check');
assert(out.checks?.source_rollup_hash_bound_green === true, 'missing rollup hash check');
assert(out.checks?.source_final_seal_index_closeout_hash_bound_green === true, 'missing final seal index closeout hash check');
assert(out.checks?.source_final_seal_index_hash_bound_green === true, 'missing final seal index hash check');
assert(out.checks?.source_candidate_closeout_hash_bound_green === true, 'missing candidate closeout hash check');
...
assert(out.boundary?.operator_decision_record_candidate_dry_run_chain_status_rollup_closeout_final_seal_index_only === true, 'candidate chain status rollup closeout final seal index boundary missing');
...
NODE_BINDING_EOF
echo "operator_decision_record_candidate_dry_run_chain_status_rollup_closeout_final_seal_index_binding_green=true"

printf '== bad marker rejection ==\n'
cat > "${BAD}" <<JSON
{"marker":"VOID_BAD_MARKER"}
JSON
if node "${TOOL}" --source "${BAD}" --out "${BAD_OUT}" >/tmp/void-bad-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-bad-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log
...
if node "${TOOL}" --source "${CREATED}" --out "${CREATED_OUT}" >/tmp/void-created-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-created-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log
...
if node "${TOOL}" --source "${SIGNED}" --out "${SIGNED_OUT}" >/tmp/void-signed-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-signed-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log
...
if node "${TOOL}" --source "${EXEC_READY}" --out "${EXEC_READY_OUT}" >/tmp/void-exec-ready-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-exec-ready-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log
...
if node "${TOOL}" --source "${AUTH_EXEC}" --out "${AUTH_EXEC_OUT}" >/tmp/void-auth-exec-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log 2>&1; then
  cat /tmp/void-auth-exec-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index.log
