...
BRICK="datanet-wc-first-work-pack-private-operator-ledger-write-authorization-example-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_WRITE_AUTHORIZATION_EXAMPLE_HOLD_V1"
...
echo "== JSON parse / private authorization example binding =="
python3 - <<PY
import json
...
auth = record["private_operator_authorization_example"]
need(auth["authorization_example_packet_created"] is True, "authorization example packet must be true")
need(auth["private_operator_only"] is True, "private operator only must be true")
need(auth["requires_private_ledger_write_authorization_candidate"] is True, "auth candidate required")
need(auth["requires_private_approval_closeout"] is True, "approval closeout required")
need(auth["requires_private_decision_closeout"] is True, "decision closeout required")
need(auth["requires_private_preflight_closeout"] is True, "preflight closeout required")
need(auth["example_authorization_is_not_effective"] is True, "example authorization not effective")
need(auth["example_authorization_is_not_ledger_write_authorization"] is True, "example authorization not ledger write auth")
need(auth["example_authorization_is_not_ledger_append"] is True, "example authorization not ledger append")
need(auth["example_amount_wc"] == 100, "example amount must be 100")
need(auth["example_amount_is_not_approved_here"] is True, "example amount not approved")
need(auth["example_amount_is_not_supply_limit"] is True, "example amount not supply limit")
need(auth["effective_ledger_write_authorization_created_here"] is False, "effective ledger auth must be false")
need(auth["effective_operator_approval_created_here"] is False, "effective approval must be false")
need(auth["operator_append_decision_created_here"] is False, "append decision must be false")
need(auth["award_created_here"] is False, "award created must be false")
need(auth["award_approved_here"] is False, "award approved must be false")
need(auth["ledger_write_authorized_here"] is False, "ledger authorization must be false")
need(auth["ledger_line_created_here"] is False, "ledger line must be false")
need(auth["ledger_append_performed_here"] is False, "ledger append must be false")
need(auth["wc_issuance_created_here"] is False, "WC issuance must be false")
need(auth["wc_ledger_write_created_here"] is False, "WC ledger write must be false")
need(auth["reward_created_here"] is False, "reward must be false")
need(auth["void_transfer_created_here"] is False, "VOID transfer must be false")
need(auth["runtime_append_endpoint_created"] is False, "runtime endpoint must be false")

packet = record["authorization_example_packet"]
need(packet["example_authorization_is_not_effective"] is True, "packet auth not effective")
need(packet["example_authorization_is_not_ledger_write_authorization"] is True, "packet auth not ledger write auth")
need(packet["example_authorization_is_not_ledger_append"] is True, "packet auth not ledger append")
need(packet["authorization_example_created_here"] is True, "packet auth example created")
need(packet["effective_ledger_write_authorization_created_here"] is False, "packet effective ledger auth false")
...
need(packet["candidate_ledger_context"]["amount"] == 100, "packet amount must be 100")
need(packet["candidate_ledger_context"]["amount_is_not_approved_here"] is True, "packet amount not approved")
need(packet["candidate_ledger_context"]["amount_is_not_supply_limit"] is True, "packet amount not supply limit")
...
ab = record["authorization_boundary"]
need(ab["authorization_example_only"] is True, "authorization example only must be true")
need(ab["effective_ledger_write_authorization_created"] is False, "effective ledger auth must be false")
...
print("private_operator_ledger_write_authorization_example_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_ledger_write_authorization_example_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_ledger_write_authorization_example_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_WRITE_AUTHORIZATION_EXAMPLE_HOLD_V1_GREEN"
