...
DOC="docs/work-credits/void-datanet-wc-availability-ledger-write-execute-gate-hold-v1.md"
SCHEMA="fixtures/work-credits/void-datanet-wc-availability-ledger-write-execute-gate-hold-schema-v1.json"
EXAMPLE="fixtures/work-credits/void-datanet-wc-availability-ledger-write-execute-gate-hold-example-v1.json"
MARKER="VOID_DATANET_WC_AVAILABILITY_LEDGER_WRITE_EXECUTE_GATE_HOLD_V1"

fail() {
  echo "void_datanet_wc_availability_ledger_write_execute_gate_hold_v1_proof=FAIL reason=$1"
  exit 1
}
...
grep -Fq "Ledger-write execute gate hold only; no WC issuance and no WC ledger write." "$DOC" || fail "missing_status"
grep -Fq "This artifact does not open execution, authorize execution, issue Work Credits, write the WC ledger, create a ledger line, or append to a ledger file." "$DOC" || fail "missing_no_execute_or_mutation_clause"
...
assert schema["status"] == "ledger_write_execute_gate_hold_only_no_wc_issuance_no_wc_ledger_write"
...
print("example_ledger_write_execute_gate_hold_green=true")
PY

if grep -R "issues_work_credits.*true\|writes_wc_ledger.*true\|creates_ledger_line.*true\|appends_to_ledger_file.*true\|allocates_void.*true\|transfers_void.*true\|automatic_reward.*true\|approves_ledger_write.*true\|executes_ledger_write.*true\|authorizes_ledger_write_execution.*true\|opens_execute_gate.*true\|performs_ledger_mutation.*true\|activates_public_mutation.*true\|grants_signer_wallet_access.*true\|authorizes_execution.*true\|moves_funds.*true" "$SCHEMA" "$EXAMPLE" "$DOC" >/tmp/void_datanet_wc_ledger_write_execute_gate_hold_forbidden_true.txt; then
  cat /tmp/void_datanet_wc_ledger_write_execute_gate_hold_forbidden_true.txt
  fail "forbidden_true_flag"
fi

echo "void_datanet_wc_availability_ledger_write_execute_gate_hold_v1_proof=GREEN marker=$MARKER"
