...
DOC="docs/work-credits/void-datanet-wc-availability-ledger-write-no-write-closeout-rollup-v1.md"
SCHEMA="fixtures/work-credits/void-datanet-wc-availability-ledger-write-no-write-closeout-rollup-schema-v1.json"
EXAMPLE="fixtures/work-credits/void-datanet-wc-availability-ledger-write-no-write-closeout-rollup-example-v1.json"
MARKER="VOID_DATANET_WC_AVAILABILITY_LEDGER_WRITE_NO_WRITE_CLOSEOUT_ROLLUP_V1"

fail() {
  echo "void_datanet_wc_availability_ledger_write_no_write_closeout_rollup_v1_proof=FAIL reason=$1"
  exit 1
}
...
grep -Fq "No-write closeout rollup only; no WC issuance and no WC ledger write." "$DOC" || fail "missing_status"
grep -Fq "This artifact records that the lane reached a closed no-write result." "$DOC" || fail "missing_closed_no_write_clause"
...
assert schema["status"] == "no_write_closeout_rollup_only_no_wc_issuance_no_wc_ledger_write"
assert example["packet_kind"] == schema["packet_kind"]

for field in schema["required_fields"]:
    assert field in example, f"missing required field: {field}"

for key, value in schema["covered_markers"].items():
...
assert example["closeout_status"] in schema["allowed_closeout_status"]
assert example["closeout_status"] == "closed_no_write_execute_gate_held"
