...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-creation-decision-boundary-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_creation_decision_boundary_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_CREATION_DECISION_BOUNDARY_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_CREATION_DECISION_BOUNDARY_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=ac716e09 \
  non_existence_seal_lane_seal_cross_box_green=true \
  record_absence_sealed=true \
  creation_decision_boundary_recorded=true \
...
echo "${u}_CREATION_DECISION_BOUNDARY_PROOF_V1_GREEN"
