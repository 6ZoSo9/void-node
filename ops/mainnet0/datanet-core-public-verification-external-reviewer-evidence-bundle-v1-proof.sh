...
name="datanet-core-public-verification-external-reviewer-evidence-bundle-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_evidence_bundle_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_EVIDENCE_BUNDLE"

prev_name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-manual-reviewer-candidate-record-entry-operator-reveal-command-final-approval-packet-lane-seal-v1"
prev_proof="ops/mainnet0/${prev_name}-proof.sh"
prev_u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"
prev_out="/tmp/void-proof-external-reviewer-evidence-bundle-prev-lane-seal.out"
...
bash "$prev_proof" > "$prev_out"
grep -q "${prev_u}_MANUAL_REVIEWER_CANDIDATE_RECORD_ENTRY_OPERATOR_REVEAL_COMMAND_FINAL_APPROVAL_PACKET_LANE_SEAL_PROOF_V1_GREEN" "$prev_out"
grep -q 'operator_reveal_command_final_approval_granted_now=false' "$prev_out"
grep -q 'operator_execute_command_revealed_now=false' "$prev_out"
grep -q 'operator_execute_command_printed_now=false' "$prev_out"
grep -q 'operator_execute_command_executed_now=false' "$prev_out"
grep -q 'candidate_record_entry_written_now=false' "$prev_out"
grep -q 'packet_shared_now=false' "$prev_out"
grep -q 'public_mutation=false' "$prev_out"
grep -q 'ledger_write=false' "$prev_out"
grep -q 'wc_credit_award=false' "$prev_out"

for suffix in \
  created_now=true \
  terminal_safe=true \
  static_only=true \
  latest_sealed_head=8a28c111 \
  latest_final_approval_packet_lane_seal_cross_box_green=true \
...
echo "${pfx}latest_sealed_head=8a28c111"
echo "${pfx}latest_final_approval_packet_lane_seal_cross_box_green=true"
...
echo "${pfx}proof_scope=single_static_external_reviewer_evidence_bundle_no_final_reveal_approval_no_command_reveal_no_candidate_identity_no_candidate_record_no_reviewer_contact_no_packet_share_no_mutation_no_ledger_write_no_wc_award"
