...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-terminal-closeout-index-closeout-final-seal-index"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_TERMINAL_CLOSEOUT_INDEX_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1"

SRC_LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-terminal-closeout-index-closeout"
SRC_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_TERMINAL_CLOSEOUT_INDEX_CLOSEOUT_HOLD_V1"
...
BAD_HASH="$(mktemp)"
BAD_BOUNDARY="$(mktemp)"
trap 'rm -f "$OUT1" "$OUT2" "$DISPLAY_OUT" "$VALIDATOR" "$BAD_MARKER" "$BAD_HASH" "$BAD_BOUNDARY"' EXIT
...
if (data.artifact.status !== 'read_only_terminal_closeout_index_closeout_final_seal_index_ready') fail('status mismatch');
...
for (const field of falseFields) {
  if (data.artifact.boundary?.[field] !== false) fail(`${field} must be false`);
}
...
echo "== terminal closeout index closeout final seal index deterministic / binding =="
echo "operator_decision_record_candidate_dry_run_chain_terminal_closeout_index_closeout_final_seal_index_deterministic_green=true"
echo "operator_decision_record_candidate_dry_run_chain_terminal_closeout_index_closeout_final_seal_index_binding_green=true"
...
if MARKER="$MARKER" SRC_DOC="$SRC_DOC" SRC_PROOF="$SRC_PROOF" SRC_TOOL="$SRC_TOOL" node "$VALIDATOR" "$BAD_MARKER" >/dev/null 2>&1; then
  echo "bad_marker_rejection_failed=true"; exit 1;
fi
...
if MARKER="$MARKER" SRC_DOC="$SRC_DOC" SRC_PROOF="$SRC_PROOF" SRC_TOOL="$SRC_TOOL" node "$VALIDATOR" "$BAD_HASH" >/dev/null 2>&1; then
  echo "bad_source_hash_rejection_failed=true"; exit 1;
fi
...
NODE
  if MARKER="$MARKER" SRC_DOC="$SRC_DOC" SRC_PROOF="$SRC_PROOF" SRC_TOOL="$SRC_TOOL" node "$VALIDATOR" "$BAD_BOUNDARY" >/dev/null 2>&1; then
    echo "${field}_rejection_failed=true"; exit 1;
  fi
...
if grep -RInE '100,000,000 WC|100000000 WC|100m WC|100M WC|WC supply cap|capped WC|WC cap' "$DOC" "$TOOL"; then
  echo "forbidden_wc_cap_wording_scan_failed=true"; exit 1;
fi
...
if grep -RInE '"(operator_decision_created|operator_signature_created|approval_execution_created|canonical_ledger_append_created|wc_issuance_created|wc_claim_created|wallet_transfer_created|mutation_authority_created)"[[:space:]]*:[[:space:]]*true' "$DOC" "$TOOL"; then
  echo "no_mutation_authority_scan_failed=true"; exit 1;
fi
