...
BRICK="datanet-wc-held-transfer-execution-result-candidate-hold-v1"
...
assert entry["scope"] == "datanet_held_transfer_execution_result_candidate"
assert entry["transfer_execution_result_candidate_only"] is True
assert entry["execution_result_created"] is False
assert entry["execution_result_status"] == "not_executed_hold"
...
assert entry["transaction_broadcast"] is False
...
assert card["schema"] == "void.public_node.work_credits.held_transfer_execution_result_candidate.v1"
assert card["id"] == brick
assert card["status"] == "hold"
assert card["marker"] == marker
assert card["transfer_execution_result_candidate_only"] is True
assert card["candidate_amounts"]["wc_amount"] == 0
assert card["candidate_amounts"]["void_amount"] == 0

state = card["execution_result_state"]
assert state["execution_result_created"] is False
assert state["execution_result_status"] == "not_executed_hold"
assert state["execute_gate_created"] is False
...
assert source["transfer_execute_gate_candidate"] == "/public-node/work-credits/datanet-wc-held-transfer-execute-gate-candidate-hold-v1.json"

print("held_transfer_execution_result_candidate_binding_green=true")
PY
...
paths = [
    ...
    Path("public/public-node/work-credits/datanet-wc-held-transfer-execution-result-candidate-hold-v1.json"),
    Path("docs/public-node/work-credits/datanet-wc-held-transfer-execution-result-candidate-hold-v1.md"),
]

bad = [
    '"execution_result_created": true',
    '"execute_gate_created": true',
...
echo "== result =="
echo "VOID_DATANET_WC_HELD_TRANSFER_EXECUTION_RESULT_CANDIDATE_HOLD_V1_GREEN"
