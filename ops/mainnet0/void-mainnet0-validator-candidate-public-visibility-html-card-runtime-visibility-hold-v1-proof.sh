...
SOURCE_JSON="public/public-node/validators/${SOURCE_BRICK}.json"
...
grep -F "$SOURCE_MARKER" "$SOURCE_JSON" >/dev/null
...
assert runtime["source_visibility_record_route"] == source_route
assert runtime["html_card_marker"] == html_marker
assert runtime["source_visibility_record_marker"] == source_marker
...
print("validator_candidate_public_visibility_html_runtime_binding_green=true")
PYCHECK
