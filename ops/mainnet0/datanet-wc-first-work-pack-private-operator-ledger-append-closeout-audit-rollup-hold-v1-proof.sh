...
BRICK="datanet-wc-first-work-pack-private-operator-ledger-append-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_APPEND_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
echo "== JSON parse / private ledger-append closeout binding =="
python3 - <<PY
import json
...
need(closeout["private_ledger_append_candidate_exists"] is True, "ledger-append candidate should exist")
need(closeout["private_ledger_append_example_exists"] is True, "ledger-append example should exist")
...
need(closeout["example_append_is_not_effective"] is True, "example append not effective")
need(closeout["example_append_is_not_performed"] is True, "example append not performed")
need(closeout["example_ledger_line_is_not_created"] is True, "example ledger line not created")
need(closeout["requires_future_effective_ledger_append"] is True, "future ledger append required")
need(closeout["requires_future_effective_ledger_line_creation"] is True, "future ledger line required")
...
need(closeout["effective_ledger_append_performed"] is False, "effective append must be false")
need(closeout["effective_ledger_line_created"] is False, "effective ledger line must be false")
need(closeout["effective_ledger_write_authorization_created"] is False, "effective ledger auth must be false")
...
need(closeout["award_created"] is False, "award must be false")
need(closeout["award_approved"] is False, "award approved must be false")
need(closeout["ledger_write_authorized"] is False, "ledger write auth must be false")
need(closeout["ledger_line_created"] is False, "ledger line created must be false")
need(closeout["ledger_append_performed"] is False, "ledger append must be false")
need(closeout["ledger_mutation_performed"] is False, "ledger mutation must be false")
...
need(append["private_ledger_append_closeout_only"] is True, "private ledger-append closeout only must be true")
...
need(append["ledger_mutation_performed"] is False, "ledger mutation must be false")
...
print("private_operator_ledger_append_closeout_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_ledger_append_closeout_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_ledger_append_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_APPEND_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
