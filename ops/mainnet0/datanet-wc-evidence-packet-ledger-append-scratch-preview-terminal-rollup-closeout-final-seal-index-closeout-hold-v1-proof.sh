...
TOOL="tools/datanet-wc-evidence-packet-ledger-append-scratch-preview-terminal-rollup-closeout-final-seal-index-closeout.mjs"
DOC="docs/datanet/datanet-wc-evidence-packet-ledger-append-scratch-preview-terminal-rollup-closeout-final-seal-index-closeout-hold-v1.md"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-preview-terminal-rollup-closeout-final-seal-index-hold-v1-proof.sh"
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

INPUT="$TMP_DIR/terminal-rollup-closeout-final-seal-index.json"
OUT_A="$TMP_DIR/terminal-rollup-closeout-final-seal-index-closeout-a.json"
OUT_B="$TMP_DIR/terminal-rollup-closeout-final-seal-index-closeout-b.json"
...
printf '== terminal rollup closeout final seal index source proof ==\n'
bash "$PREV_PROOF"

printf '== create fixture terminal rollup closeout final seal index ==\n'
python3 - "$INPUT" "$FINAL_SEAL_INDEX_MARKER" "$TERMINAL_ROLLUP_CLOSEOUT_MARKER" "$TERMINAL_ROLLUP_MARKER" "$PRIOR_FINAL_SEAL_INDEX_CLOSEOUT_MARKER" "$PRIOR_FINAL_SEAL_INDEX_MARKER" "$REVIEW_INDEX_CLOSEOUT_MARKER" "$REVIEW_INDEX_MARKER" "$RECEIPT_CLOSEOUT_MARKER" "$RECEIPT_MARKER" "$SCRATCH_APPLY_MARKER" "$EXECUTE_PACKET_MARKER" "$DRY_RUN_MARKER" "$LEDGER_PACKET_MARKER" <<'PYFIXTURE'
import json, sys
(out, final_marker, terminal_close_marker, terminal_marker, prior_final_close_marker, prior_final_marker, review_close_marker, review_marker, receipt_close_marker, receipt_marker, scratch_marker, execute_marker, dry_marker, ledger_marker) = sys.argv[1:]
hexes = {
    "terminal_rollup_closeout_final_seal_index_id": "0" * 64,
    "terminal_rollup_closeout_id": "1" * 64,
    "terminal_rollup_id": "2" * 64,
    "prior_final_seal_index_closeout_id": "3" * 64,
    "prior_final_seal_index_id": "4" * 64,
    "review_index_closeout_id": "5" * 64,
    "review_index_id": "6" * 64,
    "receipt_closeout_id": "7" * 64,
    "receipt_id": "8" * 64,
    "scratch_apply_id": "9" * 64,
    "execute_packet_id": "a" * 64,
    "dry_run_id": "b" * 64,
    "packet_id": "c" * 64,
    "evidence_hash": "d" * 64,
    "current_scratch_ledger_hash": "e" * 64,
    "scratch_ledger_out_hash": "f" * 64,
    "appended_line_hash": "0" * 64,
    "logical_candidate_next_ledger_hash": "1" * 64,
}
record = {
...
print("fixture_terminal_rollup_closeout_final_seal_index_green=true")
PYFIXTURE

printf '== terminal rollup closeout final seal index closeout ==\n'
VOID_LEDGER_APPEND_SCRATCH_PREVIEW_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$TOOL" \
  --terminal-rollup-closeout-final-seal-index "$INPUT" \
  --out "$OUT_A" \
  --closer "operator-terminal-rollup-closeout-final-seal-index-closeout" \
  --reason "fixture closeout binds scratch preview terminal rollup closeout final seal index only" >/tmp/void-scratch-preview-terminal-rollup-closeout-final-seal-index-closeout-a.out

VOID_LEDGER_APPEND_SCRATCH_PREVIEW_TERMINAL_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$TOOL" \
  --terminal-rollup-closeout-final-seal-index "$INPUT" \
  --out "$OUT_B" \
  --closer "operator-terminal-rollup-closeout-final-seal-index-closeout" \
  --reason "fixture closeout binds scratch preview terminal rollup closeout final seal index only" >/tmp/void-scratch-preview-terminal-rollup-closeout-final-seal-index-closeout-b.out

printf '== terminal rollup closeout final seal index closeout deterministic / binding ==\n'
python3 - "$INPUT" "$OUT_A" "$OUT_B" "$MARKER" "$FINAL_SEAL_INDEX_MARKER" "$TERMINAL_ROLLUP_CLOSEOUT_MARKER" "$TERMINAL_ROLLUP_MARKER" <<'PYCHECK'
import json, pathlib, re, sys
input_path, out_a, out_b, marker, final_marker, terminal_close_marker, terminal_marker = sys.argv[1:]
...
assert record["schema"] == "void.datanet.wc.evidence_packet_ledger_append_scratch_preview_terminal_rollup_closeout_final_seal_index_closeout.v1"
assert record["marker"] == marker
assert record["status"] == "scratch_preview_terminal_rollup_closeout_final_seal_index_closed_for_operator_review"
assert re.fullmatch(r"[0-9a-f]{64}", record["scratch_preview_terminal_rollup_closeout_final_seal_index_closeout_id"])
assert record["closer"] == "operator-terminal-rollup-closeout-final-seal-index-closeout"
assert record["final_seal_index"]["path"] == str(pathlib.Path(input_path).resolve())
assert record["final_seal_index"]["marker"] == final_marker
assert record["final_seal_index"]["scratch_preview_terminal_rollup_closeout_final_seal_index_id"] == source["scratch_preview_terminal_rollup_closeout_final_seal_index_id"]
assert record["terminal_rollup_closeout"]["marker"] == terminal_close_marker
...
assert record["terminal_rollup"]["marker"] == terminal_marker
...
assert record["prior_final_seal_index_closeout"]["scratch_apply_receipt_closeout_review_index_closeout_final_seal_index_closeout_id"] == source["final_seal_index_closeout"]["scratch_apply_receipt_closeout_review_index_closeout_final_seal_index_closeout_id"]
...
assert summary["terminal_rollup_closeout_final_seal_index_bound"] is True
assert summary["scratch_preview_chain_terminal_rollup_closeout_final_seal_index_closed_for_operator_review"] is True
assert summary["terminal_rollup_closeout_final_seal_index_closeout_only"] is True
...
assert policy["scratch_preview_terminal_rollup_closeout_final_seal_index_closeout_only"] is True
boundary = record["boundary"]
assert boundary["scratch_preview_terminal_rollup_closeout_final_seal_index_closeout_only"] is True
assert boundary["scratch_preview_terminal_rollup_closeout_final_seal_index_only_source"] is True
...
print("scratch_preview_terminal_rollup_closeout_final_seal_index_closeout_deterministic_green=true")
print("scratch_preview_terminal_rollup_closeout_final_seal_index_closeout_binding_green=true")
PYCHECK

printf '== bad terminal rollup closeout final seal index rejection ==\n'
python3 - "$INPUT" "$BAD_STATUS" <<'PYBADSTATUS'
import json, sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
record["status"] = "not_final_indexed"
open(sys.argv[2], "w", encoding="utf-8").write(json.dumps(record, indent=2) + "\n")
PYBADSTATUS
if node "$TOOL" \
  --terminal-rollup-closeout-final-seal-index "$BAD_STATUS" \
  --out "$TMP_DIR/bad-status-closeout.json" \
  --closer "operator-terminal-rollup-closeout-final-seal-index-closeout" \
  --reason "bad final seal index should fail" >/tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-status-out 2>/tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-status-err; then
  echo "bad_terminal_rollup_closeout_final_seal_index_rejection_green=false"
  cat /tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-status-out
  exit 1
fi
grep -Fq "terminal_rollup_closeout_final_seal_index_status_mismatch" /tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-status-err
echo "bad_terminal_rollup_closeout_final_seal_index_rejection_green=true"
...
PYBADMARKER
if node "$TOOL" \
  --terminal-rollup-closeout-final-seal-index "$BAD_MARKER" \
  --out "$TMP_DIR/bad-marker-closeout.json" \
  --closer "operator-terminal-rollup-closeout-final-seal-index-closeout" \
  --reason "bad marker should fail" >/tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-marker-out 2>/tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-marker-err; then
  echo "bad_marker_rejection_green=false"
  cat /tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-marker-out
  exit 1
fi
grep -Fq "terminal_rollup_closeout_final_seal_index_marker_mismatch" /tmp/void-terminal-rollup-closeout-final-seal-index-closeout-bad-marker-err
...
grep -Fq "$MARKER" "$DOC"
grep -Fq "$MARKER" "$0"
grep -Fq "terminal rollup closeout final seal index closeout" "$DOC"
grep -Fq "scratch_preview_terminal_rollup_closeout_final_seal_index_closeout" "$TOOL"
