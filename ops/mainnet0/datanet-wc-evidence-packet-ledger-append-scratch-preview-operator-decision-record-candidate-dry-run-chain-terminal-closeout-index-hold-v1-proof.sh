...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-terminal-closeout-index"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_TERMINAL_CLOSEOUT_INDEX_HOLD_V1"

SRC_LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-record-candidate-dry-run-chain-status-rollup-closeout-final-seal-index-closeout"
SRC_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_RECORD_CANDIDATE_DRY_RUN_CHAIN_STATUS_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_HOLD_V1"
...
BAD="$(mktemp)"
trap 'rm -f "$OUT1" "$OUT2" "$DISPLAY_OUT" "$VALIDATOR" "$BAD"' EXIT
...
if (data.artifact.status !== 'read_only_terminal_closeout_index_ready') fail('status mismatch');
...
echo "== terminal closeout index deterministic / binding =="
echo "operator_decision_record_candidate_dry_run_chain_terminal_closeout_index_deterministic_green=true"
echo "operator_decision_record_candidate_dry_run_chain_terminal_closeout_index_binding_green=true"

cp "$OUT1" "$BAD"
node --input-type=module - "$BAD" <<'NODE'
...
if MARKER="$MARKER" SRC_DOC="$SRC_DOC" SRC_PROOF="$SRC_PROOF" SRC_TOOL="$SRC_TOOL" node "$VALIDATOR" "$BAD" >/dev/null 2>&1; then
...
for field in operator_decision_created operator_signature_created approval_execution_created canonical_ledger_append_created wc_issuance_created wc_claim_created wallet_transfer_created mutation_authority_created; do
  cp "$OUT1" "$BAD"
  FIELD="$field" node --input-type=module - "$BAD" <<'NODE'
import { readFileSync, writeFileSync } from 'node:fs';
const file = process.argv[2];
