...
DOC="docs/datanet/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index-closeout-final-seal-index-hold-v1.md"
PREV_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index-closeout-hold-v1-proof.sh"
...
FINAL_SEAL_INDEX_A="$TMP_DIR/final-seal-index-a.json"
FINAL_SEAL_INDEX_B="$TMP_DIR/final-seal-index-b.json"
BAD_INDEX_CLOSEOUT="$TMP_DIR/bad-index-closeout.json"
BAD_MARKER="$TMP_DIR/bad-marker-index-closeout.json"
...
for tool in \
  ...
  "$REVIEW_INDEX_TOOL" "$INDEX_CLOSEOUT_TOOL" "$FINAL_SEAL_INDEX_TOOL"; do
  test -x "$tool"
  node --check "$tool"
done
bash -n "$PREV_PROOF"

printf '== scratch apply receipt closeout review index closeout source proof ==\n'
bash "$PREV_PROOF"

printf '== create scratch apply receipt closeout review index closeout ==\n'
VOID_PACKET_CREATED_AT="$CREATED_AT" node "$ROUNDTRIP_TOOL" \
  ...
  --summary-out "$SUMMARY" >/tmp/void-final-seal-index-roundtrip.out
...
VOID_REVIEW_DECISION_CREATED_AT="$CREATED_AT" node "$DECISION_TOOL" \
  ...
  --reason "fixture evidence packet verified and ready for separate award review" >/tmp/void-final-seal-index-decision.out
...
VOID_AWARD_PROPOSAL_CREATED_AT="$CREATED_AT" node "$PROPOSAL_TOOL" \
  ...
  --proposed-wc "100" \
  --reason "fixture verified useful DataNet evidence packet" >/tmp/void-final-seal-index-proposal.out
...
VOID_AWARD_APPROVAL_CREATED_AT="$CREATED_AT" node "$APPROVAL_TOOL" \
  ...
  --reason "fixture award proposal approved for separate ledger write review" >/tmp/void-final-seal-index-approval.out
...
VOID_LEDGER_WRITE_PACKET_CREATED_AT="$CREATED_AT" node "$LEDGER_PACKET_TOOL" \
  ...
  --reason "fixture ready for separate operator ledger append review" >/tmp/void-final-seal-index-ledger-packet.out
...
VOID_LEDGER_APPEND_DRY_RUN_CREATED_AT="$CREATED_AT" node "$DRY_RUN_TOOL" \
  ...
  --reason "fixture dry-run before separate operator append" >/tmp/void-final-seal-index-dry-run.out
...
VOID_LEDGER_APPEND_EXECUTE_PACKET_CREATED_AT="$CREATED_AT" node "$EXECUTE_PACKET_TOOL" \
  ...
  --reason "fixture ready for separate manual append execution" >/tmp/void-final-seal-index-execute-packet.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CREATED_AT="$CREATED_AT" node "$RECEIPT_TOOL" \
  ...
  --reason "fixture receipt binds scratch ledger preview only" >/tmp/void-final-seal-index-receipt.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$CLOSEOUT_TOOL" \
  ...
  --reason "fixture closeout binds scratch receipt only" >/tmp/void-final-seal-index-closeout.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CREATED_AT="$CREATED_AT" node "$REVIEW_INDEX_TOOL" \
  ...
  --reason "fixture index binds scratch closeout only" >/tmp/void-final-seal-index-review-index.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$INDEX_CLOSEOUT_TOOL" \
  ...
  --reason "fixture closeout binds review index only" >/tmp/void-final-seal-index-index-closeout.out

printf '== scratch apply receipt closeout review index closeout final seal index ==\n'
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CLOSEOUT_FINAL_SEAL_INDEX_CREATED_AT="$CREATED_AT" node "$FINAL_SEAL_INDEX_TOOL" \
  ...
  --out "$FINAL_SEAL_INDEX_A" \
  --indexer "operator-final-seal-indexer" \
  --reason "fixture final seal index binds review index closeout only" >/tmp/void-final-seal-index-a.out

VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CLOSEOUT_FINAL_SEAL_INDEX_CREATED_AT="$CREATED_AT" node "$FINAL_SEAL_INDEX_TOOL" \
  --review-index-closeout "$INDEX_CLOSEOUT" \
  --out "$FINAL_SEAL_INDEX_B" \
  --indexer "operator-final-seal-indexer" \
  --reason "fixture final seal index binds review index closeout only" >/tmp/void-final-seal-index-b.out

printf '== final seal index deterministic / binding ==\n'
python3 - "$INDEX_CLOSEOUT" "$FINAL_SEAL_INDEX_A" "$FINAL_SEAL_INDEX_B" "$MARKER" "$INDEX_CLOSEOUT_MARKER" "$REVIEW_INDEX_MARKER" "$CLOSEOUT_MARKER" "$RECEIPT_MARKER" "$SCRATCH_APPLY_MARKER" "$EXECUTE_PACKET_MARKER" <<'PY'
import json
import pathlib
import re
import sys
(index_closeout_path, final_a_path, final_b_path, marker, index_closeout_marker, review_index_marker, closeout_marker, receipt_marker, scratch_marker, execute_marker) = sys.argv[1:11]
index_closeout = json.load(open(index_closeout_path, encoding="utf-8"))
text_a = pathlib.Path(final_a_path).read_text(encoding="utf-8")
text_b = pathlib.Path(final_b_path).read_text(encoding="utf-8")
assert text_a == text_b
record = json.loads(text_a)
assert record["schema"] == "void.datanet.wc.evidence_packet_ledger_append_scratch_apply_receipt_closeout_review_index_closeout_final_seal_index.v1"
assert record["marker"] == marker
assert record["status"] == "scratch_preview_review_index_closeout_final_seal_indexed"
assert re.fullmatch(r"[0-9a-f]{64}", record["scratch_apply_receipt_closeout_review_index_closeout_final_seal_index_id"])
assert record["indexer"] == "operator-final-seal-indexer"
assert record["review_index_closeout"]["path"] == str(pathlib.Path(index_closeout_path).resolve())
assert record["review_index_closeout"]["marker"] == index_closeout_marker
assert record["review_index_closeout"]["scratch_apply_receipt_closeout_review_index_closeout_id"] == index_closeout["scratch_apply_receipt_closeout_review_index_closeout_id"]
assert record["review_index"]["marker"] == review_index_marker
assert record["review_index"]["scratch_apply_receipt_closeout_review_index_id"] == index_closeout["review_index"]["scratch_apply_receipt_closeout_review_index_id"]
assert record["closeout"]["marker"] == closeout_marker
assert record["closeout"]["scratch_apply_receipt_closeout_id"] == index_closeout["closeout"]["scratch_apply_receipt_closeout_id"]
assert record["receipt"]["marker"] == receipt_marker
assert record["receipt"]["scratch_apply_receipt_id"] == index_closeout["receipt"]["scratch_apply_receipt_id"]
assert record["scratch_apply"]["marker"] == scratch_marker
assert record["scratch_apply"]["scratch_apply_id"] == index_closeout["scratch_apply"]["scratch_apply_id"]
assert record["scratch_ledger"]["scratch_ledger_out_hash"] == index_closeout["scratch_ledger"]["scratch_ledger_out_hash"]
assert record["scratch_ledger"]["appended_line_hash"] == index_closeout["scratch_ledger"]["appended_line_hash"]
assert record["scratch_ledger"]["logical_candidate_next_ledger_hash"] == index_closeout["scratch_ledger"]["logical_candidate_next_ledger_hash"]
assert record["source"]["execute_packet_marker"] == execute_marker
assert record["source"]["execute_packet_id"] == index_closeout["source"]["execute_packet_id"]
summary = record["final_seal_summary"]
assert summary["scratch_review_index_closeout_bound"] is True
assert summary["scratch_preview_chain_final_sealed_for_operator_review"] is True
assert summary["operator_review_final_seal_index_only"] is True
...
assert policy["scratch_review_index_closeout_final_seal_index_only"] is True
boundary = record["boundary"]
assert boundary["scratch_review_index_closeout_final_seal_index_only"] is True
...
print("scratch_apply_receipt_closeout_review_index_closeout_final_seal_index_deterministic_green=true")
print("scratch_apply_receipt_closeout_review_index_closeout_final_seal_index_binding_green=true")
PY

printf '== bad review index closeout rejection ==\n'
python3 - "$INDEX_CLOSEOUT" "$BAD_INDEX_CLOSEOUT" <<'PY'
import json, sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
record["status"] = "not_closed"
open(sys.argv[2], "w", encoding="utf-8").write(json.dumps(record, indent=2) + "\n")
PY
if node "$FINAL_SEAL_INDEX_TOOL" \
  --review-index-closeout "$BAD_INDEX_CLOSEOUT" \
  --out "$TMP_DIR/bad-final-seal-index.json" \
  --indexer "operator-final-seal-indexer" \
  --reason "bad closeout should fail" >/tmp/void-final-seal-index-bad-closeout-out 2>/tmp/void-final-seal-index-bad-closeout-err; then
  echo "bad_review_index_closeout_rejection_green=false"
  cat /tmp/void-final-seal-index-bad-closeout-out
  exit 1
fi
grep -Fq "review_index_closeout_status_mismatch" /tmp/void-final-seal-index-bad-closeout-err
echo "bad_review_index_closeout_rejection_green=true"

printf '== bad marker rejection ==\n'
python3 - "$INDEX_CLOSEOUT" "$BAD_MARKER" <<'PY'
import json, sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
...
PY
if node "$FINAL_SEAL_INDEX_TOOL" \
  --review-index-closeout "$BAD_MARKER" \
  --out "$TMP_DIR/bad-marker-final-seal-index.json" \
  --indexer "operator-final-seal-indexer" \
  --reason "bad marker should fail" >/tmp/void-final-seal-index-bad-marker-out 2>/tmp/void-final-seal-index-bad-marker-err; then
  echo "bad_marker_rejection_green=false"
  cat /tmp/void-final-seal-index-bad-marker-out
  exit 1
fi
grep -Fq "review_index_closeout_marker_mismatch" /tmp/void-final-seal-index-bad-marker-err
echo "bad_marker_rejection_green=true"

printf '== marker/source presence ==\n'
grep -Fq "$MARKER" "$FINAL_SEAL_INDEX_TOOL"
grep -Fq "$MARKER" "$DOC"
grep -Fq "$MARKER" "$0"
grep -Fq "final seal index" "$DOC"
grep -Fq "scratch_review_index_closeout_final_seal_index" "$FINAL_SEAL_INDEX_TOOL"

printf '== forbidden WC cap wording scan ==\n'
if grep -RInE '100,000,000[[:space:]]+WC|100000000[[:space:]]+WC|WC[[:space:]]+supply[[:space:]]+cap|work[[:space:]]+credit[[:space:]]+supply[[:space:]]+cap|capped[[:space:]]+at[[:space:]]+100' "$FINAL_SEAL_INDEX_TOOL" "$DOC"; then
