...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-approval-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_APPROVAL_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
echo "== JSON parse / private approval closeout binding =="
python3 - <<PY
import json
...
need(closeout["private_approval_candidate_exists"] is True, "approval candidate should exist")
need(closeout["private_approval_example_exists"] is True, "approval example should exist")
...
need(closeout["example_approval_is_not_effective"] is True, "example approval not effective")
need(closeout["example_approval_is_not_authorization"] is True, "example approval not authorization")
need(closeout["example_approval_is_not_ledger_append"] is True, "example approval not ledger append")
need(closeout["requires_future_effective_operator_approval"] is True, "future effective approval required")
need(closeout["requires_future_ledger_write_authorization"] is True, "future ledger authorization required")
...
need(approval["private_approval_closeout_only"] is True, "private approval closeout only must be true")
...
print("private_operator_award_append_approval_closeout_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_approval_closeout_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_approval_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_APPROVAL_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
