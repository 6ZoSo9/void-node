...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-creation-final-non-execution-rollup-lane-seal-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_creation_final_non_execution_rollup_lane_seal_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_CREATION_FINAL_NON_EXECUTION_ROLLUP_LANE_SEAL_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_CREATION_FINAL_NON_EXECUTION_ROLLUP_LANE_SEAL_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=1b9f7bf4 \
  closeout_cross_box_green=true \
  packet_set_documented=true \
  lane_sealed=true \
  final_non_execution_rollup_closed=true \
  final_non_execution_rollup_lane_sealed=true \
  full_creation_non_execution_chain_sealed=true \
  sealed_lanes_remain_green=true \
...
echo "${u}_CREATION_FINAL_NON_EXECUTION_ROLLUP_LANE_SEAL_PROOF_V1_GREEN"
