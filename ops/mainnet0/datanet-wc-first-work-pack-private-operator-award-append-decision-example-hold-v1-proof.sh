...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-decision-example-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_DECISION_EXAMPLE_HOLD_V1"
...
echo "== JSON parse / private decision example binding =="
python3 - <<PY
import json
...
decision = record["private_operator_decision_example"]
need(decision["decision_example_packet_created"] is True, "decision example packet must be true")
need(decision["private_operator_only"] is True, "private operator only must be true")
need(decision["requires_private_decision_candidate"] is True, "decision candidate required")
need(decision["requires_private_preflight_closeout"] is True, "preflight closeout required")
need(decision["example_decision_is_not_effective"] is True, "example decision not effective")
need(decision["example_decision_is_not_authorization"] is True, "example decision not authorization")
need(decision["example_amount_wc"] == 100, "example amount must be 100")
need(decision["example_amount_is_not_approved_here"] is True, "example amount not approved")
need(decision["example_amount_is_not_supply_limit"] is True, "example amount not supply limit")
need(decision["operator_append_decision_created_here"] is False, "append decision must be false")
need(decision["award_created_here"] is False, "award created must be false")
need(decision["award_approved_here"] is False, "award approved must be false")
need(decision["ledger_write_authorized_here"] is False, "ledger authorization must be false")
need(decision["ledger_line_created_here"] is False, "ledger line must be false")
need(decision["ledger_append_performed_here"] is False, "ledger append must be false")
need(decision["wc_issuance_created_here"] is False, "WC issuance must be false")
need(decision["wc_ledger_write_created_here"] is False, "WC ledger write must be false")
need(decision["reward_created_here"] is False, "reward created must be false")
need(decision["void_transfer_created_here"] is False, "VOID transfer must be false")
need(decision["runtime_append_endpoint_created"] is False, "runtime endpoint must be false")

packet = record["decision_example_packet"]
need(packet["example_decision_is_not_effective"] is True, "packet example decision not effective")
need(packet["example_decision_is_not_authorization"] is True, "packet example decision not authorization")
need(packet["example_decision_is_not_ledger_append"] is True, "packet example decision not ledger append")
need(packet["decision_example_created_here"] is True, "packet example created")
...
db = record["decision_boundary"]
need(db["decision_example_only"] is True, "decision example only must be true")
need(db["operator_append_decision_created"] is False, "operator append decision must be false")
need(db["award_created"] is False, "award created must be false")
need(db["award_approved"] is False, "award approved must be false")
need(db["ledger_write_authorized"] is False, "ledger authorized must be false")
need(db["ledger_line_created"] is False, "ledger line created must be false")
need(db["ledger_append_performed"] is False, "ledger append performed must be false")
need(db["wc_issuance_created"] is False, "WC issuance must be false")
need(db["wc_ledger_write_created"] is False, "WC ledger write must be false")
need(db["reward_created"] is False, "reward must be false")
need(db["void_transfer_created"] is False, "VOID transfer must be false")
...
print("private_operator_award_append_decision_example_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_decision_example_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_decision_example_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_DECISION_EXAMPLE_HOLD_V1_GREEN"
