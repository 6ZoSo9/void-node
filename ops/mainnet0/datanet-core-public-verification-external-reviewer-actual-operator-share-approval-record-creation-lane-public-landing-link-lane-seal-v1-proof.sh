...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-creation-lane-public-landing-link-lane-seal-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_creation_lane_public_landing_link_lane_seal_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_CREATION_LANE_PUBLIC_LANDING_LINK_LANE_SEAL_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_CREATION_LANE_PUBLIC_LANDING_LINK_LANE_SEAL_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=c926d47e \
  public_landing_link_closeout_cross_box_green=true \
  public_landing_link_lane_seal_recorded=true \
  public_landing_link_lane_sealed=true \
  public_landing_link_closed=true \
...
echo "${u}_CREATION_LANE_PUBLIC_LANDING_LINK_LANE_SEAL_PROOF_V1_GREEN"
