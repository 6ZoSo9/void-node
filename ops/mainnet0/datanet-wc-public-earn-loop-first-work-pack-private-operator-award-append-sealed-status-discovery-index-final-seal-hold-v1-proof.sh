...
BRICK="datanet-wc-public-earn-loop-first-work-pack-private-operator-award-append-sealed-status-discovery-index-final-seal-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_STATUS_DISCOVERY_INDEX_FINAL_SEAL_HOLD_V1"
INDEX_CLOSEOUT_MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_STATUS_DISCOVERY_INDEX_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
PUBLIC_JSON="public/public-node/work-credits/datanet-wc-first-work-pack-private-operator-award-append-sealed-status-discovery-index-final-seal-hold-v1.json"
PUBLIC_HTML="public/public-node/work-credits/datanet-wc-first-work-pack-private-operator-award-append-sealed-status-discovery-index-final-seal-hold-v1.html"

CLOSEOUT_HTML="public/public-node/work-credits/datanet-wc-first-work-pack-private-operator-award-append-sealed-status-discovery-index-closeout-audit-rollup-hold-v1.html"
CLOSEOUT_JSON="public/public-node/work-credits/datanet-wc-first-work-pack-private-operator-award-append-sealed-status-discovery-index-closeout-audit-rollup-hold-v1.json"
...
echo "== JSON parse / public discovery index final seal binding =="

for f in "$DOC" "$INDEX_JSON" "$PUBLIC_JSON" "$PUBLIC_HTML" "$CLOSEOUT_HTML" "$CLOSEOUT_JSON" "$PATCH_HTML" "$PATCH_JSON"; do
...
const marker = "$MARKER";
const brick = "$BRICK";
const indexCloseoutMarker = "$INDEX_CLOSEOUT_MARKER";
...
assert(record.seal_type === "work_credits_public_index_discovery_final_seal", "seal type mismatch");
assert(record.source.private_marker_values === "redacted_not_published", "private marker disclosure mismatch");

assert(record.public_chain.index_closeout_marker === indexCloseoutMarker, "index closeout marker binding mismatch");
assert(record.public_chain.index_patch_marker === indexPatchMarker, "index patch marker binding mismatch");
assert(record.final_seal.index_closeout_bound === true, "index closeout final seal binding mismatch");
...
assert(record.final_seal.discovery_final_seal_bound === true, "discovery final seal binding mismatch");
assert(record.final_seal.wc_policy === "unlimited_uncapped_useful_verifiable_work_accounting", "WC policy mismatch");
assert(record.final_seal.execution_state === "not_executed_by_this_public_index_final_seal", "execution state mismatch");
assert(record.final_seal.ledger_append_state === "not_appended_by_this_public_index_final_seal", "ledger append state mismatch");
assert(record.final_seal.public_mutation_state === "not_enabled", "public mutation state mismatch");

assert(closeout.marker === indexCloseoutMarker, "closeout marker mismatch");
assert(closeout.closeout_type === "work_credits_public_index_discovery_patch_closeout_audit_rollup", "closeout type mismatch");
assert(patch.marker === indexPatchMarker, "patch marker mismatch");
...
console.log("public_discovery_index_final_seal_binding_green=true");
NODE
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_STATUS_DISCOVERY_INDEX_FINAL_SEAL_HOLD_V1_GREEN"
