...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-manual-reviewer-candidate-record-entry-manual-approval-hold-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_manual_reviewer_candidate_record_entry_manual_approval_hold_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_MANUAL_REVIEWER_CANDIDATE_RECORD_ENTRY_MANUAL_APPROVAL_HOLD_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_MANUAL_REVIEWER_CANDIDATE_RECORD_ENTRY_MANUAL_APPROVAL_HOLD_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=3562fe6b \
  candidate_record_entry_manual_approval_preflight_cross_box_green=true \
  candidate_record_entry_manual_approval_hold_recorded_now=true \
  candidate_record_entry_manual_approval_lane_held=true \
...
echo "${pfx}candidate_record_entry_manual_approval_hold_recorded_now=true"
echo "${pfx}candidate_record_entry_manual_approval_lane_held=true"
...
echo "${pfx}proof_scope=tiny_static_doc_only_candidate_record_entry_manual_approval_hold_no_candidate_record_no_candidate_named_no_reviewer_selected_no_contact_no_share_no_approval_no_mutation_no_ledger_write_no_wc_award"
echo "${u}_MANUAL_REVIEWER_CANDIDATE_RECORD_ENTRY_MANUAL_APPROVAL_HOLD_PROOF_V1_GREEN"
