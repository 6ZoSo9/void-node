...
name="datanet-core-public-verification-external-reviewer-actual-operator-share-approval-record-manual-reviewer-selection-criteria-v1"
doc="docs/public/public-node-${name}.md"
pfx="datanet_core_public_verification_external_reviewer_actual_operator_share_approval_record_manual_reviewer_selection_criteria_"
u="VOID_DATANET_CORE_PUBLIC_VERIFICATION_EXTERNAL_REVIEWER_ACTUAL_OPERATOR_SHARE_APPROVAL_RECORD"

echo "marker=${u}_MANUAL_REVIEWER_SELECTION_CRITERIA_PROOF_V1"
echo "head=$(git rev-parse --short HEAD)"

test -f "$doc"
grep -q "${u}_MANUAL_REVIEWER_SELECTION_CRITERIA_DOC_V1" "$doc"
...
for suffix in \
  ...
  static_only=true \
  base_head=ad9a6dca \
  operator_review_checklist_cross_box_green=true \
  reviewer_selection_criteria_recorded_now=true \
  reviewer_independence_required=true \
  public_artifact_review_required=true \
  no_private_credential_review_required=true \
  no_private_command_review_required=true \
  no_token_review_required=true \
  no_secret_review_required=true \
  disagreement_allowed=true \
  uncertainty_allowed=true \
  manual_operator_acceptance_required_for_future_result=true \
...
echo "${pfx}reviewer_selection_criteria_recorded_now=true"
...
echo "${pfx}proof_scope=tiny_static_doc_only_selection_criteria_no_reviewer_selected_no_contact_no_share_no_approval_no_mutation_no_ledger_write_no_wc_award"
echo "${u}_MANUAL_REVIEWER_SELECTION_CRITERIA_PROOF_V1_GREEN"
