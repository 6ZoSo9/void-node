...
BRICK="datanet-wc-held-review-decision-packet-candidate-hold-v1"
...
PROOF="ops/mainnet0/void-datanet-wc-held-review-decision-packet-candidate-hold-v1-proof.sh"
...
marker = "VOID_DATANET_WC_HELD_REVIEW_DECISION_PACKET_CANDIDATE_HOLD_V1"
entry_id = "datanet-wc-held-review-decision-packet-candidate-hold-v1"
...
assert entry["path"] == "/public-node/work-credits/datanet-wc-held-review-decision-packet-candidate-hold-v1.json"
assert entry["json"] == "datanet-wc-held-review-decision-packet-candidate-hold-v1.json"
assert entry["lane"] == "work_credits"
assert entry["scope"] == "datanet_held_review_decision_packet_candidate"
assert entry["decision_packet_candidate_only"] is True
...
card = json.loads(Path("public/public-node/work-credits/datanet-wc-held-review-decision-packet-candidate-hold-v1.json").read_text())
assert card["schema"] == "void.public_node.work_credits.datanet_wc_held_review_decision_packet_candidate_hold.v1"
assert card["id"] == entry_id
assert card["status"] == "hold"
assert card["marker"] == marker
assert card["candidate_status"] == "held_review_decision_packet_candidate_only"

shape = card["candidate_decision_packet_shape"]
assert shape["decision_packet_kind"] == "datanet_wc_held_review_decision_packet"
assert shape["decision_packet_status"] == "held_not_created"
assert shape["decision"] == "none"
assert shape["eligible_for_wc"] is False
assert shape["review_basis"]["public_safe_evidence_reviewed"] is False
assert shape["review_basis"]["private_material_required"] is False

for key in [
    "participant_identifier_public",
    "operator_identifier_public",
    "reviewer_identifier_public",
    "operator_assignment_public"
]:
    assert shape["identity_boundary"][key] is False, key

decision_state = shape["decision_state"]
for key in [
    "review_decision_active",
    "decision_packet_created",
    "decision_signed",
    "decision_approved",
    "operator_action_enabled"
]:
    assert decision_state[key] is False, key
...
print("held_review_decision_packet_candidate_binding_green=true")
PY
...
files = [
    ...
    Path("public/public-node/work-credits/datanet-wc-held-review-decision-packet-candidate-hold-v1.json"),
    Path("docs/public-node/work-credits/datanet-wc-held-review-decision-packet-candidate-hold-v1.md"),
]
