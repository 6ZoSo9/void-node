...
POLISH_BRICK="mainnet0-validator-public-node-root-reviewer-entrypoint-polish-hold-v1"
POLISH_MARKER="VOID_MAINNET0_VALIDATOR_PUBLIC_NODE_ROOT_REVIEWER_ENTRYPOINT_POLISH_HOLD_V1"
...
POLISH_JSON="public/public-node/validators/${POLISH_BRICK}.json"
...
python3 -m json.tool "$POLISH_JSON" >/dev/null
...
grep -F "$POLISH_MARKER" "$POLISH_JSON" >/dev/null
...
grep -F '"root_entrypoint_polish_only": true' "$POLISH_JSON" >/dev/null
grep -F '"runtime_visibility_only": true' "$FINAL_RUNTIME_JSON" >/dev/null
echo "source_presence_green=true"

echo "== root runtime binding =="
python3 - <<'PYCHECK'
import json
...
final_html_route = "/public-node/validators/mainnet0-validator-candidate-public-visibility-reviewer-final-seal-html-card-hold-v1.html"
runtime_route = f"/public-node/validators/{brick}.json"
polish_route = "/public-node/validators/mainnet0-validator-public-node-root-reviewer-entrypoint-polish-hold-v1.json"

root_matches = [r for r in root.get("routes", []) if r.get("route") == final_html_route]
...
section_matches = [r for r in section.get("routes", []) if r.get("route") == runtime_route]
assert len(section_matches) == 1, section_matches
route = section_matches[0]
assert route["marker"] == marker
assert route["root_entrypoint_route"] == final_html_route
assert route["source_root_entrypoint_polish_route"] == polish_route
assert route["public_safe"] is True
assert route["read_only"] is True
assert route["runtime_visibility_only"] is True
assert route["root_entrypoint_visibility_only"] is True
...
assert runtime["root_entrypoint_route"] == final_html_route
assert runtime["source_root_entrypoint_polish_route"] == polish_route
assert runtime["public_safe"] is True
assert runtime["read_only"] is True
assert runtime["runtime_visibility_only"] is True
assert runtime["root_entrypoint_visibility_only"] is True
...
for container in [root_route, route]:
    for flag in [
        "public_submit_enabled",
        "stake_lock_enabled",
        "candidate_registration_enabled",
...
print("validator_public_node_root_reviewer_entrypoint_runtime_visibility_binding_green=true")
PYCHECK
