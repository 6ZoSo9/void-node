...
BRICK="datanet-wc-first-work-pack-private-operator-ledger-write-authorization-candidate-hold-v1"
...
echo "== JSON parse / private authorization candidate binding =="
python3 - <<PY
import json
...
candidate = record["private_operator_authorization_candidate"]
need(candidate["authorization_candidate_shape_created"] is True, "authorization candidate shape must be true")
need(candidate["private_operator_only"] is True, "private operator only must be true")
need(candidate["requires_private_approval_closeout"] is True, "private approval closeout required")
...
need(candidate["requires_future_ledger_line_creation"] is True, "future ledger line required")
...
template = record["authorization_candidate_template"]
need(template["authorization_candidate_shape_created_here"] is True, "template candidate shape created")
...
need(auth["authorization_candidate_shape_only"] is True, "authorization candidate shape only must be true")
...
print("private_operator_ledger_write_authorization_candidate_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_ledger_write_authorization_candidate_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_ledger_write_authorization_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_LEDGER_WRITE_AUTHORIZATION_CANDIDATE_HOLD_V1_GREEN"
