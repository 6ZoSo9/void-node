...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate-chain-terminal-status-rollup-closeout-final-seal-index"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_CHAIN_TERMINAL_STATUS_ROLLUP_CLOSEOUT_FINAL_SEAL_INDEX_HOLD_V1"

SRC_LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate-chain-terminal-status-rollup-closeout"
SRC_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_CHAIN_TERMINAL_STATUS_ROLLUP_CLOSEOUT_HOLD_V1"
...
BAD_AUTH="$(mktemp)"
BAD_INDEX="$(mktemp)"
trap 'rm -f "$OUT1" "$OUT2" "$DISPLAY_OUT" "$VALIDATOR" "$BAD_MARKER" "$BAD_HASH" "$BAD_BOUNDARY" "$BAD_AUTH" "$BAD_INDEX"' EXIT
...
if (data.artifact.status !== 'read_only_operator_decision_creation_readiness_gate_chain_terminal_status_rollup_closeout_final_seal_index_ready') fail('status mismatch');
if (data.final_seal_index_hash !== hashCanonical(data.artifact)) fail('final seal index hash mismatch');

const index = data.artifact.terminal_status_rollup_closeout_final_seal_index ?? {};
const trueFields = [
...
echo "== operator decision creation readiness gate chain terminal status rollup closeout final seal index deterministic / binding =="
echo "operator_decision_creation_readiness_gate_chain_terminal_status_rollup_closeout_final_seal_index_deterministic_green=true"
echo "operator_decision_creation_readiness_gate_chain_terminal_status_rollup_closeout_final_seal_index_binding_green=true"
...
node --input-type=module - "$OUT1" "$BAD_INDEX" <<'NODE'
import { readFileSync, writeFileSync } from 'node:fs';
const data = JSON.parse(readFileSync(process.argv[2], 'utf8'));
data.artifact.terminal_status_rollup_closeout_final_seal_index.terminal_status_rollup_closeout_final_seal_index_created = false;
data.final_seal_index_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE

if MARKER="$MARKER" SRC_DOC="$SRC_DOC" SRC_PROOF="$SRC_PROOF" SRC_TOOL="$SRC_TOOL" node "$VALIDATOR" "$BAD_INDEX" >/dev/null 2>&1; then
  echo "bad_final_seal_index_rejection_failed=true"
  exit 1
fi
...
const data = JSON.parse(readFileSync(process.argv[2], 'utf8'));
const field = process.argv[4];
data.artifact.terminal_status_rollup_closeout_final_seal_index[field] = true;
data.final_seal_index_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE
