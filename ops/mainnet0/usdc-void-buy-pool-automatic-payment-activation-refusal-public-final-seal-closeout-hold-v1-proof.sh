...
name="usdc-void-buy-pool-automatic-payment-activation-refusal-public-final-seal-closeout-hold-v1"
marker="VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_REFUSAL_PUBLIC_FINAL_SEAL_CLOSEOUT_HOLD_V1"

doc="docs/public/$name.md"
fixture="fixtures/public/$name.json"
proof="ops/mainnet0/$name-proof.sh"

dependency_doc="docs/public/usdc-void-buy-pool-automatic-payment-activation-refusal-public-final-seal-hold-v1.md"
dependency_fixture="fixtures/public/usdc-void-buy-pool-automatic-payment-activation-refusal-public-final-seal-hold-v1.json"

echo "VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_REFUSAL_PUBLIC_FINAL_SEAL_CLOSEOUT_HOLD_V1_PROOF_BEGIN"

test -f "$doc"
test -f "$fixture"
test -f "$proof"
echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_files_exist=true"

test -f "$dependency_doc"
test -f "$dependency_fixture"
echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_dependency_exists=true"

grep -Fq "$marker" "$doc"
grep -Fq "$marker" "$fixture"
grep -Fq "$marker" "$proof"
echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_marker_green=true"
...
if git grep -Ei "private/operator|docs/private|fixtures/private" -- "$doc" "$fixture" >"$cut_private_leak_file" 2>/dev/null; then
  ...
  echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_private_leak_absent=false"
  exit 1
else
  echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_private_leak_absent=true"
fi
...
if git grep -F "$marker" -- src >/tmp/void-public-safe-summary-closeout-runtime-route.txt 2>/dev/null; then
  ...
  echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_no_runtime_route=false"
  exit 1
else
  echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_no_runtime_route=true"
fi
...
assert data["schema"] == "usdc_void_buy_pool_automatic_payment_activation_refusal_public_final_seal_closeout_hold_v1"
assert data["visibility"] == "public_safe_final_seal_closeout"
assert data["status"] == "activation_refusal_public_final_seal_closeout_hold"
...
print("automatic_payment_activation_refusal_public_final_seal_closeout_hold_json_semantics_green=true")
PY

echo "automatic_payment_activation_refusal_public_final_seal_closeout_hold_authority_false_green=true"
echo "VOID_USDC_VOID_BUY_POOL_AUTOMATIC_PAYMENT_ACTIVATION_REFUSAL_PUBLIC_FINAL_SEAL_CLOSEOUT_HOLD_V1_GREEN"
