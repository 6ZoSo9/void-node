...
DOC="docs/datanet/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index-closeout-hold-v1.md"
REVIEW_INDEX_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index-hold-v1-proof.sh"
...
INDEX_CLOSEOUT_A="$TMP_DIR/review-index-closeout-a.json"
INDEX_CLOSEOUT_B="$TMP_DIR/review-index-closeout-b.json"
BAD_INDEX="$TMP_DIR/bad-review-index.json"
BAD_MARKER="$TMP_DIR/bad-marker-review-index.json"
...
for tool in \
  ...
  "$REVIEW_INDEX_TOOL" "$INDEX_CLOSEOUT_TOOL"; do
  test -x "$tool"
  node --check "$tool"
done
bash -n "$REVIEW_INDEX_PROOF"

printf '== scratch apply receipt closeout review index source proof ==\n'
bash "$REVIEW_INDEX_PROOF"

printf '== create scratch apply receipt closeout review index ==\n'
VOID_PACKET_CREATED_AT="$CREATED_AT" node "$ROUNDTRIP_TOOL" \
  ...
  --summary-out "$SUMMARY" >/tmp/void-scratch-review-index-closeout-roundtrip.out
...
VOID_REVIEW_DECISION_CREATED_AT="$CREATED_AT" node "$DECISION_TOOL" \
  ...
  --reason "fixture evidence packet verified and ready for separate award review" >/tmp/void-scratch-review-index-closeout-decision.out
...
VOID_AWARD_PROPOSAL_CREATED_AT="$CREATED_AT" node "$PROPOSAL_TOOL" \
  ...
  --proposed-wc "100" \
  --reason "fixture verified useful DataNet evidence packet" >/tmp/void-scratch-review-index-closeout-proposal.out
...
VOID_AWARD_APPROVAL_CREATED_AT="$CREATED_AT" node "$APPROVAL_TOOL" \
  ...
  --reason "fixture award proposal approved for separate ledger write review" >/tmp/void-scratch-review-index-closeout-approval.out
...
VOID_LEDGER_WRITE_PACKET_CREATED_AT="$CREATED_AT" node "$LEDGER_PACKET_TOOL" \
  ...
  --reason "fixture ready for separate operator ledger append review" >/tmp/void-scratch-review-index-closeout-ledger-packet.out
...
VOID_LEDGER_APPEND_DRY_RUN_CREATED_AT="$CREATED_AT" node "$DRY_RUN_TOOL" \
  ...
  --reason "fixture dry-run before separate operator append" >/tmp/void-scratch-review-index-closeout-dry-run.out
...
VOID_LEDGER_APPEND_EXECUTE_PACKET_CREATED_AT="$CREATED_AT" node "$EXECUTE_PACKET_TOOL" \
  ...
  --reason "fixture ready for separate manual append execution" >/tmp/void-scratch-review-index-closeout-execute-packet.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CREATED_AT="$CREATED_AT" node "$RECEIPT_TOOL" \
  ...
  --reason "fixture receipt binds scratch ledger preview only" >/tmp/void-scratch-review-index-closeout-receipt.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$CLOSEOUT_TOOL" \
  ...
  --reason "fixture closeout binds scratch receipt only" >/tmp/void-scratch-review-index-closeout-closeout.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CREATED_AT="$CREATED_AT" node "$REVIEW_INDEX_TOOL" \
  ...
  --reason "fixture index binds scratch closeout only" >/tmp/void-scratch-review-index-closeout-review-index.out

printf '== scratch apply receipt closeout review index closeout ==\n'
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$INDEX_CLOSEOUT_TOOL" \
  ...
  --out "$INDEX_CLOSEOUT_A" \
  --closer "operator-review-closeout" \
  --reason "fixture closeout binds review index only" >/tmp/void-scratch-review-index-closeout-a.out

VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$INDEX_CLOSEOUT_TOOL" \
  --review-index "$REVIEW_INDEX" \
  --out "$INDEX_CLOSEOUT_B" \
  --closer "operator-review-closeout" \
  --reason "fixture closeout binds review index only" >/tmp/void-scratch-review-index-closeout-b.out

printf '== review index closeout deterministic / binding ==\n'
python3 - "$REVIEW_INDEX" "$INDEX_CLOSEOUT_A" "$INDEX_CLOSEOUT_B" "$MARKER" "$REVIEW_INDEX_MARKER" "$CLOSEOUT_MARKER" "$RECEIPT_MARKER" "$SCRATCH_APPLY_MARKER" "$EXECUTE_PACKET_MARKER" <<'PY'
import json
import pathlib
import re
import sys
review_index_path, closeout_a_path, closeout_b_path, marker, review_marker, closeout_marker, receipt_marker, scratch_marker, execute_marker = sys.argv[1:10]
review_index = json.load(open(review_index_path, encoding="utf-8"))
...
assert record["schema"] == "void.datanet.wc.evidence_packet_ledger_append_scratch_apply_receipt_closeout_review_index_closeout.v1"
assert record["marker"] == marker
assert record["status"] == "scratch_receipt_closeout_review_index_closed_for_operator_review"
assert re.fullmatch(r"[0-9a-f]{64}", record["scratch_apply_receipt_closeout_review_index_closeout_id"])
assert record["closer"] == "operator-review-closeout"
assert record["review_index"]["path"] == str(pathlib.Path(review_index_path).resolve())
assert record["review_index"]["marker"] == review_marker
assert record["review_index"]["scratch_apply_receipt_closeout_review_index_id"] == review_index["scratch_apply_receipt_closeout_review_index_id"]
assert record["closeout"]["marker"] == closeout_marker
assert record["closeout"]["scratch_apply_receipt_closeout_id"] == review_index["closeout"]["scratch_apply_receipt_closeout_id"]
assert record["receipt"]["marker"] == receipt_marker
assert record["receipt"]["scratch_apply_receipt_id"] == review_index["receipt"]["scratch_apply_receipt_id"]
assert record["scratch_apply"]["marker"] == scratch_marker
assert record["scratch_apply"]["scratch_apply_id"] == review_index["scratch_apply"]["scratch_apply_id"]
assert record["scratch_ledger"]["scratch_ledger_out_hash"] == review_index["scratch_ledger"]["scratch_ledger_out_hash"]
assert record["scratch_ledger"]["appended_line_hash"] == review_index["scratch_ledger"]["appended_line_hash"]
assert record["scratch_ledger"]["logical_candidate_next_ledger_hash"] == review_index["scratch_ledger"]["logical_candidate_next_ledger_hash"]
assert record["source"]["execute_packet_marker"] == execute_marker
assert record["source"]["execute_packet_id"] == review_index["source"]["execute_packet_id"]
summary = record["closeout_summary"]
assert summary["scratch_receipt_closeout_review_index_bound"] is True
assert summary["scratch_preview_chain_review_index_closed"] is True
assert summary["operator_review_index_closeout_only"] is True
...
assert policy["scratch_receipt_closeout_review_index_closeout_only"] is True
boundary = record["boundary"]
assert boundary["scratch_apply_receipt_closeout_review_index_closeout_only"] is True
...
print("scratch_apply_receipt_closeout_review_index_closeout_deterministic_green=true")
print("scratch_apply_receipt_closeout_review_index_closeout_binding_green=true")
PY

printf '== bad review index rejection ==\n'
python3 - "$REVIEW_INDEX" "$BAD_INDEX" <<'PY'
import json, sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
record["status"] = "not_indexed"
open(sys.argv[2], "w", encoding="utf-8").write(json.dumps(record, indent=2) + "\n")
PY
if node "$INDEX_CLOSEOUT_TOOL" \
  --review-index "$BAD_INDEX" \
  --out "$TMP_DIR/bad-index-closeout.json" \
  --closer "operator-review-closeout" \
  --reason "bad index should fail" >/tmp/void-scratch-review-index-closeout-bad-index-out 2>/tmp/void-scratch-review-index-closeout-bad-index-err; then
  echo "bad_review_index_rejection_green=false"
  cat /tmp/void-scratch-review-index-closeout-bad-index-out
  exit 1
fi
grep -Fq "review_index_status_mismatch" /tmp/void-scratch-review-index-closeout-bad-index-err
echo "bad_review_index_rejection_green=true"

printf '== bad marker rejection ==\n'
python3 - "$REVIEW_INDEX" "$BAD_MARKER" <<'PY'
import json, sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
...
PY
if node "$INDEX_CLOSEOUT_TOOL" \
  --review-index "$BAD_MARKER" \
  --out "$TMP_DIR/bad-marker-index-closeout.json" \
  --closer "operator-review-closeout" \
  --reason "bad marker should fail" >/tmp/void-scratch-review-index-closeout-bad-marker-out 2>/tmp/void-scratch-review-index-closeout-bad-marker-err; then
  echo "bad_marker_rejection_green=false"
  cat /tmp/void-scratch-review-index-closeout-bad-marker-out
  exit 1
fi
grep -Fq "review_index_marker_mismatch" /tmp/void-scratch-review-index-closeout-bad-marker-err
echo "bad_marker_rejection_green=true"

printf '== marker/source presence ==\n'
grep -Fq "$MARKER" "$INDEX_CLOSEOUT_TOOL"
grep -Fq "$MARKER" "$DOC"
grep -Fq "$MARKER" "$0"
grep -Fq "scratch apply receipt closeout review index closeout" "$DOC"
grep -Fq "scratch_apply_receipt_closeout_review_index_closeout" "$INDEX_CLOSEOUT_TOOL"

printf '== forbidden WC cap wording scan ==\n'
if grep -RInE '100,000,000[[:space:]]+WC|100000000[[:space:]]+WC|WC[[:space:]]+supply[[:space:]]+cap|work[[:space:]]+credit[[:space:]]+supply[[:space:]]+cap|capped[[:space:]]+at[[:space:]]+100' "$INDEX_CLOSEOUT_TOOL" "$DOC"; then
