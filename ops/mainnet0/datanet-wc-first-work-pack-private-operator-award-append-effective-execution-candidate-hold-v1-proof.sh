...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-effective-execution-candidate-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_CANDIDATE_HOLD_V1"
...
echo "== JSON parse / private effective execution candidate binding =="
...
for (const obj of [example, privateRecord]) {
  ...
  assert(obj.effective_execution_candidate.authorization_closeout_state === "bound", "authorization closeout state mismatch");
  assert(obj.effective_execution_candidate.effective_execution_state === "candidate_not_executed", "effective execution state mismatch");
  assert(obj.effective_execution_candidate.ledger_append_state === "not_appended", "ledger append state mismatch");
  assert(obj.effective_execution_candidate.wc_policy === "unlimited_uncapped_useful_verifiable_work_accounting", "WC policy mismatch");
...
console.log("private_operator_effective_execution_candidate_binding_green=true");
NODE

echo "== forbidden public mutation scan =="
if git grep -n "$MARKER" -- public >/tmp/void-private-effective-execution-candidate-public-leak.txt 2>/dev/null; then
  cat /tmp/void-private-effective-execution-candidate-public-leak.txt
  echo "private_effective_execution_candidate_marker_not_in_public_tree_green=false"
  exit 1
fi
rm -f /tmp/void-private-effective-execution-candidate-public-leak.txt
echo "private_effective_execution_candidate_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_EFFECTIVE_EXECUTION_CANDIDATE_HOLD_V1_GREEN"
