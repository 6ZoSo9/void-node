...
BRICK="mainnet0-validator-public-visibility-root-to-reviewer-chain-audit-rollup-html-card-hold-v1"
MARKER="VOID_MAINNET0_VALIDATOR_PUBLIC_VISIBILITY_ROOT_TO_REVIEWER_CHAIN_AUDIT_ROLLUP_HTML_CARD_HOLD_V1"
...
echo "== source audit presence =="
grep -F "$AUDIT_MARKER" "$AUDIT_JSON" >/dev/null
grep -F '"audit_rollup_only": true' "$AUDIT_JSON" >/dev/null
grep -F '"root_to_reviewer_chain_audit_only": true' "$AUDIT_JSON" >/dev/null
grep -F '"public_submit_enabled": false' "$AUDIT_JSON" >/dev/null
grep -F '"active_admission_enabled": false' "$AUDIT_JSON" >/dev/null
echo "source_audit_green=true"

echo "== html source =="
test -f "$CARD_HTML"
grep -F "$MARKER" "$CARD_HTML" >/dev/null
grep -F "The public-node root index points to the validator reviewer final seal HTML card." "$CARD_HTML" >/dev/null
...
assert card["preferred_reviewer_entrypoint"] == preferred
...
print("validator_public_visibility_root_to_reviewer_chain_audit_rollup_html_binding_green=true")
PYCHECK
...
paths = [
    ...
    Path("public/public-node/validators/mainnet0-validator-public-visibility-root-to-reviewer-chain-audit-rollup-html-card-hold-v1.json"),
    Path("public/public-node/validators/mainnet0-validator-public-visibility-root-to-reviewer-chain-audit-rollup-html-card-hold-v1.html"),
]
