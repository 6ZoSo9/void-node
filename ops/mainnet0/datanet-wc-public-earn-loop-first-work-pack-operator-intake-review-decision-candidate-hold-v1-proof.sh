...
BRICK="datanet-wc-public-earn-loop-first-work-pack-operator-intake-review-decision-candidate-hold-v1"
MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_OPERATOR_INTAKE_REVIEW_DECISION_CANDIDATE_HOLD_V1"
KEY="datanet_wc_public_earn_loop_first_work_pack_operator_intake_review_decision_candidate"

OP_REVIEW_CANDIDATE_MARKER="VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_OPERATOR_INTAKE_REVIEW_PACKET_CANDIDATE_HOLD_V1"
...
decision = record["operator_decision_candidate"]
need(decision["decision_candidate_created"] is True, "decision candidate should be true")
need(decision["decision_options_defined"] == ["accept", "reject", "needs_more_info"], "decision options mismatch")
need(decision["deterministic_decision_rules_defined"] is True, "deterministic rules must be true")
need(decision["operator_review_required"] is True, "operator review required must be true")
need(decision["decision_fixture_created_here"] is False, "decision fixture must be false")
need(decision["accepted_here"] is False, "accepted_here must be false")
need(decision["rejected_here"] is False, "rejected_here must be false")
need(decision["needs_more_info_here"] is False, "needs_more_info_here must be false")
need(decision["wc_amount_approved_here"] is False, "WC amount approved must be false")
need(decision["ledger_write_candidate_created_here"] is False, "ledger write candidate must be false")
need(decision["runtime_decision_endpoint_created"] is False, "runtime endpoint must be false")

boundary = record["decision_boundary"]
need(boundary["decision_candidate_only"] is True, "decision candidate only must be true")
...
template = record["operator_review_decision_candidate_template"]
for option in ("accept", "reject", "needs_more_info"):
    need(option in template["decision_options"], f"missing decision option {option}")
    need(template["decision_options"][option]["allowed_here"] is True, f"{option} should be allowed here as candidate")
    need(template["decision_options"][option]["wc_amount_approved_here"] is False, f"{option} WC approval must be false")
    need(template["decision_options"][option]["ledger_write_candidate_created_here"] is False, f"{option} ledger candidate must be false")

need(template["decision_fixture_created_here"] is False, "template decision fixture must be false")
...
print("operator_intake_review_decision_candidate_binding_green=true")
PY
...
echo "== result =="
echo "VOID_DATANET_WC_PUBLIC_EARN_LOOP_FIRST_WORK_PACK_OPERATOR_INTAKE_REVIEW_DECISION_CANDIDATE_HOLD_V1_GREEN"
