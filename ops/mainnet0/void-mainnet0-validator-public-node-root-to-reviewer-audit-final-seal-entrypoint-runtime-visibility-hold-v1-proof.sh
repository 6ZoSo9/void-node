...
brick = "mainnet0-validator-public-node-root-to-reviewer-audit-final-seal-entrypoint-runtime-visibility-hold-v1"
marker = "VOID_MAINNET0_VALIDATOR_PUBLIC_NODE_ROOT_TO_REVIEWER_AUDIT_FINAL_SEAL_ENTRYPOINT_RUNTIME_VISIBILITY_HOLD_V1"
...
doc_path = Path("docs/public-node/validators/mainnet0-validator-public-node-root-to-reviewer-audit-final-seal-entrypoint-runtime-visibility-hold-v1.md")
proof_path = Path("ops/mainnet0/void-mainnet0-validator-public-node-root-to-reviewer-audit-final-seal-entrypoint-runtime-visibility-hold-v1-proof.sh")
...
ensure(root_index[root_key].get("route") == polish_route, "root index polish route mismatch")
...
ensure(polish.get("marker") == polish_marker, "source polish marker mismatch")
ensure(polish.get("preferred_browser_visible_reviewer_route") == final_html_route, "source polish preferred route mismatch")
ensure(section_index.get(runtime_key, {}).get("marker") == marker, "validators index runtime marker mismatch")
ensure(section_index[runtime_key].get("route") == runtime_route, "validators index runtime route mismatch")
...
ensure(runtime.get("source_final_seal_html_card_route") == final_html_route, "runtime html route mismatch")
ensure(runtime.get("source_final_seal_html_card_json_route") == final_html_json_route, "runtime html json route mismatch")
ensure(runtime.get("source_final_seal_html_runtime_visibility_route") == final_runtime_route, "runtime final html runtime route mismatch")
rv = runtime.get("runtime_visibility", {})
ensure(rv.get("root_index_entry_present") is True, "root visibility flag must be true")
ensure(rv.get("validator_section_entry_present") is True, "section visibility flag must be true")
ensure(rv.get("preferred_browser_visible_reviewer_route_present") is True, "preferred route visibility flag must be true")
ensure(rv.get("runtime_mutation_enabled") is False, "runtime mutation must remain false")
print("validator_public_node_root_to_reviewer_audit_final_seal_entrypoint_runtime_visibility_binding_green=true")
...
print("== result ==")
print("VOID_MAINNET0_VALIDATOR_PUBLIC_NODE_ROOT_TO_REVIEWER_AUDIT_FINAL_SEAL_ENTRYPOINT_RUNTIME_VISIBILITY_HOLD_V1_GREEN")
PYPROOF
