...
LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate-chain-status-rollup"
MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_CHAIN_STATUS_ROLLUP_HOLD_V1"

SRC_LANE="datanet-wc-evidence-packet-ledger-append-scratch-preview-operator-decision-creation-readiness-gate-closeout-final-seal-index-closeout"
SRC_MARKER="VOID_DATANET_WC_EVIDENCE_PACKET_LEDGER_APPEND_SCRATCH_PREVIEW_OPERATOR_DECISION_CREATION_READINESS_GATE_CLOSEOUT_FINAL_SEAL_INDEX_CLOSEOUT_HOLD_V1"
...
if (data.artifact.status !== 'read_only_operator_decision_creation_readiness_gate_chain_status_rollup_ready') fail('status mismatch');
if (data.rollup_hash !== hashCanonical(data.artifact)) fail('rollup hash mismatch');

const rollup = data.artifact.chain_status_rollup ?? {};
const trueRollupFields = [
...
for (const field of trueRollupFields) {
...
echo "== operator decision creation readiness gate chain status rollup deterministic / binding =="
echo "operator_decision_creation_readiness_gate_chain_status_rollup_deterministic_green=true"
echo "operator_decision_creation_readiness_gate_chain_status_rollup_binding_green=true"
...
import { readFileSync, writeFileSync } from 'node:fs';
const data = JSON.parse(readFileSync(process.argv[2], 'utf8'));
data.artifact.chain_status_rollup.chain_status_rollup_created = false;
data.rollup_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE

if MARKER="$MARKER" SRC_DOC="$SRC_DOC" SRC_PROOF="$SRC_PROOF" SRC_TOOL="$SRC_TOOL" node "$VALIDATOR" "$BAD_ROLLUP" >/dev/null 2>&1; then
  echo "bad_rollup_rejection_failed=true"
  exit 1
fi
echo "bad_rollup_rejection_green=true"
...
const data = JSON.parse(readFileSync(process.argv[2], 'utf8'));
const field = process.argv[4];
data.artifact.chain_status_rollup[field] = true;
data.rollup_hash = '0'.repeat(64);
writeFileSync(process.argv[3], `${JSON.stringify(data, null, 2)}\n`);
NODE
