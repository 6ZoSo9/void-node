...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-preflight-closeout-audit-rollup-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_PREFLIGHT_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1"
...
echo "== JSON parse / private closeout binding =="
python3 - <<PY
import json
...
need(closeout["private_preflight_template_exists"] is True, "preflight template should exist")
need(closeout["private_preflight_example_exists"] is True, "preflight example should exist")
...
need(closeout["root_public_index_mutated"] is False, "root index mutation must be false")
need(closeout["requires_future_operator_decision"] is True, "future operator decision required")
...
need(append["private_preflight_closeout_only"] is True, "private preflight closeout only must be true")
...
print("private_operator_award_append_preflight_closeout_binding_green=true")
PY
...
if grep -R "$MARKER" public/public-node docs/public-node examples/public-node 2>/dev/null; then
  echo "private_closeout_marker_leaked_to_public_tree=true"
  exit 1
fi
echo "private_closeout_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_PREFLIGHT_CLOSEOUT_AUDIT_ROLLUP_HOLD_V1_GREEN"
