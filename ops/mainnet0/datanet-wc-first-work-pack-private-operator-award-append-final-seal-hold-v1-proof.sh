...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-final-seal-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_FINAL_SEAL_HOLD_V1"
...
echo "== JSON parse / private award append final seal binding =="
...
assert(record.seal_type === "private_operator_award_append_final_seal", "seal type mismatch");
...
assert(record.final_seal.chain_closeout_bound === true, "chain closeout final seal mismatch");
assert(record.final_seal.effective_execution_final_seal_bound === true, "effective final seal mismatch");
assert(record.final_seal.public_earn_loop_bound === true, "public earn loop final seal mismatch");
assert(record.final_seal.private_operator_chain_bound === true, "private operator chain final seal mismatch");
...
console.log("private_operator_award_append_final_seal_binding_green=true");
NODE

echo "== forbidden public mutation scan =="
if git grep -n "$MARKER" -- public >/tmp/void-private-award-append-final-seal-public-leak.txt 2>/dev/null; then
  cat /tmp/void-private-award-append-final-seal-public-leak.txt
  echo "private_award_append_final_seal_marker_not_in_public_tree_green=false"
  exit 1
fi
rm -f /tmp/void-private-award-append-final-seal-public-leak.txt
echo "private_award_append_final_seal_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_FINAL_SEAL_HOLD_V1_GREEN"
