...
INDEX_TOOL="tools/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index.mjs"
DOC="docs/datanet/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-review-index-hold-v1.md"
CLOSEOUT_PROOF="ops/mainnet0/datanet-wc-evidence-packet-ledger-append-scratch-apply-receipt-closeout-hold-v1-proof.sh"
...
INDEX_A="$TMP_DIR/index-a.json"
INDEX_B="$TMP_DIR/index-b.json"
BAD_CLOSEOUT="$TMP_DIR/bad-closeout.json"
BAD_MARKER="$TMP_DIR/bad-marker-closeout.json"
...
for tool in \
  ...
  "$EXECUTE_PACKET_TOOL" "$SCRATCH_APPLY_TOOL" "$RECEIPT_TOOL" "$CLOSEOUT_TOOL" "$INDEX_TOOL"; do
  test -x "$tool"
  node --check "$tool"
done
bash -n "$CLOSEOUT_PROOF"

printf '== scratch apply receipt closeout source proof ==\n'
bash "$CLOSEOUT_PROOF"

printf '== create scratch apply receipt closeout ==\n'
VOID_PACKET_CREATED_AT="$CREATED_AT" node "$ROUNDTRIP_TOOL" \
  ...
  --summary-out "$SUMMARY" >/tmp/void-scratch-closeout-index-roundtrip.out
...
VOID_REVIEW_DECISION_CREATED_AT="$CREATED_AT" node "$DECISION_TOOL" \
  ...
  --reason "fixture evidence packet verified and ready for separate award review" >/tmp/void-scratch-closeout-index-decision.out
...
VOID_AWARD_PROPOSAL_CREATED_AT="$CREATED_AT" node "$PROPOSAL_TOOL" \
  ...
  --proposed-wc "100" \
  --reason "fixture verified useful DataNet evidence packet" >/tmp/void-scratch-closeout-index-proposal.out
...
VOID_AWARD_APPROVAL_CREATED_AT="$CREATED_AT" node "$APPROVAL_TOOL" \
  ...
  --reason "fixture award proposal approved for separate ledger write review" >/tmp/void-scratch-closeout-index-approval.out
...
VOID_LEDGER_WRITE_PACKET_CREATED_AT="$CREATED_AT" node "$LEDGER_PACKET_TOOL" \
  ...
  --reason "fixture ready for separate operator ledger append review" >/tmp/void-scratch-closeout-index-ledger-packet.out
...
VOID_LEDGER_APPEND_DRY_RUN_CREATED_AT="$CREATED_AT" node "$DRY_RUN_TOOL" \
  ...
  --reason "fixture dry-run before separate operator append" >/tmp/void-scratch-closeout-index-dry-run.out
...
VOID_LEDGER_APPEND_EXECUTE_PACKET_CREATED_AT="$CREATED_AT" node "$EXECUTE_PACKET_TOOL" \
  ...
  --reason "fixture ready for separate manual append execution" >/tmp/void-scratch-closeout-index-execute-packet.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CREATED_AT="$CREATED_AT" node "$RECEIPT_TOOL" \
  ...
  --reason "fixture receipt binds scratch ledger preview only" >/tmp/void-scratch-closeout-index-receipt.out
...
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_CREATED_AT="$CREATED_AT" node "$CLOSEOUT_TOOL" \
  ...
  --reason "fixture closeout binds scratch receipt only" >/tmp/void-scratch-closeout-index-closeout.out

printf '== scratch apply receipt closeout review index ==\n'
VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CREATED_AT="$CREATED_AT" node "$INDEX_TOOL" \
  --closeout "$CLOSEOUT" \
  --out "$INDEX_A" \
  --indexer "operator-review-indexer" \
  --reason "fixture index binds scratch closeout only" >/tmp/void-scratch-closeout-index-a.out

VOID_LEDGER_APPEND_SCRATCH_APPLY_RECEIPT_CLOSEOUT_REVIEW_INDEX_CREATED_AT="$CREATED_AT" node "$INDEX_TOOL" \
  --closeout "$CLOSEOUT" \
  --out "$INDEX_B" \
  --indexer "operator-review-indexer" \
  --reason "fixture index binds scratch closeout only" >/tmp/void-scratch-closeout-index-b.out

printf '== review index deterministic / binding ==\n'
python3 - "$CLOSEOUT" "$INDEX_A" "$INDEX_B" "$MARKER" "$CLOSEOUT_MARKER" "$RECEIPT_MARKER" "$SCRATCH_APPLY_MARKER" "$EXECUTE_PACKET_MARKER" <<'PY'
import json
import pathlib
import re
import sys
closeout_path, index_a_path, index_b_path, marker, closeout_marker, receipt_marker, scratch_marker, execute_marker = sys.argv[1:9]
closeout = json.load(open(closeout_path, encoding="utf-8"))
index_a_text = pathlib.Path(index_a_path).read_text(encoding="utf-8")
index_b_text = pathlib.Path(index_b_path).read_text(encoding="utf-8")
assert index_a_text == index_b_text
idx = json.loads(index_a_text)
assert idx["schema"] == "void.datanet.wc.evidence_packet_ledger_append_scratch_apply_receipt_closeout_review_index.v1"
assert idx["marker"] == marker
assert idx["status"] == "scratch_receipt_closeout_indexed_for_operator_review"
assert re.fullmatch(r"[0-9a-f]{64}", idx["scratch_apply_receipt_closeout_review_index_id"])
assert idx["indexer"] == "operator-review-indexer"
assert idx["closeout"]["path"] == str(pathlib.Path(closeout_path).resolve())
assert idx["closeout"]["marker"] == closeout_marker
assert idx["closeout"]["scratch_apply_receipt_closeout_id"] == closeout["scratch_apply_receipt_closeout_id"]
assert idx["receipt"]["marker"] == receipt_marker
assert idx["receipt"]["scratch_apply_receipt_id"] == closeout["receipt"]["scratch_apply_receipt_id"]
assert idx["scratch_apply"]["marker"] == scratch_marker
assert idx["scratch_apply"]["scratch_apply_id"] == closeout["scratch_apply"]["scratch_apply_id"]
assert idx["scratch_ledger"]["scratch_ledger_out_hash"] == closeout["scratch_ledger"]["scratch_ledger_out_hash"]
assert idx["scratch_ledger"]["appended_line_hash"] == closeout["scratch_ledger"]["appended_line_hash"]
assert idx["scratch_ledger"]["logical_candidate_next_ledger_hash"] == closeout["scratch_ledger"]["logical_candidate_next_ledger_hash"]
assert idx["source"]["execute_packet_marker"] == execute_marker
assert idx["source"]["execute_packet_id"] == closeout["source"]["execute_packet_id"]
summary = idx["review_index_summary"]
assert summary["scratch_receipt_closeout_bound"] is True
assert summary["scratch_preview_chain_indexed"] is True
assert summary["operator_review_index_only"] is True
...
policy = idx["work_credits_policy"]
...
assert policy["scratch_receipt_closeout_review_index_only"] is True
boundary = idx["boundary"]
assert boundary["scratch_apply_receipt_closeout_review_index_only"] is True
...
print("scratch_apply_receipt_closeout_review_index_deterministic_green=true")
print("scratch_apply_receipt_closeout_review_index_binding_green=true")
PY

printf '== bad closeout rejection ==\n'
python3 - "$CLOSEOUT" "$BAD_CLOSEOUT" <<'PY'
import json, sys
...
PY
if node "$INDEX_TOOL" \
  --closeout "$BAD_CLOSEOUT" \
  --out "$TMP_DIR/bad-closeout-index.json" \
  --indexer "operator-review-indexer" \
  --reason "bad closeout should fail" >/tmp/void-scratch-closeout-index-bad-closeout-out 2>/tmp/void-scratch-closeout-index-bad-closeout-err; then
  echo "bad_closeout_rejection_green=false"
  cat /tmp/void-scratch-closeout-index-bad-closeout-out
  exit 1
fi
grep -Fq "closeout_status_mismatch" /tmp/void-scratch-closeout-index-bad-closeout-err
echo "bad_closeout_rejection_green=true"

printf '== bad marker rejection ==\n'
python3 - "$CLOSEOUT" "$BAD_MARKER" <<'PY'
import json, sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
...
if node "$INDEX_TOOL" \
  --closeout "$BAD_MARKER" \
  --out "$TMP_DIR/bad-marker-index.json" \
  --indexer "operator-review-indexer" \
  --reason "bad marker should fail" >/tmp/void-scratch-closeout-index-bad-marker-out 2>/tmp/void-scratch-closeout-index-bad-marker-err; then
  echo "bad_marker_rejection_green=false"
  cat /tmp/void-scratch-closeout-index-bad-marker-out
  exit 1
fi
grep -Fq "closeout_marker_mismatch" /tmp/void-scratch-closeout-index-bad-marker-err
echo "bad_marker_rejection_green=true"

printf '== marker/source presence ==\n'
grep -Fq "$MARKER" "$INDEX_TOOL"
grep -Fq "$MARKER" "$DOC"
grep -Fq "$MARKER" "$0"
grep -Fq "scratch apply receipt closeout review index" "$DOC"
grep -Fq "scratch_apply_receipt_closeout_review_index" "$INDEX_TOOL"

printf '== forbidden WC cap wording scan ==\n'
if grep -RInE '100,000,000[[:space:]]+WC|100000000[[:space:]]+WC|WC[[:space:]]+supply[[:space:]]+cap|work[[:space:]]+credit[[:space:]]+supply[[:space:]]+cap|capped[[:space:]]+at[[:space:]]+100' "$INDEX_TOOL" "$DOC"; then
