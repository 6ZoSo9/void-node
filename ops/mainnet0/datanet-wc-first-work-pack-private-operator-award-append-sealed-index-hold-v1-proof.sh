...
BRICK="datanet-wc-first-work-pack-private-operator-award-append-sealed-index-hold-v1"
MARKER="VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_INDEX_HOLD_V1"
...
echo "== JSON parse / private award append sealed index binding =="
...
assert(record.index_type === "private_operator_award_append_sealed_index", "index type mismatch");
...
assert(record.sealed_index.private_award_append_final_seal_bound === true, "final seal index mismatch");
assert(record.sealed_index.private_award_append_chain_closeout_bound === true, "chain closeout index mismatch");
assert(record.sealed_index.effective_execution_final_seal_bound === true, "effective execution final seal index mismatch");
assert(record.sealed_index.index_state === "sealed_index_hold_for_review", "index state mismatch");
assert(record.sealed_index.execution_state === "not_executed", "execution state mismatch");
assert(record.sealed_index.ledger_append_state === "not_appended", "ledger append state mismatch");
assert(record.sealed_index.wc_policy === "unlimited_uncapped_useful_verifiable_work_accounting", "WC policy mismatch");
...
console.log("private_operator_award_append_sealed_index_binding_green=true");
NODE

echo "== forbidden public mutation scan =="
if git grep -n "$MARKER" -- public >/tmp/void-private-award-append-sealed-index-public-leak.txt 2>/dev/null; then
  cat /tmp/void-private-award-append-sealed-index-public-leak.txt
  echo "private_award_append_sealed_index_marker_not_in_public_tree_green=false"
  exit 1
fi
rm -f /tmp/void-private-award-append-sealed-index-public-leak.txt
echo "private_award_append_sealed_index_marker_not_in_public_tree_green=true"
...
echo "== result =="
echo "VOID_DATANET_WC_FIRST_WORK_PACK_PRIVATE_OPERATOR_AWARD_APPEND_SEALED_INDEX_HOLD_V1_GREEN"
