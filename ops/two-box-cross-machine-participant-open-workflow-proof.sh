...
OUT="${OUT:-/tmp/two-box-cross-machine-participant-open-workflow-proof-$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$OUT"

ACCOUNT="${ACCOUNT:-cross-machine-participant-open-user-$(date +%Y%m%d-%H%M%S)}"
PLAINTEXT="${PLAINTEXT:-cross machine participant open workflow $(date +%Y%m%d-%H%M%S)}"
...
echo
echo "=== [5] verify participant shell exposes consume-view path ==="
python3 - "$OUT/remote.participant.html" <<'PY' | tee "$OUT/participant-shell-summary.json"
import json, sys
html = open(sys.argv[1], "r", encoding="utf-8").read()
summary = {
    "ok": (
        "<html" in html.lower() and
        "networkValueCard" in html and
...
if not summary["ok"]:
    raise SystemExit("FAIL: participant shell does not expose consume-view path")
PY

echo
echo "=== [6] open Precision dataset through Alienware consume-view route ==="
...
echo
echo "=== [7] verify HTML render + local materialization on Alienware ==="
...
print(json.dumps(summary, indent=2))
if not summary["ok"]:
    raise SystemExit("FAIL: cross-machine participant open workflow did not pass cleanly")
PY

echo
echo "[ok] two-box cross-machine participant open workflow proof green"
